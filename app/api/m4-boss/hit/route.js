...
export async function POST(req) {
  ...
  const bossGx = Number(body.bossGx ?? M4_KIM_BOSS_SPAWN.gx)
  const bossGy = Number(body.bossGy ?? M4_KIM_BOSS_SPAWN.gy)
  ...
  if (Math.hypot(playerGx - bossGx, playerGy - bossGy) > M4_KIM_BOSS_HIT_RANGE) {
  ...
  return Response.json({
    ...
    maxHealth: Number(data?.max_health ?? 3500),
