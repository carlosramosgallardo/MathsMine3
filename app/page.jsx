...
function PageInner() {
  ...
  const topRef = useRef({ currentTop: null });
  ...
  const reconcileTop1Color = useCallback(async () => {
    ...
    topRef.current.currentTop = topLower;
    if (!topLower) return;
  ...
  /* ===== Auto-scroll compacto al board + verificación de borde inferior (con guard) ===== */
  useEffect(() => {
    ...
    <>
      ...
      <main className="relative z-10 flex flex-col items-center w-full px-4 pt-6 pb-20 text-lg font-mono text-white bg-black">
        <div className="w-full mx-auto">
          ...
          {/* Compacta toda la tipografía del carrusel (tabs incluidos) */}
