...
- **Bloque/Penalización** — Market NFTmoji block owned (with HEX link) and current active penalty from IRC commands (0 or negative, blinks when unpaid)
- Accuracy percentage across all answers

Paginated at 50 per page. Sortable by level, MM3, money, NFTmoji count, and EXECs.

### Bloque/Penalización column
...
- A wallet may own **at most 1 Market NFTmoji at a time**. Buying a new one auto-sells the current one (same resell rules apply).
...
2. Click **BUY** — in-game fiat balance is debited
3. The price equivalent in MM3 (at current global rate) is injected into the MM3 pool
4. If you held a different Market NFTmoji, it is automatically resold (50% return, 50% to pool)
...
### Claim flow (numeric answer)

1. Select a block — its detail card appears
2. The answer input unlocks when you have enough in-game funds
...
5. On match: in-game balance debited, block sealed permanently under your wallet
...
Command example (Genesis Uplink):
  wall '[Aserejee@freakingAI] solve => (log10(100000)*(4000+x))+(12*(300+x))+((6000+3*x)/3) = ?'
...
- The 5-digit result changes each time it is launched (regenerated on execution, stored in DB)
...
- Affected wallets see the penalty in the Prestige Bloque/Penalización column as a negative blinking value
...
- **Live presence** — compact connected user list sorted by most-recent connection first, shows up to 5 wallets by default with +5-at-a-time expansion
...
- **Wallet identity** — every message is authored by the wallet address (shortened)
- **System relay notices** — players see who connects and disconnects in real time
