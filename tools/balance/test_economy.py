...
from simulator import PlayerModel, median, play_day, simulate_cohort
...
def test_perfect_player_reaches_legend_inside_daily_cap():
    result = play_day(PlayerModel(accuracy=1, speed=0, name="perfect"), rng=Random(0))
...
def test_all_fail_player_stays_on_the_floor():
    result = play_day(PlayerModel(accuracy=0, speed=1, name="brick"), rng=Random(1))
...
def test_level_never_escapes_0_100():
    for accuracy in (0, 0.5, 1):
        result = play_day(PlayerModel(accuracy=accuracy, speed=0.2), start_level=90, rng=Random(7))
