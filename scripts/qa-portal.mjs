...
// Portal accesses are a stacked list (PortalCardList, LandingHero.jsx) —
// every row is always in the DOM at once, so there's no "rotate/select
// until it's centered" step anymore; a given href's row is just read
// directly by its data-portal-href.
async function waitHomePortalReady(page) {
  try {
    for (let i = 0; i < 3; i += 1) {
      await dismissHomeStageZoom(page)
      const ready = await page.evaluate(() => document.querySelectorAll('[data-portal-href]').length >= 10)
...
async function readPortalRowLabel(page, href) {
  return page.evaluate((target) => {
    const row = document.querySelector(`[data-portal-href="${target}"]`)
    const link = row?.querySelector('.mm3-portal-row-name')
...
async function runPhase2(page, base, { ok, nok, skip }) {
  ...
  try {
    ...
    // Home portal-list labels flip with language (Manifesto row)
    await goto(page, base, '/')
    await waitHomePortalReady(page)
    await setLanguage(page, 'es')
    await page.waitForFunction(
      () => localStorage.getItem('mm3-language') === 'es' && document.documentElement.lang === 'es',
      { timeout: 8000 },
    )
    await page.waitForTimeout(400)
    const homeEs = await readPortalRowLabel(page, '/manifesto')
    ...
    else nok('portal.lang.es.home.manifestoLabel', `row=${homeEs}`)

    await goto(page, base, '/')
    await waitHomePortalReady(page)
    await setLanguage(page, 'en')
    await page.waitForFunction(
      () => localStorage.getItem('mm3-language') === 'en' && document.documentElement.lang === 'en',
      { timeout: 8000 },
    )
    await page.waitForTimeout(400)
    const homeEn = await readPortalRowLabel(page, '/manifesto')
    ...
    else nok('portal.lang.en.home.manifestoLabel', `row=${homeEn}`)
  } catch (e) {
