...
function isRedirectToEvil(res, locationHeader) {
  ...
  try {
    return new URL(locationHeader, 'https://placeholder.invalid').hostname.toLowerCase() === EVIL_DOMAIN.toLowerCase()
  } catch {
    return false
  }
}
...
function isJsRedirect(res, locationHeader, body) {
  ...
  if (DANGEROUS_SCHEMES.some(scheme => loc.startsWith(scheme))) return true
