...
// Classify result: 200=fail (except leaderboard clamp probes), 500+leak=warn, 4xx/405=pass
...
function extractLeaderboardMeta(preview) {
  const pageMatch = preview.match(/"page"\s*:\s*(\d+)/)
  const limitMatch = preview.match(/"limit"\s*:\s*(\d+)/)
  if (!pageMatch || !limitMatch) return null
  return { page: Number(pageMatch[1]), limit: Number(limitMatch[1]) }
}

function classifyLeaderboardBounds(probeId, status, preview) {
  if (status !== 200) return classify(status, preview)
  const meta = extractLeaderboardMeta(preview)
  if (!meta) return classify(status, preview)
  const { page, limit } = meta
  if (probeId === 'int_page_negative') {
    if (page >= 1 && limit >= 1 && limit <= 200) {
      return { result: 'pass', note: `HTTP 200 with clamped page=${page}, limit=${limit}` }
    }
    return { result: 'fail', note: 'HTTP 200 but negative page/limit were not clamped in response' }
  }
  if (probeId === 'int_limit_overflow') {
    if (limit >= 1 && limit <= 200) {
      return { result: 'pass', note: `HTTP 200 with limit clamped to ${limit}` }
    }
    return { result: 'fail', note: 'HTTP 200 but oversized limit was not clamped to max 200' }
  }
  return classify(status, preview)
}
...
export async function runInjectionScanCheck(siteUrl) {
  ...
  const PROBES = [
    ...
    },
    {
      ...
      rationale: 'chip is Number(body.chip) — Number([1,2]) = NaN, rejected by chip validation. Tests NaN handling in chip number check.',
  ...
  const findings = PROBES.map((probe, i) => {
    ...
    const { result, note } = (probe.id === 'int_page_negative' || probe.id === 'int_limit_overflow')
      ? classifyLeaderboardBounds(probe.id, r.status, r.preview)
      : classify(r.status, r.preview)
