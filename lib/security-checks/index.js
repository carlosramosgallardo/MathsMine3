...
export const CHECKS = [
  ...
  { id: 'security_headers', weight: 13, run: url => runHeadersCheck(url)        },
  { id: 'api_auth',         weight: 12, run: url => runApiAuthCheck(url)        },
  { id: 'sensitive_paths',  weight: 10, run: url => runSensitivePathsCheck(url) },
  { id: 'cors_policy',      weight: 8,  run: url => runCorsPolicyCheck(url)     },
  { id: 'http_methods',     weight: 5,  run: url => runHttpMethodsCheck(url)    },
  { id: 'wallet_auth',      weight: 17, run: url => runWalletAuthCheck(url)     },
  { id: 'injection_scan',   weight: 25, run: url => runInjectionScanCheck(url)  },
]
