...
export async function runBusinessLogicCheck(siteUrl) {
  ...
  const PROBES = [
    ...
    // ── 11. Chain solve — MAX_SAFE_INTEGER answer ───────────────────────────
    {
      ...
      expect: [200, 400, 401, 409, 429], // 200 is OK (wrong answer is fine), 429 is daily limit
    ...
    // ── 13. PvP hit — self-attack ─────────────────────────────────────────────
    {
      id: 'pvp_self_hit',
      label: 'pvp-hit — attacker === victim (self-attack)',
      category: 'Business Logic',
      severity: 'LOW',
      endpoint: 'POST /api/pvp-hit',
      body: { attacker: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', victim: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' },
      expect: [400, 401, 403],
      rationale: 'attacker === victim must be rejected by the invalid_params guard. Self-damage would corrupt HP values and could enable self-kill farming for death-respawn exploits.',
      attacks: 'Self-hit to force own death cooldown bypass, HP manipulation via self-damage loop',
      run: () => post(`${siteUrl}/api/pvp-hit`, { attacker: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', victim: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266' }),
    },

    // ── 14. pvp-death POST — write kill with no auth (CRITICAL) ──────────────
    {
      id: 'pvp_death_no_auth',
      label: 'pvp-death — write 5-min kill state with no auth',
      category: 'Missing Auth',
      severity: 'CRITICAL',
      endpoint: 'POST /api/pvp-death',
      body: { wallet: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', gx: 14.5, gy: 14.5 },
      expect: [401, 403],
      rationale: 'pvp-death POST upserts pvp_dead_until for any wallet with SERVICE_ROLE access and NO authentication. Any unauthenticated caller can force-kill any player, triggering a 5-minute game lockout for the victim.',
      attacks: 'Griefing via forced kill injection — any IP can lock out any wallet indefinitely (1 req / 5 min per victim), disabling mining, training, relaying, and PvP',
      run: () => post(`${siteUrl}/api/pvp-death`, { wallet: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', gx: 14.5, gy: 14.5 }),
    },
  ]
  ...
  const findings = PROBES.map((probe, i) => {
    ...
    // For unauth-write probes: if status 200, it's a FAIL (missing auth). Flag specially.
    const isNoAuthFail = (probe.id === 'macro_no_auth' || probe.id === 'pvp_death_no_auth') && r.status === 200

    const status   = isNoAuthFail ? 'fail' : res === 'pass' ? 'pass' : res === 'warn' ? 'warn' : 'fail'
  ...
  return {
    ...
    probeDetails: {
      ...
      probeMatrix: {
        ...
        'Missing Auth':    'Economy/PvP endpoints without wallet or token authentication (nudge-macro, pvp-death)',
        'Business Logic':  'Self-relay, self-hit, pool operations without membership, phantom vote',
