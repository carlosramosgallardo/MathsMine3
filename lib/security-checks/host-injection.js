...
function locationHostMatches(locationHeader, evilHost) {
  ...
  try {
    const url = new URL(locationHeader, 'http://placeholder.invalid')
    return url.hostname === evilHost
  } catch {
    return false
  }
}
...
export async function runHostInjectionCheck(siteUrl) {
  ...
  const results = await Promise.allSettled(
    allProbes.map(async ({ header, path }) => {
      ...
      // Location is a URL: compare hostnames exactly. Body/cookie are free-text content
      // reflection checks (not host validation), so a plain substring scan is appropriate there.
      const reflected = locationHostMatches(location, EVIL_HOST) || [body, cookie].some(s => s.includes(EVIL_HOST))
