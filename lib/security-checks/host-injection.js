...
function hostEquals(candidate, evilHost) {
  if (!candidate) return false
  try {
    return new URL(candidate, 'https://placeholder.invalid').hostname.toLowerCase() === evilHost.toLowerCase()
  } catch {
    return false
  }
}

// Exact-host reflection check for a Location/redirect target — parses the URL
// rather than substring-matching, so a lookalike host (e.g. "evil-attacker.example.com.attacker.net")
// can't produce a false positive/negative.
function locationHostMatches(locationHeader, evilHost) {
  return hostEquals(locationHeader, evilHost)
}

/** Set-Cookie Domain= attribute equals evil host (not a lookalike substring). */
function cookieDomainMatches(cookieHeader, evilHost) {
  if (!cookieHeader) return false
  for (const part of cookieHeader.split(';')) {
    const trimmed = part.trim()
    if (!/^domain=/i.test(trimmed)) continue
    const raw = trimmed.slice(trimmed.indexOf('=') + 1).trim().replace(/^\./, '')
    if (hostEquals(`http://${raw}`, evilHost)) return true
  }
  return false
}

/**
 * Body reflection: only count parsed hostnames (URLs / host= / DNS-shaped tokens),
 * never a raw substring includes() — that is bypassable and trips CodeQL.
 */
function bodyReflectsHost(body, evilHost) {
  if (!body) return false
  const candidates = []
  for (const m of body.matchAll(/\bhttps?:\/\/[^\s"'<>]+/gi)) candidates.push(m[0])
  for (const m of body.matchAll(/\/\/[^\s"'<>]+/gi)) candidates.push(`http:${m[0]}`)
  for (const m of body.matchAll(/(?:^|[^\w.-])host=([^"\s,;]+)/gi)) candidates.push(`http://${m[1]}`)
  for (const m of body.matchAll(/\b[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)+\b/gi)) {
    candidates.push(`http://${m[0]}`)
  }
  return candidates.some((c) => hostEquals(c, evilHost))
}
...
export async function runHostInjectionCheck(siteUrl) {
  ...
  const results = await Promise.allSettled(
    allProbes.map(async ({ header, path }) => {
      ...
      const reflected =
        locationHostMatches(location, EVIL_HOST) ||
        cookieDomainMatches(cookie, EVIL_HOST) ||
        bodyReflectsHost(body, EVIL_HOST)
  ...
  try {
    ...
    // Anchor the host boundary (protocol prefix + non-hostname char or end) so a
    // lookalike domain containing our hostname as a substring can't false-match.
    const downgradeRe = new RegExp(`http://${siteHost.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:[:/"'?#\\s]|$)`, 'i')
    const downgrades = location.startsWith('http://') || downgradeRe.test(body)
    if (downgrades) {
      findings.push({
