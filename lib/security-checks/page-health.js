...
const LEAK_PATTERNS = [
  ...
  // Require a word boundary before pg_ so hashed asset names (e.g. icon.*mpg_vz.png)
  // do not false-positive; only match known Postgres internals / error tokens.
  { re: /(?:^|[^a-z0-9])pg_(?:sleep|catalog|stat|statements|proc|class|constraint|type|get_function)\b/i, label: 'PostgreSQL internal keyword', severity: 'MEDIUM' },
