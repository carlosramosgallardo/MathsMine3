...
const ENDPOINTS = [
  ...
  },
  {
    ...
    method: 'POST', path: '/api/chain-check', expect: [401],
    label: 'POST /api/chain-check — session required',
    section: 'Training',
  ...
  {
    method: 'POST', path: '/api/m3-boss/attack', expect: [400, 401, 403],
  ...
  {
    method: 'POST', path: '/api/m4-boss/attack', expect: [400, 401, 403],
    label: 'POST /api/m4-boss/attack — empty body', section: 'World Bosses',
    rationale: 'Boss-on-player damage requires wallet + positions. Empty body must reject before any HP write.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/m5-boss/attack', expect: [400, 401, 403],
    label: 'POST /api/m5-boss/attack — empty body', section: 'World Bosses',
    rationale: 'Boss-on-player damage requires wallet + positions. Empty body must reject before any HP write.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/m3-boss/hit', expect: [400, 401, 403],
  ...
  {
    method: 'POST', path: '/api/m4-boss/hit', expect: [400, 401, 403],
    label: 'POST /api/m4-boss/hit — empty body', section: 'World Bosses',
    rationale: 'Player-on-boss hit requires wallet + positions. Empty body must reject before boss HP write.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/m5-boss/hit', expect: [400, 401, 403],
  ...
  // nftji-buy/nftji-resell/code-redeem were removed (dead routes, no caller —
  // real buy/resell flow is /api/mine-block + /api/resell-nftji below). Confirm
  // they stay gone rather than silently reappearing unauthenticated.
  {
    method: 'POST', path: '/api/mining/nftji-buy', expect: [404],
    label: 'POST /api/mining/nftji-buy — removed route stays removed', section: 'Mining',
    rationale: 'Dead mass-assignment route was deleted; a 200/other status here means it came back.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/mining/nftji-resell', expect: [404],
    label: 'POST /api/mining/nftji-resell — removed route stays removed', section: 'Mining',
    rationale: 'Dead mass-assignment route was deleted; a 200/other status here means it came back.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/mining/code-redeem', expect: [404],
    label: 'POST /api/mining/code-redeem — removed route stays removed', section: 'Mining',
    rationale: 'Dead mass-assignment route was deleted; a 200/other status here means it came back.',
    body: '{}',
  },
  {
    method: 'POST', path: '/api/chain-solve/demine', expect: [400, 401, 403, 409],
  ...
  {
    method: 'POST', path: '/api/resell-nftji', expect: [400, 401, 404],
  ...
  {
    method: 'POST', path: '/api/pool-heal', expect: [400, 401, 403, 409],
  ...
  {
    method: 'POST', path: '/api/rl-mount', expect: [400, 401, 403],
  ...
  {
    method: 'POST', path: '/api/redeem-penalty', expect: [400, 401, 404],
