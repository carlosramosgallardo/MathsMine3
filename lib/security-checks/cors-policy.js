const PROBES = [
  // publicPage: true — ACAO:* is expected for public HTML, only flag reflected evil origin
  { path: '/',                        method: 'GET',  body: null, label: 'Home — arbitrary origin read',       publicPage: true  },
  { path: '/api/status',              method: 'GET',  body: null, label: 'Status API — arbitrary origin read', publicPage: false  },
  { path: '/api/daily-tasks/claim',   method: 'POST', body: '{}', label: 'Claim API — cross-origin POST',      publicPage: false  },
]
...
export async function runCorsPolicyCheck(siteUrl) {
  ...
  for (const probe of PROBES) {
    ...
    // ACAO:* on public HTML pages is expected Next.js behaviour — not a vulnerability.
    // Only flag wildcard as a problem on API endpoints returning data.
    const isFail = credWithEvil || reflectsEvil
    const isWarn = wildcardOpen && !credWithEvil && !probe.publicPage
    ...
    else if (reflectsEvil) rationale = 'ACAO reflects arbitrary origin — attacker can read cross-origin responses from this endpoint.'
    else if (wildcardOpen && probe.publicPage) rationale = 'ACAO: * on a public HTML page — expected behaviour, not a vulnerability. Public pages carry no sensitive data.'
    else if (wildcardOpen) rationale = 'ACAO: * on API endpoint — any origin can read the response. Acceptable only if the endpoint returns no sensitive data.'
    else rationale = 'CORS policy correctly ignores the attacker origin — browser blocks cross-origin read.'
