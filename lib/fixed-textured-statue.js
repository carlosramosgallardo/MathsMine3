...
import { attachCapsuleAnimDriver, configureCapsuleVisibility } from './capsule-anim-driver'
import { mountRigidBipedLimbs } from './rigid-biped-limbs'

/**
 * Textured multi-mesh GLB. Plaza statues (Macron/Zelensky/Milei) sit on the
 * shared Roman column base; rigid bosses (Kim/Putin) plant on the floor. Optional
...
export function createFixedTexturedStatueVisual(THREE, {
  ...
  walkableStatue = true,
  /** Roman column base under the figure (Macron/Zelensky/Milei). */
  withPlinth = false,
  ...
  /** Funny per-statue arm idle on the live RPM skeleton. */
  plazaArmIdle = null,
  capsuleBulk = 1,
  capsuleSleeve = 'long',
  ...
  if (plazaArmIdle) bodyPivot.userData.plazaArmIdle = plazaArmIdle

  if (withPlinth) {
    attachStatuePlinth(THREE, group, { retro })
  }
  ...
  const finishModel = (ok) => {
    ...
    // Retro keeps the voxel capsule on screen while its tier streams (the
    // non-retro path hid it up front in attachCapsuleAnimDriver); once the
    // pixel-art body is attached, retire the stand-in the same way.
    if (ok && retro && capsuleParts) configureCapsuleVisibility(bodyPivot, capsuleParts, false)
  ...
  if (skinnedDriver && withCapsuleDriver) {
    attachHumanoidGlb(THREE, bodyPivot, {
      ...
      retro,
      onReady: (clone) => {
        if (clone && rigidLimbDriver) mountRigidBipedLimbs(THREE, bodyPivot, bodyPivot.userData.statueGlbFit, clone)
  ...
  bodyPivot.traverse((obj) => {
    ...
    // Mining culls the rigid capsule/placeholder parts; Home keeps them drawn.
    obj.frustumCulled = retro
