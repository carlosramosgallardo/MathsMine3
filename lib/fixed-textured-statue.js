...
export function createFixedTexturedStatueVisual(THREE, {
  ...
  if (withCapsuleDriver) {
    const parts = attachCapsuleAnimDriver(THREE, bodyPivot, {
      ...
      lowDetail,
      // Plaza statues strip the T-pose body; show capsules. Combat bosses keep
      // the full textured mesh, so limbs stay invisible drivers only.
      visibleLimbs: withPlinth,
    })
