...
export function createFixedTexturedStatueVisual(THREE, {
  ...
  tokenTex.colorSpace = THREE.SRGBColorSpace
  const tokenMatOpts = {
    map: tokenTex,
    roughness: 0.14,
    metalness: 0.85,
  ...
  const tokenSide = new THREE.MeshStandardMaterial(tokenMatOpts)
  tokenSide.userData.ownedMap = true
  const tokenSide2 = tokenSide.clone()
  tokenSide2.userData.ownedMap = true
