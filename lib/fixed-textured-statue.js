...
export function createFixedTexturedStatueVisual(THREE, {
  ...
  eyeSize = 0.07,
  eyeLine = 0.48,
  /** Planted props face −Z after π yaw — lock so hair/gear cannot flip glows aft. */
  eyeFaceSign = -1,
  eyeForwardFrac = 0.045,
  eyeSkullFrac = 0.18,
  eyeSpacingFrac = 0.11,
  shadowRadius = 0.58,
  renderOrder = 5,
  ...
  const finishModel = (ok) => {
    ...
    if (ok) {
      placeEyeGlowsFromFit(THREE, bodyPivot, {
        eyeLine,
        size: eyeSize,
        faceSign: eyeFaceSign,
        forwardFrac: eyeForwardFrac,
        skullFrac: eyeSkullFrac,
        spacingFrac: eyeSpacingFrac,
      })
    }
