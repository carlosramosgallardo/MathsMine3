...
/** M1: full-width stone causeways spanning the entire edge (not band-limited). */
function buildFullEdgeGatewayCauseways(direction) {
  if (direction === 'north' || direction === 'south') {
    const shoreRow = direction === 'south' ? PLAY_MAX : PLAY_MIN
    const seaRow = direction === 'south' ? PLAY_MAX + GATEWAY_CAUSEWAY_SEA_EXTENT : PLAY_MIN - GATEWAY_CAUSEWAY_SEA_EXTENT
    return [{
...
const ARENA_STEPS = Object.freeze([0.75, 1.45, 2.15, 2.85, 3.55])
const ARENA_ARCADE_HEIGHT = 3.85
...
/** Walkable cells: perimeter ring + compass cross + full south gateway strip to M1. */
function isPerimeterFree(row, col) {
  if (row <= 4 || col <= 4 || col >= 51) return true
  if (row >= 49) return true
  if (row >= 36) return true
...
function frostColiseum() {
  ...
  for (let r = PLAY_MIN; r <= PLAY_MAX; r += 1) {
    for (let c = PLAY_MIN; c <= PLAY_MAX; c += 1) {
      ...
      if ((r + c) % 2 === 0) out.push(wall(`${r},${c}`, W_SLATE, ARENA_ARCADE_HEIGHT, 'ARENA ARCADE', { biome }))
    }
  }
  return out
}
...
function buildFrostColiseumDecor() {
  ...
  for (let r = PLAY_MIN; r <= PLAY_MAX; r += 1) {
    for (let c = PLAY_MIN; c <= PLAY_MAX; c += 1) {
      ...
      arches.push({
        ...
        top: isGateRing ? 1.15 : ARENA_ARCADE_HEIGHT,
        span: isGateRing ? 2.05 : 2.1,
        rise: isGateRing ? 0.9 : 1.62,
      })
    }
  }
  const panels = []
  ...
  for (let i = 0; i < 32; i += 1) {
    ...
    if (Math.abs(pz - cz) > 2.5 && Math.abs(px - cx) > 2.5) {
      panels.push({ x: px, z: pz, y: 2.18, yaw: Math.atan2(cx - px, cz - pz), tilt: -0.98, width: 2.9, height: 3.05, side })
    }
    const qx = cx + Math.cos(theta) * ARENA.b * 1.02
    const qz = cz + Math.sin(theta) * ARENA.a * 1.02
    if (Math.abs(qz - cz) > 2.8 && Math.abs(qx - cx) > 2.8) {
      canopies.push({ x: qx, z: qz, y: 4.35, yaw: Math.atan2(cx - qx, cz - qz), tilt: 0.38, width: 3.25, height: 2.25, side: qx >= cx ? 'east' : 'west' })
    }
  }
  const banners = []
  for (const arch of arches) {
    if (arch.top < 2) continue
    banners.push({ x: arch.x, z: arch.z, y: 3.05, yaw: Math.atan2(cx - arch.x, cz - arch.z), side: arch.x >= cx ? 'east' : 'west' })
  }
  const topiary = []
...
function canPlaceM3SceneryDecor(x, z) {
  const row = z
  const col = x
  if (isPeachPerimeterFree(Math.floor(row), Math.floor(col))) return false
  if (inPeachCastleFootprint(Math.floor(row), Math.floor(col))) return false
  const dr = row - M3_PUTIN_BOSS_SPAWN.gy
  const dc = col - M3_PUTIN_BOSS_SPAWN.gx
  if (dr * dr + dc * dc < 100) return false
  if (Math.abs(col - PEACH.roadCol) <= 1.5 && row >= 19 && row <= 48) return false
  if (Math.abs(row - 30) <= 1.2 && col >= 8 && col <= 46) return false
  return true
}
...
function buildPeachCastleDecor() {
  ...
  const horseSpots = [
    { x: 10.5, z: 22, yaw: Math.PI / 2 },
    { x: 14.5, z: 28, yaw: 0.2 },
    { x: 18.5, z: 34, yaw: Math.PI / 2 + 0.15 },
    { x: 11.5, z: 40, yaw: -Math.PI / 2 },
    { x: 38.5, z: 24, yaw: -Math.PI / 2 },
    { x: 42.5, z: 30, yaw: Math.PI },
    { x: 36.5, z: 36, yaw: -Math.PI / 4 },
    { x: 44.5, z: 40, yaw: Math.PI / 2 },
    { x: 20.5, z: 30.5, yaw: 0 },
    { x: 34.5, z: 30.5, yaw: Math.PI },
  ]
  const monumentSpots = [
    { x: 20, z: 20, yaw: 0, kind: 'star' },
    { x: 35, z: 20, yaw: Math.PI, kind: 'star' },
    { x: 15, z: 26, yaw: Math.PI / 2, kind: 'hammer' },
    { x: 40, z: 32, yaw: -Math.PI / 2, kind: 'hammer' },
    { x: 22, z: 38, yaw: 0, kind: 'worker' },
    { x: 33, z: 38, yaw: Math.PI, kind: 'worker' },
    { x: 12, z: 32, yaw: 0.35, kind: 'worker' },
    { x: 43, z: 26, yaw: -0.35, kind: 'star' },
    { x: 18, z: 22, yaw: Math.PI / 4, kind: 'hammer' },
    { x: 37, z: 22, yaw: (-3 * Math.PI) / 4, kind: 'hammer' },
  ]
  const horses = horseSpots.filter(({ x, z }) => canPlaceM3SceneryDecor(x, z))
  const monuments = monumentSpots.filter(({ x, z }) => canPlaceM3SceneryDecor(x, z))
...
function canPlaceMilitaryDecor(x, z) {
  const row = z
  const col = x
  if (isDesertPerimeterFree(Math.floor(row), Math.floor(col))) return false
  if (inOasisTowerFootprint(Math.floor(row), Math.floor(col))) return false
  const dr = row - OASIS.row
  const dc = col - OASIS.col
  if (dr * dr + dc * dc < 100) return false
...
function buildDesertOasisDecor() {
  ...
  const watchtowerSpots = [
    { x: 22, z: 10, yaw: Math.PI / 4 },
    { x: 34, z: 10, yaw: -Math.PI / 4 },
    { x: 22, z: 44, yaw: (3 * Math.PI) / 4 },
    { x: 34, z: 44, yaw: (-3 * Math.PI) / 4 },
    { x: 24, z: 19, yaw: 0 },
    { x: 24, z: 36, yaw: Math.PI },
    { x: 36, z: 16, yaw: -Math.PI / 2 },
    { x: 36, z: 40, yaw: -Math.PI / 2 },
  ]
  const tankSpots = [
    { x: 21, z: 24, yaw: Math.PI / 2 },
    { x: 21, z: 31, yaw: Math.PI / 2 },
    { x: 32, z: 22, yaw: -Math.PI / 4 },
    { x: 32, z: 33, yaw: (-3 * Math.PI) / 4 },
    { x: 25, z: 12, yaw: Math.PI / 2 },
    { x: 25, z: 43, yaw: -Math.PI / 2 },
    { x: 29, z: 18, yaw: 0.15 },
    { x: 29, z: 37, yaw: -0.15 },
  ]
  const cannonSpots = [
    { x: 20, z: 18, yaw: Math.PI / 2 },
    { x: 20, z: 37, yaw: Math.PI / 2 },
    { x: 30, z: 14, yaw: 0.35 },
    { x: 30, z: 41, yaw: -0.35 },
    { x: 38, z: 24, yaw: -Math.PI / 2 },
    { x: 38, z: 31, yaw: -Math.PI / 2 },
    { x: 23, z: 42, yaw: Math.PI / 2 + 0.2 },
    { x: 23, z: 13, yaw: Math.PI / 2 - 0.2 },
  ]
  const watchtowers = watchtowerSpots.filter(({ x, z }) => canPlaceMilitaryDecor(x, z))
  const tanks = tankSpots.filter(({ x, z }) => canPlaceMilitaryDecor(x, z))
  const cannons = cannonSpots.filter(({ x, z }) => canPlaceMilitaryDecor(x, z))
...
// ── Map 5 — Isla Cristal (full map) ─────────────────────────────────────────
// Bioluminescent fantasy island: purple cliffs, cyan/purple crystals, glowing
// mushrooms and tiered mystic trees. East gateway spines stay clear for M1.
const MYSTIC = Object.freeze({ row: 27.5, col: 27.5 })
const MYSTIC_HEART_BOUNDS = Object.freeze({ minRow: 17, maxRow: 37, minCol: 3, maxCol: 18 })

function isMysticPerimeterFree(row, col) {
  if (row <= 4 || col <= 4 || col >= 51 || row >= 49) return true
  if (col >= 36) return true
  if (onMysticPath(row, col)) return true
  const dr = row - MYSTIC.row
  const dc = col - MYSTIC.col
  // Boss arena — no minable blocks in the centre (see map-boss-mining-exclusion.js)
  if (dr * dr + dc * dc <= 81) return true
  return false
}

function onMysticPath(row, col) {
  const pathCol = MYSTIC.col + Math.sin((row - 8) * 0.11) * 5.5
  if (Math.abs(col - pathCol) <= 1.4) return true
  if (Math.abs(row - MYSTIC.row) <= 1.4 && col >= 10 && col <= 44) return true
  return false
}

function inMysticHeartFootprint(row, col) {
  if (row < MYSTIC_HEART_BOUNDS.minRow || row > MYSTIC_HEART_BOUNDS.maxRow) return false
  if (col < MYSTIC_HEART_BOUNDS.minCol || col > MYSTIC_HEART_BOUNDS.maxCol) return false
  if (col >= 15 && row >= 25 && row <= 30) return false
  return true
}

function mysticRockCell(row, col) {
  if (isMysticPerimeterFree(row, col)) return false
  if (inMysticHeartFootprint(row, col)) return false
  const n = seededMapAmbient(row * 97 + col * 59)
  const edgeCliff = col <= 10 || col >= 46 || row <= 10 || row >= 44
  const northWood = row < 24 && col > 12 && col < 42
  const southWood = row > 32 && col > 12 && col < 42
  if (!(edgeCliff || northWood || southWood)) return false
  return n > 0.38
}

function mysticCrystalCell(row, col) {
  if (isMysticPerimeterFree(row, col)) return false
  if (inMysticHeartFootprint(row, col)) return false
  const n = seededMapAmbient(row * 151 + col * 83)
  const northField = row < 26 && col > 18 && col < 36
  const southField = row > 30 && col > 18 && col < 36
  if (!(northField || southField)) return false
  return n > 0.52
}

function mysticIsleMap() {
  const out = []
  for (let r = PLAY_MIN; r <= PLAY_MAX; r += 1) {
    for (let c = PLAY_MIN; c <= PLAY_MAX; c += 1) {
      if (isMysticPerimeterFree(r, c)) continue
      if (inMysticHeartFootprint(r, c)) {
        const core = c <= 10 && r >= 22 && r <= 32
        const height = core
          ? 3.2 + (32 - r) * 0.08
          : 1.8 + seededMapAmbient(r * 19 + c * 29) * 0.75
        out.push(wall(`${r},${c}`, W_ICE, Math.min(4.4, height), 'MYSTIC HEART', { biome: 'ice' }))
        continue
      }
      if (mysticCrystalCell(r, c)) {
        const height = 1.05 + seededMapAmbient(r * 37 + c * 43) * 2.2
        const biome = (r + c) % 2 === 0 ? 'ice' : 'mountain'
        out.push(wall(`${r},${c}`, W_ICE, height, 'MYSTIC CRYSTAL', { biome }))
        continue
      }
      if (!mysticRockCell(r, c)) continue
      const height = 0.85 + seededMapAmbient(r * 41 + c * 67) * 1.65
      out.push(wall(`${r},${c}`, W_DARK, height, 'MYSTIC ROCK', { biome: 'mountain' }))
    }
  }
  return out
}

function canPlaceM5SceneryDecor(x, z) {
  const row = z
  const col = x
  if (isMysticPerimeterFree(Math.floor(row), Math.floor(col))) return false
  if (inMysticHeartFootprint(Math.floor(row), Math.floor(col))) return false
  const dr = row - (MYSTIC.row + 0.5)
  const dc = col - (MYSTIC.col + 0.5)
  if (dr * dr + dc * dc < 100) return false
  if (onMysticPath(Math.floor(row), Math.floor(col))) return false
  return true
}

function buildMysticIsleDecor() {
  const cx = MYSTIC.col + 0.5
  const cz = MYSTIC.row + 0.5
  const crystals = []
  for (let i = 0; i < 34; i += 1) {
    const x = 18 + seededMapAmbient(i * 61 + 4) * 28
    const z = 10 + seededMapAmbient(i * 67 + 8) * 34
    if (isMysticPerimeterFree(Math.floor(z), Math.floor(x))) continue
    if (inMysticHeartFootprint(Math.floor(z), Math.floor(x))) continue
    crystals.push({
      x, z,
      height: 1.2 + seededMapAmbient(i * 71 + 12) * 2.4,
      tone: i % 2 === 0 ? 'cyan' : 'purple',
      yaw: seededMapAmbient(i * 73 + 16) * Math.PI * 2,
    })
  }
  const mushrooms = []
  for (let i = 0; i < 38; i += 1) {
    const x = 10 + seededMapAmbient(i * 79 + 2) * 34
    const z = 12 + seededMapAmbient(i * 83 + 6) * 30
    if (isMysticPerimeterFree(Math.floor(z), Math.floor(x))) continue
    if (onMysticPath(Math.floor(z), Math.floor(x))) continue
    mushrooms.push({ x, z, scale: 0.55 + seededMapAmbient(i * 89 + 10) * 0.65 })
  }
  const trees = []
  for (let i = 0; i < 26; i += 1) {
    const x = 12 + seededMapAmbient(i * 91 + 3) * 30
    const z = 8 + seededMapAmbient(i * 97 + 7) * 38
    if (isMysticPerimeterFree(Math.floor(z), Math.floor(x))) continue
    if (Math.abs(x - cx) < 3 && Math.abs(z - cz) < 3) continue
    trees.push({ x, z, scale: 0.9 + seededMapAmbient(i * 101 + 11) * 0.5, tone: i % 2 === 0 ? 'violet' : 'teal' })
  }
  const wisps = []
  for (let i = 0; i < 22; i += 1) {
    wisps.push({
      x: 14 + seededMapAmbient(i * 103 + 5) * 26,
      z: 14 + seededMapAmbient(i * 107 + 9) * 26,
      phase: i * 0.9,
    })
  }

  const teslaSpots = [
    { x: 22, z: 14, yaw: 0.15, kind: 'model3' },
    { x: 32, z: 16, yaw: -Math.PI / 4, kind: 'cybertruck' },
    { x: 20, z: 22, yaw: Math.PI / 2, kind: 'model3' },
    { x: 34, z: 24, yaw: Math.PI, kind: 'cybertruck' },
    { x: 18, z: 34, yaw: 0.4, kind: 'model3' },
    { x: 30, z: 38, yaw: -Math.PI / 3, kind: 'cybertruck' },
    { x: 24, z: 42, yaw: Math.PI / 2 + 0.2, kind: 'model3' },
    { x: 36, z: 18, yaw: -0.2, kind: 'model3' },
  ]
  const rocketSpots = [
    { x: 20, z: 38, yaw: 0, vertical: true },
    { x: 34, z: 40, yaw: Math.PI / 6, vertical: true },
    { x: 24, z: 12, yaw: -Math.PI / 5, vertical: false },
    { x: 32, z: 14, yaw: Math.PI / 4, vertical: false },
    { x: 16, z: 28, yaw: 0, vertical: true },
    { x: 38, z: 32, yaw: -Math.PI / 8, vertical: true },
  ]
  const starlinkSpots = [
    { x: 18, z: 22, yaw: 0.35, kind: 'dish' },
    { x: 30, z: 34, yaw: -0.25, kind: 'satellite' },
    { x: 14, z: 40, yaw: 0.5, kind: 'dish' },
    { x: 26, z: 18, yaw: Math.PI / 3, kind: 'satellite' },
    { x: 22, z: 36, yaw: -Math.PI / 4, kind: 'dish' },
    { x: 35, z: 26, yaw: 0.1, kind: 'satellite' },
    { x: 16, z: 16, yaw: 0.8, kind: 'dish' },
    { x: 28, z: 42, yaw: -0.6, kind: 'satellite' },
  ]
  const teslas = teslaSpots.filter(({ x, z }) => canPlaceM5SceneryDecor(x, z))
  const rockets = rocketSpots.filter(({ x, z }) => canPlaceM5SceneryDecor(x, z))
  const starlinks = starlinkSpots.filter(({ x, z }) => canPlaceM5SceneryDecor(x, z))

  return {
    center: { x: cx, z: cz },
    heartGate: { x: 16.5, z: MYSTIC.row + 0.5 },
    glade: { x: cx, z: cz, radius: 3.6 },
    heartSpire: { x: 8.5, z: MYSTIC.row + 0.5, height: 8.8, radius: 2.8 },
    crystals,
    mushrooms,
    trees,
    wisps,
...
export const MYSTIC_ISLE_ARENA = MYSTIC
export const MYSTIC_ISLE_DECOR = Object.freeze(buildMysticIsleDecor())
...
/** Map 4 — entire map is Korea (desert oasis venue). */
const MAP_4_AMBIENT = [
  ...desertOasisMap(),
]
/** Map 5 — entire map is Epstein Island. */
const MAP_5_AMBIENT = [
  ...mysticIsleMap(),
]
...
/** Walkable cells on peripheral maps — safe for minable block placement. */
export function isMiningMapPerimeterFree(mapId, row, col) {
  switch (String(mapId)) {
    case '2': return isPerimeterFree(row, col)
    case '3': return isPeachPerimeterFree(row, col)
    case '4': return isDesertPerimeterFree(row, col)
    case '5': return isMysticPerimeterFree(row, col)
...
/** Full-edge plaza + causeway on the side that connects back to M1. */
function buildPeripheralM1GatewayFeatures(edge) {
  if (!edge) return []
  return [
    ...buildFullEdgeGatewayCauseways(edge),
    ...(edge === 'north'
      ? [{ kind: 'plaza', minRow: 1, maxRow: 18, minCol: PLAY_MIN, maxCol: PLAY_MAX }]
      : edge === 'south'
        ? [{ kind: 'plaza', minRow: 36, maxRow: 54, minCol: PLAY_MIN, maxCol: PLAY_MAX }]
        : edge === 'west'
          ? [{ kind: 'plaza', minRow: PLAY_MIN, maxRow: PLAY_MAX, minCol: 1, maxCol: 18 }]
          : [{ kind: 'plaza', minRow: PLAY_MIN, maxRow: PLAY_MAX, minCol: 36, maxCol: 54 }]),
  ]
}
...
function buildMap3GroundFeatures() {
  ...
  features.push(...buildPeripheralM1GatewayFeatures('north'))
...
function buildMap4GroundFeatures() {
  ...
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 50.8, maxCol: 54 })
  features.push({ kind: 'plaza', minRow: 48.8, maxRow: 54, minCol: 1, maxCol: 54 })
  features.push(...buildPeripheralM1GatewayFeatures('west'))
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 18, maxCol: 38 })
  return Object.freeze(features)
}

function buildMap5GroundFeatures() {
  const cx = MYSTIC.col
  const cz = MYSTIC.row
  const features = [
    { kind: 'field', minRow: 5, maxRow: 49, minCol: 5, maxCol: 49 },
    { kind: 'lagoon', minRow: cz - 4, maxRow: cz + 4, minCol: cx - 4.5, maxCol: cx + 4.5 },
    { kind: 'plaza', minRow: cz - 1.8, maxRow: cz + 1.8, minCol: 1, maxCol: 54 },
  ]
  features.push({ kind: 'plaza', minRow: 1, maxRow: 4.2, minCol: 1, maxCol: 54 })
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 1, maxCol: 4.2 })
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 50.8, maxCol: 54 })
  features.push({ kind: 'plaza', minRow: 48.8, maxRow: 54, minCol: 1, maxCol: 54 })
  features.push(...buildPeripheralM1GatewayFeatures('east'))
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 16, maxCol: 36 })
  return Object.freeze(features)
}
...
function buildEmptyPeripheralGroundFeatures(gatewayEdge) {
  const features = [...buildPeripheralPerimeterPlazas(), ...buildPeripheralM1GatewayFeatures(gatewayEdge)]
  return Object.freeze(features)
}
...
function buildMap2GroundFeatures() {
  ...
  features.push({ kind: 'plaza', minRow: 1, maxRow: 54, minCol: 50.8, maxCol: 54 })
  features.push({ kind: 'plaza', minRow: 48.8, maxRow: 54, minCol: 1, maxCol: 54 })
  features.push(...buildPeripheralM1GatewayFeatures('south'))
...
function buildMap1GroundFeatures() {
  const features = [
    ...buildFullEdgeGatewayCauseways('north'),
    ...buildFullEdgeGatewayCauseways('south'),
    ...buildFullEdgeGatewayCauseways('east'),
    ...buildFullEdgeGatewayCauseways('west'),
    // Full-edge plaza corridors — same stone tone as other map gateway spines.
    { kind: 'plaza', minRow: 1, maxRow: 18, minCol: PLAY_MIN, maxCol: PLAY_MAX },
    { kind: 'plaza', minRow: 36, maxRow: 54, minCol: PLAY_MIN, maxCol: PLAY_MAX },
    { kind: 'plaza', minRow: PLAY_MIN, maxRow: PLAY_MAX, minCol: 36, maxCol: 54 },
    { kind: 'plaza', minRow: PLAY_MIN, maxRow: PLAY_MAX, minCol: 1, maxCol: 18 },
