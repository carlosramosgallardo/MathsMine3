...
import { bossHeadFlushMountY } from './boss-head-photo'
import { buildHumanoidBody } from './humanoid-body'
import { attachZelensky3DHead } from './zelensky-head-3d'
...
export function createM1ZelenskyStatueVisual(THREE, lowDetail = false) {
  ...
  // Realistic head test: the 3D depth-relief of the face (zelensky-head-3d.glb)
  // with the portrait projected onto it, sized closer to human proportions
  // than the oversized mask heads. Mount origin is still the head centre, so
  // the head-tracking animation keeps working unchanged.
  const headHeight = 0.52
  const headMount = attachZelensky3DHead(THREE, bodyPivot, lowDetail, {
