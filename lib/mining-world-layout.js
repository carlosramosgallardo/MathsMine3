...
// Ground-to-first-floor staircase and suspended east-wing deck.
for (const key of ['12,10', '11,10', '10,10']) houseStructureCells.add(key)
for (let row = 7; row <= 9; row += 1) {
  for (let col = 8; col <= 11; col += 1) houseStructureCells.add(`${row},${col}`)
}
for (const key of ['10,8', '10,9', '10,11']) houseStructureCells.add(key)

// First-floor-to-roof staircase and roof deck around the portal atrium.
for (const key of ['7,7', '6,7', '5,7']) houseStructureCells.add(key)
for (let row = 4; row <= 6; row += 1) {
  for (let col = 4; col <= 6; col += 1) {
    if (row !== 5 || col !== 5) houseStructureCells.add(`${row},${col}`)
  }
}
