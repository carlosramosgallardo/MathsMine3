...
export function spawnBossHammerSickleBurst(existing, { fromGx, fromGy, toGx, toGy, at, mapId = '3', count = 6 }) {
  ...
  for (let i = 0; i < count; i++) {
    const spread = (unitRandom() - 0.5) * 0.55
    const speed = 3.5 + unitRandom() * 2.2
    burst.push(createBossHammerSickleSymbol({
      at: at + i * 28 + unitRandom() * 24,
      x: fromGx + dirX * 0.35 + (unitRandom() - 0.5) * 0.18,
      y: 1.05 + unitRandom() * 0.55,
      z: fromGy + dirY * 0.35 + (unitRandom() - 0.5) * 0.18,
      vx: dirX * speed + perpX * spread,
      vy: 2.0 + unitRandom() * 1.5,
      vz: dirY * speed + perpY * spread,
      rot: unitRandom() * Math.PI * 2,
      spin: (unitRandom() - 0.5) * 2.2,
      mapId,
    }))
  }
