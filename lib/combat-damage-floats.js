...
export function createCombatDamageFloat(opts = {}) {
  return {
    id: `${opts.at ?? performance.now()}-${unitRandom().toString(36).slice(2, 8)}`,
    ...
    hacked: Boolean(opts.hacked),
    label: opts.label ? String(opts.label) : '',
  }
}

export function combatDamageFloatLines(entry) {
  if (entry.dodged) return ['DODGE']
  if (entry.damage <= 0 && !entry.label) return []
  const lines = []
  if (entry.damage > 0) lines.push(`-${entry.damage}`)
  if (entry.hacked) lines.push('👾 OFFLINE')
  if (entry.label) lines.push(entry.label)
  return lines.length ? lines : ['DODGE']
}

export function combatDamageFloatText(entry) {
  return combatDamageFloatLines(entry).join('\n')
}
