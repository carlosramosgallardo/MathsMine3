...
export function batchMiningStaticDecor(world, animated = new Set()) {
  ...
  world.traverse(object => {
    ...
    if (!object.isMesh || object.isSkinnedMesh || Object.keys(object.geometry?.morphAttributes || {}).length || object.isInstancedMesh || object.children.length || !object.visible
      ...
      || protectedBranch(object, world, animated)) return
    for (let p = object.parent; p && p !== world; p = p.parent) if (!p.visible) return
...
export function addM1RetroDecor(world, obstacles, cells = new Map()) {
  ...
  const add = (shape, x, y, z, sx, sy, sz, color) => {
  ...
  for (let side = 0; side < 4; side++) {
    for (let i = 0; i < 20; i++) {
      ...
      const x = side === 0 ? along : side === 1 ? 56 - outward : side === 2 ? along : outward
      const z = side === 0 ? outward : side === 1 ? along : side === 2 ? 56 - outward : along
  ...
  for (const [key, obstacle] of obstacles) {
    ...
    if (!Number.isFinite(row) || !Number.isFinite(col) || row < 2 || row > 53 || col < 2 || col > 53) continue
    if ((row < 17 && col < 17) || (row >= 17 && row <= 37 && col >= 17 && col <= 37)) continue
    if (obstacle.isHouse || obstacle.shape || obstacle.bottom || obstacle.isStatue || obstacle.isNukeCube) continue
    // Only ordinary generated obstacles: special platform data must remain untouched.
    if (Object.keys(obstacle).some(k => /statue|plinth|deck|nuke|gateway/i.test(k))) continue
    const top = Number(obstacle.visualHeight ?? obstacle.height)
    if (!Number.isFinite(top) || top < .4 || top > 4.5 || (row * 17 + col * 11) % 3 !== 0) continue
  ...
  for (let row = 3; row < 54; row += 3) {
    for (let col = 3; col < 54; col += 3) {
      if ((row < 18 && col < 18) || (row > 15 && row < 40 && col > 15 && col < 40)) continue
      if ((row > 23 && row < 33) || (col > 23 && col < 33)) continue
      let occupied = false
      for (let dr = -1; dr <= 1; dr++) for (let dc = -1; dc <= 1; dc++) {
        if (obstacles.has(`${row + dr},${col + dc}`) || cells.has(`${row + dr},${col + dc}`)) occupied = true
      }
