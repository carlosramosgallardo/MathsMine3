 ...
 */
export function createMiningDecorBatcher(world, animated = new Set(), { origin = null } = {}) {
  ...
  if (origin) {
    const d2 = b => (b.cx - origin.x) ** 2 + (b.cz - origin.z) ** 2
  ...
  const mergeBucket = ({ objects, cx, cz }) => {
...
export function cullMiningBatchesByDistance(world, gx, gz, radius) {
  ...
  const limit = (radius + BATCH_BUCKET_REACH) ** 2
  let hidden = 0
  for (const mesh of batches) {
    const { cx, cz } = mesh.userData.retroBatch
    const visible = (cx - gx) ** 2 + (cz - gz) ** 2 <= limit
