...
const BATCH_BUCKET_SIZE = 12
// A bucket's merged mesh can reach ~8.5 units past its centre (half diagonal);
// distance culling keeps that margin so a bucket whose near edge is in view
// never blinks out because its centre happens to sit past the radius.
const BATCH_BUCKET_REACH = BATCH_BUCKET_SIZE * Math.SQRT1_2 + 1
 ...
 *
 * Returns a stepper so the merge can be spread across frames: buckets are
 * ordered nearest-first from `origin` and `step(budgetMs)` merges as many as
 * fit in the budget. Until a bucket is merged its originals simply keep
 * drawing (more draw calls for a few frames, never a hole), so map entry no
 * longer pays the whole merge in one synchronous stall — and the stall would
 * grow linearly with every extra piece of decor. Shared geometry/material
 * retirement waits for finish(): a piece may be shared across buckets.
 */
export function createMiningDecorBatcher(world, animated = new Set(), { origin = null, hideBeyond = Infinity } = {}) {
  ...
  world.traverse(object => {
    ...
    const bx = Math.floor(pos.x / BATCH_BUCKET_SIZE), bz = Math.floor(pos.z / BATCH_BUCKET_SIZE)
    const key = JSON.stringify([bx, bz,
    ...
    if (!buckets.has(key)) buckets.set(key, { objects: [], cx: (bx + .5) * BATCH_BUCKET_SIZE, cz: (bz + .5) * BATCH_BUCKET_SIZE })
    buckets.get(key).objects.push(object)
  })
  const queue = [...buckets.values()].filter(b => b.objects.length >= 3)
  const d2 = b => origin ? (b.cx - origin.x) ** 2 + (b.cz - origin.z) ** 2 : 0
  if (origin) queue.sort((a, b) => d2(a) - d2(b))
  // Render distance: a bucket past it must not draw at all until the player
  // gets there, so its originals are hidden now (still raycastable) and the
  // merged batch is born hidden; the distance cull shows it on approach.
  const beyond = b => origin && d2(b) > (hideBeyond + BATCH_BUCKET_REACH) ** 2
  for (const bucket of queue) if (beyond(bucket)) for (const object of bucket.objects) object.visible = false
  ...
  const batches = world.userData.retroBatches ||= []
  let removedDraws = 0
  let finished = false

  const mergeBucket = (bucket) => {
    const { objects, cx, cz } = bucket
    ...
    // World-local centre for distance culling (world is identity in Mining,
    // but stay honest about the inverse like the vertex bake above).
    const centre = new THREE.Vector3(cx, 0, cz).applyMatrix4(inverse)
    mesh.userData.retroBatch = { cx: centre.x, cz: centre.z }
    mesh.visible = !beyond(bucket)
  ...
  const finish = () => {
    ...
    // Accumulate: batchMiningShoreline adds its own savings, possibly before
    // an incremental batcher finishes.
    world.userData.retroSavedDraws = (world.userData.retroSavedDraws || 0) + removedDraws
  }
  const batcher = {
    get done() { return finished },
    get pending() { return queue.length },
    get savedDraws() { return removedDraws },
    /** Merge buckets until `budgetMs` elapses; returns true once everything is merged. */
    step(budgetMs = 3) {
      if (finished) return true
      const deadline = (typeof performance !== 'undefined' ? performance.now() : Date.now()) + budgetMs
      do {
        mergeBucket(queue.shift())
      } while (queue.length && (typeof performance !== 'undefined' ? performance.now() : Date.now()) < deadline)
      if (!queue.length) finish()
      return finished
    },
    /** Stop merging (world being discarded); what was already retired is still released. */
    cancel() { queue.length = 0; finish() },
  }
  if (!queue.length) finish()
  return batcher
}

/** Synchronous form: merge every bucket now. Returns the draw calls saved. */
export function batchMiningStaticDecor(world, animated = new Set()) {
  const batcher = createMiningDecorBatcher(world, animated)
  while (!batcher.done) batcher.step(Infinity)
  return batcher.savedDraws
}

/**
 * Where the fog has swallowed a batch, stop drawing it. FogExp2 leaves
 * exp(-(density·d)²) of a surface visible; at density·d = 1.6 that is ~8%,
 * below what the 768×480 retro framebuffer resolves, so the cut is invisible.
 * The radius therefore follows the fog: a denser fog is the one knob that
 * both shortens the view and buys headroom for denser decoration.
 */
export function miningViewRadius(fogDensity) {
  return fogDensity > 0 ? 1.6 / fogDensity : Infinity
}

/** Hide merged decor batches beyond the fog-derived radius from (gx, gz). */
export function cullMiningBatchesByDistance(world, gx, gz, radius) {
  const batches = world?.userData?.retroBatches
  if (!batches?.length) return 0
  let hidden = 0
  for (const mesh of batches) {
    const { cx, cz, reach = BATCH_BUCKET_REACH } = mesh.userData.retroBatch
    const limit = radius + reach
    const visible = (cx - gx) ** 2 + (cz - gz) ** 2 <= limit * limit
...
/**
 * One InstancedMesh per `chunk`-cell square instead of one per map, so the
 * same distance/frustum culling that governs decor also governs the walls,
 * biome blocks and houses — the geometry that made a map appear all at once.
 * `cellOf(entry)` returns [row, col]; `place(entry, index, matrix)` fills the
 * instance matrix. Collision raycasts still visit every chunk mesh (three
 * does not skip invisible objects), so a hidden far chunk keeps colliding.
 */
export function addChunkedInstancedMeshes(world, { geometry, material, entries, cellOf, place, chunk = 14, collidable = false, name = 'chunked instances' }) {
  const groups = new Map()
  for (const entry of entries) {
    const [row, col] = cellOf(entry)
    const bx = Math.floor(col / chunk), bz = Math.floor(row / chunk)
    const key = `${bx},${bz}`
    if (!groups.has(key)) groups.set(key, { bx, bz, entries: [] })
    groups.get(key).entries.push(entry)
  }
  const batches = world.userData.retroBatches ||= []
  const matrix = new THREE.Matrix4()
  const meshes = []
  for (const { bx, bz, entries: chunkEntries } of groups.values()) {
    const mesh = new THREE.InstancedMesh(geometry, material, chunkEntries.length)
    mesh.name = name
    chunkEntries.forEach((entry, index) => { place(entry, index, matrix); mesh.setMatrixAt(index, matrix) })
    mesh.instanceMatrix.needsUpdate = true
    mesh.computeBoundingSphere()
    if (collidable) mesh.userData.collidable = true
    mesh.userData.retroBatch = { cx: (bx + .5) * chunk, cz: (bz + .5) * chunk, reach: chunk * Math.SQRT1_2 + 1 }
    batches.push(mesh)
    world.add(mesh)
    meshes.push(mesh)
  }
...
export function addM1RetroDecor(world, obstacles, cells = new Map()) {
  ...
  const add = (shape, ...dimensions) => {
    ...
    const bx = Math.floor(x / 14), bz = Math.floor(z / 14)
    const key = `${shape}:${bx}:${bz}`
    if (!groups.has(key)) groups.set(key, { shape, bx, bz, entries: [] })
...
function buildRetroDecorInstances(world, groups, count) {
  ...
  const batches = world.userData.retroBatches ||= []
  for (const { shape, bx, bz, entries } of groups.values()) {
    const mesh = new THREE.InstancedMesh(geometries[shape], material, entries.length)
    // Same 14-cell chunking as the walls: distance culling treats it as a batch.
    mesh.userData.retroBatch = { cx: (bx + .5) * 14, cz: (bz + .5) * 14, reach: 14 * Math.SQRT1_2 + 1 }
    batches.push(mesh)
