...
import { humanoidGlbFit, loadHumanoidGlbPrototype, makeStatueDisplayMaterial, makeVividAlbedoMaterial, bindPlazaStatueSkin, enableCharacterCulling, HUMANOID_GLB_YAW, HUMANOID_GLB_CROWN_Y } from './humanoid-glb'
...
function attachVertexColoredGlb(THREE, parent, {
  ...
  onReady = null,
  // Same default every other body uses (attachTexturedStatueGlb, humanoid-glb.js)
  // — override only for a source sculpt baked facing a different way.
  ...
  loadVisibleModel(parent, () => loadHumanoidGlbPrototype(THREE, url, { retro })).then((proto) => {
    ...
    if (retro) enableCharacterCulling(clone)
    fit.add(clone)
    if (tagQuadruped) {
 ...
 */
export function attachQuadrupedGlb(THREE, parent, { url, tint = null, onReady = null, fitFn = null, yaw = HUMANOID_GLB_YAW, retro = false } = {}) {
...
export function attachTexturedStatueGlb(THREE, parent, {
  ...
  loadVisibleModel(parent, () => loadHumanoidGlbPrototype(THREE, url, { retro })).then((proto) => {
    ...
    // Rigid props (skin already baked) clone the scene. RPM plaza avatars keep
    // a skeleton — SkeletonUtils.clone so each instance has its own bind.
    let clone
    let skinned = false
    proto.gltf.scene.traverse((obj) => {
      if (obj.isSkinnedMesh) skinned = true
    })
    try {
      clone = skinned
        ? proto.SkeletonUtils.clone(proto.gltf.scene)
    ...
    if (skinned) bindPlazaStatueSkin(parent, clone)
    onReady?.(clone)
  }).catch((err) => {
