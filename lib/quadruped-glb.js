  ...
  loadHumanoidGlbPrototype(THREE, url).then((proto) => {
    ...
    clone.traverse((obj) => {
      ...
      for (const mat of mats) {
        ...
        if ('roughness' in mat) mat.roughness = Math.max(Number(mat.roughness) || 0.82, 0.82)
        ...
        // Keep albedo tint white so the baseColor map reads true; untextured
        // scraps (eyelashes) get a dark fallback so they don't flash white.
        if (mat.map) mat.color?.setHex?.(0xffffff)
        else mat.color?.setHex?.(0x2a2a2a)
