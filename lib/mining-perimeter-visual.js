...
import { isM1GatewayApproachCell } from './mining-gateway-corridors'
import {
...
function getGatewayPerimeterSide(row, col, mapId) {
  if (mapId === '1') {
    ...
    if (isM1GatewayApproachCell(row, col)) {
      if (row <= MIN + 11) return 'north'
      if (row >= MAX - 11) return 'south'
      if (col <= MIN + 11) return 'west'
      if (col >= MAX - 11) return 'east'
    }
    return null
  }
  if (mapId === '2' && row === MAX && inGatewayColBand(col)) return 'south'
  return null
}
