export const MARKET_COMMANDS = [
  ...
  },
  {
    ...
    payment: 'mm3',
    effect: 'mm3',
    command: '/mm3 [freakingAI@MM3] siphon => 68000+(x*53)+(9999/9) = ?',
    solve: (x) => 68000 + (x * 53) + (9999 / 9),
  },
  {
    key: 'mm3-2da',
    emoji: '🔋',
    payment: 'mm3',
    effect: 'mm3',
    command: '/mm3 [freakingAI@MM3] siphon => 73000+(x*59)+(16384/16) = ?',
    solve: (x) => 73000 + (x * 59) + (16384 / 16),
  },
  {
    key: 'mm3-2f9',
    emoji: '🎛️',
    payment: 'mm3',
    effect: 'mm3',
    command: '/mm3 [freakingAI@MM3] siphon => 79000+(x*67)+(22222%999) = ?',
    solve: (x) => 79000 + (x * 67) + (22222 % 999),
  },
];
...
export function marketCommandFromBlock(block) {
  ...
  const isMm3 = command.toLowerCase().startsWith('/mm3');
  return {
    key: block.block_key,
    ...
    payment: isMm3 ? 'mm3' : 'money',
    effect: isMm3 ? 'mm3' : 'money',
    command,
