...
test('standing biped: outer arms and legs classify off the torso', () => {
  assert.equal(classifyStandingBipedVertex(-0.25, 0.55, PUTIN_BIPED_PROFILE), 'la')
  assert.equal(classifyStandingBipedVertex(0.25, 0.55, PUTIN_BIPED_PROFILE), 'ra')
  assert.equal(classifyStandingBipedVertex(-0.12, 0.2, PUTIN_BIPED_PROFILE), 'll')
  assert.equal(classifyStandingBipedVertex(0.12, 0.2, PUTIN_BIPED_PROFILE), 'rl')
})

test('chest and head stay on the rigid torso', () => {
  assert.equal(classifyStandingBipedVertex(0, 0.9, PUTIN_BIPED_PROFILE), 'torso')
  assert.equal(classifyStandingBipedVertex(0.05, 0.6, PUTIN_BIPED_PROFILE), 'torso')
})
