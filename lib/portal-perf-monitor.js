...
function getTargetFrameMs() {
  if (typeof window === 'undefined') return TARGET_FRAME_MS
  const w = window.innerWidth
  const h = window.innerHeight
  const portraitMobile = h > w && w < 820
  if (portraitMobile || w < 980) return MOBILE_TARGET_FRAME_MS
...
export function createPortalPerfSampler({ onUpdate, intervalMs = 600 } = {}) {
  ...
  function emitSample() {
    ...
    const targetFrameMs = getTargetFrameMs()
    const cpuLoad = avgFrameMs > 0
      ? Math.min(150, Math.round((avgFrameMs / targetFrameMs) * 100))
      : 0
