...
test('crawling never sinks below the ground and stays inside the gait envelope', () => {
  ...
  const rocked = poses.some((pose) => Math.abs(pose.pitch) > QUADRUPED_GAIT.pitch * 0.5)
  const rolled = poses.some((pose) => Math.abs(pose.roll) > QUADRUPED_GAIT.roll * 0.5)
  assert.ok(rocked && rolled, 'the crawl reads through both the fore/aft rock and the weight roll')
})
