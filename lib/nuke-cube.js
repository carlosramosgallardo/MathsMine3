...
async function loadNukePrototype(THREE) {
  ...
  protoPromise = import('three/addons/loaders/GLTFLoader.js')
    ...
    .then((gltf) => {
      ...
      root.traverse(obj => { if (obj.geometry) obj.geometry.userData.skipDispose = true })
...
export function createNukeCubeVisual(THREE, lowDetail = false, { retro = false } = {}) {
  ...
  if (retro) addRetroNukePanels(THREE, group)
  else loadVisibleModel(group, () => loadNukePrototype(THREE)).then((proto) => {
