...
async function loadNukePrototype(THREE) {
  ...
  protoPromise = import('three/addons/loaders/GLTFLoader.js')
    .then(({ GLTFLoader }) => new GLTFLoader().loadAsync(runtimeModelUrl(NUKE_CUBE_MODEL_URL)))
    .then((gltf) => {
...
export function createNukeCubeVisual(THREE, lowDetail = false, { retro = false } = {}) {
  ...
  // Retro: caution panels dress the fallback cube at once; the modelled
  // chamber replaces both when it streams in (or neither, if it never does).
  const panels = retro ? addRetroNukePanels(THREE, group) : []
  loadVisibleModel(group, () => loadNukePrototype(THREE)).then((proto) => {
