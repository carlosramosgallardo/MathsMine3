...
export function createWalletNonce(wallet) {
  ...
  const iat = Date.now();
  const sig = hmac(['nonce', wallet, nonce, String(iat)].join('.'));
  return { token: encode(['nonce', wallet, nonce, String(iat), sig]), message: nonceMessage(wallet, nonce, iat) };
}
...
export function createSessionToken(wallet) {
  ...
  const sig = hmac(['session', wallet, String(exp)].join('.'));
  return encode(['session', wallet, String(exp), sig]);
}
