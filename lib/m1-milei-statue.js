...
import { applyRigidBipedGait, mountRigidBipedLimbs } from './rigid-biped-limbs'
...
export function createM1MileiStatueVisual(THREE, lowDetail = false) {
  ...
  attachQuadrupedGlb(THREE, bodyPivot, {
    ...
    onReady: (clone) => {
      if (clone) mountRigidBipedLimbs(THREE, bodyPivot, bodyPivot.userData.quadrupedGlbFit, clone)
