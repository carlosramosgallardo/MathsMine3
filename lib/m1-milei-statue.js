...
export function createM1MileiStatueVisual(THREE, lowDetail = false) {
  ...
  const facePlane = new THREE.Mesh(
    new THREE.PlaneGeometry(0.56, 0.58),
  ...
  facePlane.name = 'm1MileiHeadPhoto'
  facePlane.position.set(0, 1.02, -0.132)
  facePlane.rotation.y = Math.PI
  facePlane.renderOrder = 8
  facePlane.frustumCulled = false
  bodyPivot.add(facePlane)

  const textureLoader = new THREE.TextureLoader()
  textureLoader.load(
    M1_MILEI_HEAD_TEXTURE_URL,
    (texture) => {
  ...
  // Side hair volume so the head does not look paper-thin from oblique angles
  addBox(bodyPivot, 0.08, 0.36, 0.20, hairDark, -0.18, 1.00, 0.04)
  addBox(bodyPivot, 0.08, 0.36, 0.20, hairDark, 0.18, 1.00, 0.04)
  addBox(bodyPivot, 0.34, 0.10, 0.24, hairDark, 0, 1.28, 0.06)
