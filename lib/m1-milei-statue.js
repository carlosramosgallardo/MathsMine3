...
import { attachBossMaskHead, bossHeadFlushMountY, BOSS_TORSO_TOP_Y } from './boss-head-photo'
...
export function createM1MileiStatueVisual(THREE, lowDetail = false) {
  ...
  // Animation target: a pivot at the base of the skull, so head motion reads as
  // a neck articulation instead of the whole head swivelling around its center.
  const neckPivot = new THREE.Group()
  neckPivot.name = 'm1MileiNeckPivot'
  neckPivot.position.set(0, bossHeadFlushMountY(headHeight) - 0.28, -0.002)
  bodyPivot.add(neckPivot)
  group.userData.homeHead = neckPivot
  // Same mask design as the other bosses: alpha-cutout face on a curved shell,
  // suit-coloured skull mold, white strap around the nape.
  attachBossMaskHead(THREE, neckPivot, M1_MILEI_MASK_TEXTURE_URL, lowDetail, {
