 ...
 */
export function createM1MileiStatueVisual(THREE, lowDetail = false, { retro = false } = {}) {
  ...
  const finishModel = (ok) => {
    ...
    // Same deferred swap as fixed-textured-statue: the voxel stand-in stays
    // until the pixel-art sculpt is mounted, then steps aside.
    if (ok && retro) configureCapsuleVisibility(bodyPivot, parts, false)
  ...
  attachQuadrupedGlb(THREE, bodyPivot, {
    ...
    // The source figure faces +Z. Keep its mount neutral; group.rotation.y is
    // the single source of truth for both plaza facing and patrol direction.
