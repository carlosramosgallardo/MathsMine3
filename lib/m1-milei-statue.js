...
export function createM1MileiStatueVisual(THREE, lowDetail = false) {
  ...
  const shirt = mat('#f8fafc', 0.72, 0.04)
  const tie = mat('#111827', 0.55, 0.12)
  const skin = mat('#f0d0b8', 0.58, 0.06)
  const skinLight = mat('#f8e4d2', 0.52, 0.04)
  const hair = mat('#3d2914', 0.62, 0.08)
  const hairDark = mat('#26180d', 0.68, 0.06)
  const shoe = mat('#111827', 0.62, 0.18)
  const sashBlue = basic('#74acdf')
  const sashWhite = basic('#f8fafc')
  const sun = basic('#fbbf24')
  const eye = basic('#38bdf8')
  ...
  // Head
  addBox(bodyPivot, 0.34, 0.30, 0.28, skin, 0, 1.00, 0)
  addBox(bodyPivot, 0.36, 0.10, 0.30, hair, 0, 1.18, 0.02)
  addBox(bodyPivot, 0.14, 0.18, 0.26, hairDark, -0.20, 1.08, 0.01)
  addBox(bodyPivot, 0.14, 0.18, 0.26, hairDark, 0.20, 1.08, 0.01)
  addBox(bodyPivot, 0.10, 0.08, 0.24, hairDark, 0, 1.22, 0.04)
  addBox(bodyPivot, 0.04, 0.02, 0.01, eye, -0.08, 1.02, -0.142)
  addBox(bodyPivot, 0.04, 0.02, 0.01, eye, 0.08, 1.02, -0.142)
  addBox(bodyPivot, 0.08, 0.02, 0.01, mat('#c48878', 0.6, 0), 0, 0.92, -0.143)
