import { runtimeModelUrl } from './runtime-model-url.js'
...
export function prepareSharedGltfMap(THREE, map) {
  ...
  // Mining was briefly drawn with nearest sampling and flat shading to force a
  // pixel-art look; on scanned faces that read as shimmer and facets, not
  // style. Its 768×480 framebuffer already pixelates the whole scene.
  map.anisotropy = 2
...
export function loadHumanoidGlbPrototype(THREE, url = HUMANOID_GLB_URL, { priority = false } = {}) {
  if (typeof window === 'undefined') return Promise.resolve(null)
  url = runtimeModelUrl(url)
...
export function attachHumanoidGlb(THREE, parent, {
  ...
  /** Mining: keep the voxel stand-in on screen until the model lands, then frustum-cull it. */
  retro = false,
  onReady = null,
} = {}) {
  ...
  loadVisibleModel(parent, () => loadHumanoidGlbPrototype(THREE, url)).then((proto) => {
    if (!proto || !fit.parent) {
