import { runtimeModelUrl, retroModelUrl } from './runtime-model-url.js'
import { loadVisibleModel } from './model-load-scheduling.js'
/**
...
/**
 * Cockpit peek: parent-space Y of the neck while seated. The Fennec roof sits
 * near y≈0.78 from the front; 0.52–0.68 only cleared the forehead. 1.00 puts
 * the shoulders over the roofline without moving the car on X/Z.
 */
export const HUMANOID_GLB_CAR_NECK_Y = 1
...
/**
 * Frustum-cull a character's meshes (Mining). Characters were built with
 * frustumCulled=false across the board, so every boss, statue and bot was
 * drawn even when behind the camera. Rigid meshes cull safely as they are.
 * Skinned ones need care: three computes their sphere once, from whatever
 * pose the first rendered frame had, and a later walk or wave can carry a
 * limb past it and pop the mesh out at the screen edge — so they get the
 * bind-pose sphere padded by `pad`, which no animation here escapes.
 */
export function enableCharacterCulling(root, { pad = 1.5 } = {}) {
  root.traverse((obj) => {
    if (!obj.isMesh) return
    if (obj.isSkinnedMesh) {
      const geometry = obj.geometry
      if (!geometry.boundingSphere) geometry.computeBoundingSphere()
      obj.boundingSphere = geometry.boundingSphere.clone()
      obj.boundingSphere.radius *= pad
    }
...
/**
 * Capsule parts the scan stands in for: every body mesh except the group that
 * holds the GLB itself and the procedural hands, which keep their connectors.
 */
export function capsulePartsHiddenByGlb(bodyMeshes, bodyHold, hands) {
  return (bodyMeshes || []).filter((mesh) => (
    mesh && mesh !== bodyHold && mesh.isMesh && !keepProceduralHandWithGlb(mesh, hands)
  ))
}
...
export function prepareSharedGltfMap(THREE, map) {
  if (!map) return null
  map.userData ||= {}
  if (map.userData.mm3Prepared) return map
  map.userData.mm3Prepared = true
  map.colorSpace = THREE.SRGBColorSpace
  // Both tiers filter the same way. Mining's retro tier was briefly drawn with
  // nearest sampling and flat shading to force a pixel-art look; on scanned
  // faces that read as shimmer and facets, not style. The retro framebuffer
  // (768×480) already pixelates the whole scene uniformly — the tier only
  // needs to be lighter, not to look different from Home's.
  map.anisotropy = 2
...
/** Capsule hip pitch copied onto the RPM bind, not replacing it. */
function posePlazaWalk(host) {
  const bones = host?.userData?.humanoidGlbBones
  ...
  const hip = (capsule) => {
    if (!capsule) return 0
    return glbLimbFromCapsule(capsule.rotation, capsule.userData.baseRotZ || 0).x
  }
  const lx = hip(lLeg)
  const rx = hip(rLeg)
  addLocalEuler(bones.LeftUpperLeg, lx, 0, 0)
  addLocalEuler(bones.RightUpperLeg, rx, 0, 0)
  addLocalEuler(bones.LeftLowerLeg, Math.abs(lx) * 0.5, 0, 0)
  addLocalEuler(bones.RightLowerLeg, Math.abs(rx) * 0.5, 0, 0)
  const ax = hip(lArm)
  const ar = hip(rArm)
  addLocalEuler(bones.LeftUpperArm, ax * 0.55, 0, 0)
  addLocalEuler(bones.RightUpperArm, ar * 0.55, 0, 0)
  return true
}
...
function copyCapsulePoseToGlbBones(host) {
  ...
  // Patrol walk sets strideFloorY; skip the idle cheer/wave so hips stride.
  if (host.userData.plazaArmIdle && Number.isFinite(host.userData.strideFloorY)) {
    posePlazaWalk(host)
    return
  }
...
// Home's carousel mounts every boss/statue at once (HomeMiningWorld3D.jsx —
// "the framing always shows the maximum number of members at once"), which
// used to mean up to 6 multi-MB character GLBs (kim/putin/trump/milei/
// zelensky/macron, 1.6-7.5MB each) all hitting the network and the GPU
// upload queue in the same tick. Everything still loads — this just caps
// how many fetch+parse+upload at once, so the first few (whichever the
// scene created first) actually finish sooner instead of all of them
// crawling forward together; the rest queue and start as slots free up.
const MAX_CONCURRENT_GLB_LOADS = 3
let activeGlbLoads = 0
const queuedGlbLoads = []

function runNextQueuedGlbLoad() {
  if (activeGlbLoads >= MAX_CONCURRENT_GLB_LOADS) return
  const next = queuedGlbLoads.shift()
  if (!next) return
  activeGlbLoads += 1
  next()
}

export function loadHumanoidGlbPrototype(THREE, url = HUMANOID_GLB_URL, { priority = false, retro = false } = {}) {
  if (typeof window === 'undefined') return Promise.resolve(null)
  // Tiers resolve to distinct URLs, so Home's runtime proto and Mining's retro
  // proto for the same character never share a cache slot or its textures.
  url = retro ? retroModelUrl(url) : runtimeModelUrl(url)
  const cached = protoPromises.get(url)
  if (cached) return cached
  const promise = new Promise((resolveQueued) => {
    const startLoad = () => {
      Promise.all([
        ...
        new GLTFLoader().loadAsync(url).then((gltf) => {
          ...
          gltf.scene.traverse((obj) => {
            ...
            if (!obj.isMesh) return
            obj.geometry.userData.skipDispose = true
        ...
        activeGlbLoads -= 1
        runNextQueuedGlbLoad()
        resolveQueued(result)
      })
    }
    // Tiny structural props such as the statue pedestal must not sit behind
    // every multi-megabyte character: Mining needs the fixed base immediately
    // even while the walking figure models continue streaming.
    if (priority) queuedGlbLoads.unshift(startLoad)
    else queuedGlbLoads.push(startLoad)
    runNextQueuedGlbLoad()
  })
...
export function attachHumanoidGlb(THREE, parent, {
  ...
  /** Mining: load the pixel-art tier and keep the voxel stand-in on screen until it lands. */
  retro = false,
  onReady = null,
} = {}) {
  ...
  // Hide the capsule body up front rather than when the GLB lands. It is a
  // different-looking character, so showing it first read as the avatar
  // changing identity mid-load, and it was not free: its meshes and materials
  // were drawn — and their shaders compiled — for the seconds before the scan
  // replaced them. The rig itself stays: it is what drives the GLB's bones.
  // Procedural hands are exempt here for the same reason they are afterwards.
  //
  // Retro is the one case that keeps the capsule on screen instead: there the
  // stand-in is a deliberate voxel figure, not a slab, and the tier it waits
  // for is small enough that a brief voxel→low-poly swap beats an empty plaza
  // at map entry. The success path below hides it once the GLB is attached.
  const suppressedParts = capsulePartsHiddenByGlb(bodyMeshes, bodyHold, hands)
  if (!retro) {
    for (const mesh of suppressedParts) {
      mesh.userData.glbSuppressed = true
      mesh.visible = false
    }
    // Heads built before this call; the ones built after read useGlbHead above.
    // A GLB that brings no head (photo/mask characters) keeps the procedural one.
    if (!hideHead) {
      for (const mesh of parent.userData.proceduralHeadMeshes || []) mesh.visible = false
    }
  }
  // A GLB that never arrives (offline, decode failure) must not leave an
  // invisible avatar behind, so every failure path puts the capsule back.
  const restoreCapsuleBody = () => {
    for (const mesh of suppressedParts) {
      mesh.userData.glbSuppressed = false
      mesh.visible = true
    }
    // Clearing the flag covers heads built after this point too, so a failed
    // scan can never leave a headless avatar behind.
    parent.userData.useGlbHead = false
    for (const mesh of parent.userData.proceduralHeadMeshes || []) mesh.visible = true
  }

  loadVisibleModel(parent, () => loadHumanoidGlbPrototype(THREE, url, { retro })).then((proto) => {
    if (!proto || !fit.parent) {
      if (!proto) restoreCapsuleBody()
      onReady?.(null)
    ...
    bodyHold.add(clone)
    if (retro) {
      enableCharacterCulling(clone)
      // The deferred half of the retro swap: the voxel head stayed visible
      // while this streamed (body parts are re-suppressed a few lines down).
      if (!hideHead) {
        for (const mesh of parent.userData.proceduralHeadMeshes || []) mesh.visible = false
      }
    }
    ...
    // Re-run for anything the rig added after the pre-hide pass above.
    for (const mesh of capsulePartsHiddenByGlb(bodyMeshes, bodyHold, hands)) {
      mesh.userData.glbSuppressed = true
      mesh.visible = false
    }
    hookGlbPoseSync(parent)
...
export function seatHumanoidGlbInCar(parent, { neckY = 0.48, neckZ = 0 } = {}) {
  ...
  // Until the scan's upper body exists, the stand-in torso and head ARE the
  // rider. Hiding them regardless (as this did for #282's GLB-less bots, and
  // for retro bots still waiting on their tier) left an empty cockpit with a
  // tool floating in it. Arms and legs still go — they poke through the car.
  // The GLB success path re-seats once the split meshes exist and this then
  // retires the stand-in for real.
  const glbUpperReady = Boolean(headMesh || parent.userData.humanoidGlbHead)
...
export function applyHumanoidCarMount(parent, { neckY = 0.48, neckZ = 0 } = {}) {
  ...
  if (parent.userData.tool) parent.userData.tool.visible = true
  // Same rule as seatHumanoidGlbInCar: the stand-in head stays until the
  // scan's head is there to replace it.
  if (upper) for (const mesh of parent.userData.proceduralHeadMeshes || []) mesh.visible = false
}
...
export function unseatHumanoidGlb(parent) {
  ...
  if (parent.userData.tool) parent.userData.tool.visible = true
  for (const hand of [parent.userData.humanoidGlbLeftHand, parent.userData.humanoidGlbRightHand]) {
    if (hand && !hand.userData?.glbSuppressed) hand.visible = true
  }
  if (fit) {
