...
/** Duplicate a GLTFLoader map so scene teardown never poisons the proto cache. */
export function cloneOwnedGltfMap(THREE, map) {
  ...
  tex.userData.ownedMap = true
  return tex
}

/**
 * Unlit albedo for home carousel statues — always shows baked JPEGs regardless
 * of ACES exposure or metal/rough maps on the source GLTF material.
 */
export function makeStatueDisplayMaterial(THREE, protoMat, { side = THREE.DoubleSide } = {}) {
  const map = cloneOwnedGltfMap(THREE, protoMat?.map)
  if (map) {
    const mat = new THREE.MeshBasicMaterial({ map, color: 0xffffff, side })
    mat.userData.ownedMap = true
    return mat
  }
  const mat = new THREE.MeshBasicMaterial({
    color: protoMat?.color?.getHex?.() ?? 0x8a8a8a,
    side,
  })
  return mat
}

/** Runtime albedo for a GLTFLoader material — shares cached `.map` textures. */
export function makeTexturedMaterial(THREE, protoMat, { side = THREE.FrontSide, ownedMap = false } = {}) {
  const map = ownedMap
    ? cloneOwnedGltfMap(THREE, protoMat?.map)
    : (protoMat?.map || null)
  ...
  if (map && ownedMap) mat.userData.ownedMap = true
  else if (protoMat?.map) mat.userData.sharedGltfMap = true
  return mat
}
...
export function attachHumanoidGlb(THREE, parent, {
  ...
  loadHumanoidGlbPrototype(THREE, url).then((proto) => {
    ...
    const mat = (preserveMap && proto.material?.map)
      ? makeTexturedMaterial(THREE, proto.material, { ownedMap: true })
