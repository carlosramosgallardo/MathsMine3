...
export function seatHumanoidGlbInCar(parent, { neckY = 0.48, neckZ = 0 } = {}) {
  ...
  // Full body mesh includes legs that poke under the car — keep the skull and
  // hide the below-neck mesh. Shoulders read from the head/neck join.
  ...
  // Move eye sprites onto the seated skull so combat red sits in the sockets.
  const headLocalY = (HUMANOID_GLB_SRC_HEAD.minY + HUMANOID_GLB_SRC_HEAD.maxY) * 0.5 * scale + fit.position.y
  const headLocalZ = fit.position.z - 0.12 * scale
  parent.traverse((obj) => {
    if (!obj.userData?.bossEyeGlow) return
    const side = Math.sign(obj.position.x) || 1
    obj.position.set(side * 0.045, headLocalY + 0.02, headLocalZ)
