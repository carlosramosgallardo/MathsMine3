...
export function attachHumanoidGlb(THREE, parent, {
  ...
  loadHumanoidGlbPrototype(THREE, url).then((proto) => {
    ...
    if (parent.userData.humanoidGlbSeated) {
      ...
      // Plant combat/idle eye sprites on the man.glb sockets (not the procedural
      // head coords, which leave empty sockets + stray glows on the torso).
      import('./prop-eye-glows').then(({ placeEyeGlowsFromFit }) => {
        if (!parent.userData.humanoidGlbFit || parent.userData.humanoidGlbSeated) return
        placeEyeGlowsFromFit(THREE, parent, {
          eyeLine: 0.52,
          size: 0.042,
          faceSign: -1,
          forwardFrac: 0.04,
