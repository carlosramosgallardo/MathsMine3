...
function makeTexturedMaterial(THREE, protoMat) {
  const mat = protoMat.clone()
  mat.vertexColors = false
  mat.metalnessMap = null
  mat.roughnessMap = null
  mat.metalness = 0
  // Matte figurine read (matches Milei sculpt under ACES) — not chrome cloth.
  mat.roughness = 0.88
  ...
  loadHumanoidGlbPrototype(THREE, url).then((proto) => {
    ...
    if (skinned) {
      ...
      if (!hideHead) {
        ...
        // Seated car riders keep the procedural head (readable above the tub).
        if (procedural && !parent.userData.humanoidGlbSeated) {
...
/** Hide legs/arms/tools and drop skull + upper torso into a cockpit. */
export function seatHumanoidGlbInCar(parent, { neckY = 0.48, neckZ = 0 } = {}) {
  ...
  // Full body mesh includes legs that poke under the car — hide it. Prefer the
  // procedural human head (wallet skin/hair) over the dark scanned skull so the
  // rider reads clearly above the cockpit tub.
  ...
  // Procedural capsule torso/limbs (if still around) stay hidden in the cabin.
  ...
  // Eye sprites on the procedural skull (centre ~0.83 local before fit offset).
  const headLocalY = 0.83 + (fit.position.y - offsetY) * 0.15
  const headLocalZ = (neckZ || 0) - 0.06
  parent.traverse((obj) => {
    if (!obj.userData?.bossEyeGlow) return
    const side = Math.sign(obj.position.x) || 1
    obj.position.set(side * 0.048, Math.max(0.78, headLocalY), headLocalZ)
