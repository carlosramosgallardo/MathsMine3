/** Shared A-pose game mesh used by bots, bosses and statues. */
...
/** Source AABB Y from man.glb (Y-up, feet at 0, crown at max). */
export const HUMANOID_GLB_SRC_YMIN = 0
export const HUMANOID_GLB_SRC_YMAX = 1.89499998

/** Local Y where the skull starts — used to split body vs head. */
export const HUMANOID_GLB_SRC_NECK_Y = 1.55
...
/** A-pose torso half-width (not the outstretched hands). Hit box stays ≤ 0.38. */
export const HUMANOID_GLB_SRC_TORSO_HALF_X = 0.35
...
uniform float uSleeve;`,
      ...
      .replace(
        ...
        `#include <map_fragment>
vec3 region = uTorso;
if (vBodySrc.y < 0.08) region = uShoes;
else if (vBodySrc.y < 0.86) region = uLegs;
else if (vBodySrc.y >= 1.55) region = uSkin;
float arm = step(0.40, abs(vBodySrc.x)) * step(0.85, vBodySrc.y) * step(vBodySrc.y, 1.52);
if (arm > 0.5) {
  if (uSleeve > 1.5) region = uSkin;
  else if (uSleeve > 0.5 && vBodySrc.y < 1.18) region = uSkin;
  else region = uTorso;
}
...
/**
 * Overlay the A-pose GLB on the capsule rig. Named clips (idle, walk, jumps,
 * point, wave, salute) play from the live capsule pose so RJ45/USB stay in
 * the palms. `hideHead` (bosses/statues) drops the mesh skull so the portrait
 * stays; bots keep it for the RL-car.
 */
export function attachHumanoidGlb(THREE, parent, {
  ...
  loadHumanoidGlbPrototype(THREE).then((proto) => {
    ...
    const actions = {}
    for (const clip of proto.gltf.animations || []) {
...
/** Hide the torso/limbs and drop the skull into a cockpit. */
