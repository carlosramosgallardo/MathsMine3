...
export function prepareSharedGltfMap(THREE, map, { pixelArt = false } = {}) {
  if (!map) return null
  map.userData ||= {}
  ...
  if (pixelArt) {
    // The retro tier ships 256² textures meant to be seen as texels: nearest
    // sampling with no mipmaps keeps every pixel hard-edged instead of
    // smearing the reduced map back into a blur.
    map.anisotropy = 1
...
/**
 * Faceted shading for the retro tier: the decimated meshes are meant to read
 * as low-poly, so let each triangle show instead of smoothing normals across
 * them. Works on whatever material the attach helpers built (Standard,
 * Lambert, statue display) without replacing their tint/albedo setup.
 */
export function applyRetroShading(root) {
  root.traverse((obj) => {
    if (!obj.isMesh) return
    for (const mat of Array.isArray(obj.material) ? obj.material : [obj.material]) {
      if (!mat || mat.flatShading) continue
      ...
      Promise.all([
        ...
        new GLTFLoader().loadAsync(url).then((gltf) => {
          ...
          gltf.scene.traverse((obj) => {
            ...
            for (const m of mats) {
              if (m?.map) prepareSharedGltfMap(THREE, m.map, { pixelArt: retro })
            }
          })
