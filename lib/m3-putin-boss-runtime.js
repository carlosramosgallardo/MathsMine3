...
import { attachBossHeadPhoto, bossHeadFlushMountY, BOSS_TORSO_TOP_Y } from './boss-head-photo'
...
export function createM3PutinBossVisual(THREE, lowDetail = false) {
  ...
  // Shirtless torso & abs — front detail sits behind the portrait cube
  addBox(bodyPivot, 0.50, 0.44, 0.28, skinMat, 0, 0.50, 0.02)
  addBox(bodyPivot, 0.08, 0.06, 0.02, skinBright, -0.08, 0.46, 0.04)
  addBox(bodyPivot, 0.08, 0.06, 0.02, skinBright, 0.08, 0.46, 0.04)
  addBox(bodyPivot, 0.08, 0.06, 0.02, skinBright, 0, 0.40, 0.04)
  addBox(bodyPivot, 0.06, 0.05, 0.02, skinBright, -0.06, 0.54, 0.04)
  addBox(bodyPivot, 0.06, 0.05, 0.02, skinBright, 0.06, 0.54, 0.04)
  if (!lowDetail) {
    // Visible front sculpting for the home hero and FPV close-ups.
    addBox(bodyPivot, 0.22, 0.10, 0.05, skinMat, 0, 0.665, -0.05)
    addBox(bodyPivot, 0.24, 0.045, 0.03, skinBright, 0, 0.63, -0.135)
    addBox(bodyPivot, 0.11, 0.05, 0.018, skinBright, -0.08, 0.57, -0.132)
    addBox(bodyPivot, 0.11, 0.05, 0.018, skinBright, 0.08, 0.57, -0.132)
    addBox(bodyPivot, 0.08, 0.16, 0.018, skinShade, -0.07, 0.45, -0.134)
    addBox(bodyPivot, 0.08, 0.16, 0.018, skinShade, 0.07, 0.45, -0.134)
    addBox(bodyPivot, 0.04, 0.25, 0.018, skinShade, 0, 0.45, -0.136)
    addBox(bodyPivot, 0.50, 0.055, 0.026, camoDark, 0, 0.275, -0.145)
  }
  ...
  addBox(bodyPivot, 0.11, 0.11, 0.12, skinBright, 0.34, 0.28, -0.02)
  if (!lowDetail) {
    addBox(bodyPivot, 0.07, 0.08, 0.026, camoSand, -0.15, 0.30, -0.086)
    addBox(bodyPivot, 0.07, 0.08, 0.026, camoDark, 0.14, 0.31, -0.086)
  }
  ...
  attachBossHeadPhoto(THREE, bodyPivot, M3_PUTIN_HEAD_TEXTURE_URL, lowDetail, {
    ...
    bottomColor: '#e8c4a8',
    backColor: '#e8c4a8',
    uvLayout: { frontV0: 0.02, frontV: 0.9, fringeV: 0.1, sideU: 0.13, frontU: 0.66 },
  })
...
export function updateM3PutinBoss({
  ...
  if (!fighting) {
    ...
    runtime.targetWallet = null
    runtime.facing = M3_PUTIN_BOSS_IDLE_FACING + Math.sin(runtime.idlePhase * 0.55) * 0.48
    return runtime
  }
