...
export function createM3PutinBossVisual(THREE, lowDetail = false) {
  ...
  buildHumanoidBody(THREE, bodyPivot, {
    mat,
    lowDetail,
    bulk,
    handStyle: 'rj45',
    // The putin.glb albedo already paints the face — no flat mask on top.
    photoHead: false,
    preserveMap: true,
    glbUrl: M3_PUTIN_BOSS_MODEL_URL,
    sleeve: 'long',
    colors: {
      skin: '#e8c4a8',
      torso: '#1e293b',
  ...
  // Combat eye glow without a photo mask: sprites sit on the textured skull and
  // stay invisible until setBossMaskEyesRed flips them to the fight colour.
  // Putin's face sits lower in the scanned crown than man.glb — eyes were on
  // the forehead when keyed to humanoidGlbHeadBounds().centerY.
  const head = humanoidGlbHeadBounds(bulk)
  attachBotEyeGlows(THREE, bodyPivot, {
    color: '#67e8f9',
    size: 0.07,
    idleOpacity: 0,
    points: [
      { x: -0.042 * bulk, y: head.centerY - 0.085, z: Math.max(0.14, Math.abs(head.centerZ) + head.depth * 0.42) },
      { x: 0.042 * bulk, y: head.centerY - 0.085, z: Math.max(0.14, Math.abs(head.centerZ) + head.depth * 0.42) },
    ],
  })
  // Capsule→bone pose sync stretches Putin's belly toward the hands on the
  // home rail; freeze bind pose for the showcase (FPV combat still syncs).
  bodyPivot.userData.freezeGlbPose = false
  group.userData.freezeGlbPoseOnHome = true
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    if (runtime.waiting && !attacking) {
      // Military salute: right arm raised to forehead, left arm at ease
      const arms = bodyPivot.userData?.humanArms
      if (arms) {
        const [lArm, rArm] = arms
        const breathe = Math.sin(time * 1.8) * 0.018
        rArm.rotation.x = 2.0 + breathe
        rArm.rotation.z = (rArm.userData.baseRotZ || 0) - 1.1
        const lPhase = lArm.userData.swayPhase || 0
        lArm.rotation.x = Math.sin(time * 0.9 + lPhase) * 0.04
        lArm.rotation.z = (lArm.userData.baseRotZ || 0) + Math.sin(time * 0.63 + lPhase) * 0.03
      }
    } else {
