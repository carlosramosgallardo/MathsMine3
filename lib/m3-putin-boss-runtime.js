...
export function createM3PutinBossVisual(THREE, lowDetail = false) {
  ...
  // Putin's face sits lower in the scanned crown than man.glb — keep sprites
  // in the eye sockets (not the forehead).
  ...
  attachBotEyeGlows(THREE, bodyPivot, {
    ...
    idleOpacity: 0,
    points: [
      { x: -0.038 * bulk, y: head.centerY - 0.11, z: Math.max(0.15, Math.abs(head.centerZ) + head.depth * 0.45) },
      { x: 0.038 * bulk, y: head.centerY - 0.11, z: Math.max(0.15, Math.abs(head.centerZ) + head.depth * 0.45) },
    ],
  })
  // Home showcase: let the scanned mesh follow capsule walk / attack bones.
