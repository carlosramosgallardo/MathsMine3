...
const MOVE_SPD = 47
const DEFAULT_ATTACK_MS = M3_PUTIN_BOSS_ATTACKS[0].durationMs

/**
 * Textured Putin prop (Sketchfab CC BY). Invisible capsules drive walk/attack
 * while the multi-mesh outfit stays whole — carving it into limb shells
 * detached the suit (home lineup).
 */
export function createM3PutinBossVisual(THREE, lowDetail = false, { retro = false } = {}) {
  const bulk = 1.0
  ...
  const visual = createFixedTexturedStatueVisual(THREE, {
    ...
    // Keep the textured suit fused. Splitting it into rigid arm/leg shells
    // opens seams through the jacket and trousers while walking.
    rigidLimbDriver: false,
    capsuleBulk: bulk,
    capsuleSleeve: 'bare',
...
export function createBossRuntime(state) {
  return {
    ...
    hitFlashUntil: 0,
    attackUntil: 0,
    currentAttack: M3_PUTIN_BOSS_ATTACKS[0],
...
export function updateM3PutinBoss({
  ...
  runtime.currentAttack = selectBossAttack(M3_PUTIN_BOSS_ATTACKS, unitRandom())
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    if (attacking) {
      const attack = runtime.currentAttack || M3_PUTIN_BOSS_ATTACKS[0]
      const at = Math.min(1, (performance.now() - runtime.lastAttackMs) / attack.durationMs)
      if (attack.id === 'sweeping-hook') {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'overhead' })
        const sweep = Math.sin(at * Math.PI)
        bodyPivot.rotation.y = Math.PI + sweep * 0.72
        bodyPivot.rotation.z -= sweep * 0.16
      } else if (attack.id === 'spinning-kick') {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'thrust' })
        const spin = Math.sin(Math.min(1, at / 0.72) * Math.PI)
        const kick = Math.sin(Math.max(0, Math.min(1, (at - 0.18) / 0.56)) * Math.PI)
        bodyPivot.rotation.y = Math.PI + spin * Math.PI * 1.65
        bodyPivot.rotation.z = -kick * 0.22
        const legs = bodyPivot.userData?.humanLegs
        if (legs) {
          legs[0].rotation.x = -kick * 1.18
          legs[1].rotation.x = kick * 0.28
        }
        bodyPivot.position.y += kick * 0.08
      } else {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'thrust' })
      }
