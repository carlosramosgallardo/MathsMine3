...
export function createM3PutinBossVisual(THREE, lowDetail = false) {
  ...
  attachBotEyeGlows(THREE, bodyPivot, {
    color: '#67e8f9',
    y: head.centerY + 0.015,
    // bodyPivot is yawed π and the GLB fit adds another π, so the face looks
    // along +Z in bodyPivot space — place the halos in front of the eyes.
    z: Math.max(0.12, Math.abs(head.centerZ) + head.depth * 0.28),
    spacing: 0.052 * bulk,
    size: 0.088,
    idleOpacity: 0,
  })
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  // Textured skull has no mask eyes; the sprites from attachBotEyeGlows still
  // flip through the same helper as every other boss.
