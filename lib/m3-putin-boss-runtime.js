...
export function createM3PutinBossVisual(THREE, lowDetail = false) {
  ...
  const hairMat = mat('#9ca3af', 0.48, 0.12)
  ...
  // Head — receding grey hair
  addBox(bodyPivot, 0.10, 0.06, 0.10, skinMat, 0, 0.82, 0)
  addBox(bodyPivot, 0.36, 0.32, 0.28, skinMat, 0, 1.00, 0)
  addBox(bodyPivot, 0.38, 0.08, 0.30, hairMat, 0, 1.18, 0.04)
  addBox(bodyPivot, 0.12, 0.16, 0.26, hairMat, -0.22, 1.06, 0.02)
  addBox(bodyPivot, 0.12, 0.16, 0.26, hairMat, 0.22, 1.06, 0.02)
  addBox(bodyPivot, 0.04, 0.025, 0.01, basic('#1a1208'), -0.08, 1.04, -0.142)
  addBox(bodyPivot, 0.04, 0.025, 0.01, basic('#1a1208'), 0.08, 1.04, -0.142)
  addBox(bodyPivot, 0.10, 0.015, 0.01, mat('#c48878', 0.6, 0), 0, 0.90, -0.143)
...
export function syncBossVisual(group, runtime, bossState, time) {
  ...
  if (label) {
    label.position.y = 2.45 * M3_PUTIN_BOSS_SCALE + bob
  }
  if (label?.material?.color) {
    label.material.color.set(active ? '#fb7185' : '#fca5a5')
  }
