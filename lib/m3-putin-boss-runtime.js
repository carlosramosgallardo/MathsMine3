 ...
 * while the multi-mesh outfit stays rigid — no skinning stretch on the suit.
 */
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  const bob = active
    ? (attacking ? Math.sin(time * 12) * 0.018 : Math.sin(time * 4.2) * 0.012)
    : Math.sin(time * 2.2 + runtime.idlePhase) * 0.028
