...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    const rigidAmount = moved > 0.0008 ? 1 : attacking ? 0.9 : active ? 0.68 : 0.36
    const rigidPhase = moved > 0.0008 ? runtime.walkPhase : time * (attacking ? 8.2 : 5.4)
    applyRigidBipedGait(bodyPivot, rigidPhase, rigidAmount, { armScale: attacking ? 1.18 : 1 })
  }
