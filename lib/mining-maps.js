...
export const MINING_MAP_DEFINITIONS = Object.freeze({
  ...
  '2': {
    ...
    chunkX: 0,
    chunkY: -1,
    labelEn: 'Northern Wastes',
    labelEs: 'Yermos del Norte',
    primaryBiome: 'ice',
  },
  '3': {
    id: '3',
    chunkX: 0,
    chunkY: 1,
    labelEn: 'Southern Furnace',
    labelEs: 'Horno del Sur',
  ...
  '4': {
    ...
    chunkX: 1,
    chunkY: 0,
    labelEn: 'Eastern Reach',
    labelEs: 'Confines del Este',
    primaryBiome: 'coast',
  },
  '5': {
    id: '5',
    chunkX: -1,
    chunkY: 0,
    labelEn: 'Western Peaks',
    labelEs: 'Picos del Oeste',
    primaryBiome: 'mountain',
  },
})

/** Corridor bands aligned with existing labyrinth gateway gaps. */
const NORTH_SOUTH_COL_BANDS = MINING_MAP_GATEWAY_COL_BANDS

const EAST_WEST_ROW_BANDS = MINING_MAP_GATEWAY_ROW_BANDS

function inBand(value, bands) {
  return bands.some(([min, max]) => value >= min && value <= max)
}
...
export function detectMiningMapTransition(mapId, gx, gy) {
  ...
  if (id === '1') {
    if (gy < MINING_WORLD_PLAYABLE_MIN_ROW + 0.55 && inBand(col, NORTH_SOUTH_COL_BANDS)) {
      return { targetMapId: '2', row: MINING_WORLD_PLAYABLE_MAX_ROW - 1, col }
    }
    if (gy > MINING_WORLD_PLAYABLE_MAX_ROW + 0.45 && inBand(col, NORTH_SOUTH_COL_BANDS)) {
      return { targetMapId: '3', row: MINING_WORLD_PLAYABLE_MIN_ROW + 1, col }
    }
    if (gx > MINING_WORLD_PLAYABLE_MAX_COL + 0.45 && inBand(row, EAST_WEST_ROW_BANDS)) {
      return { targetMapId: '4', row, col: MINING_WORLD_PLAYABLE_MIN_COL + 1 }
    }
    if (gx < MINING_WORLD_PLAYABLE_MIN_COL + 0.55 && inBand(row, EAST_WEST_ROW_BANDS)) {
      return { targetMapId: '5', row, col: MINING_WORLD_PLAYABLE_MAX_COL - 1 }
    }
    return null
  }

  if (id === '2' && gy > MINING_WORLD_PLAYABLE_MAX_ROW + 0.45 && inBand(col, NORTH_SOUTH_COL_BANDS)) {
    return { targetMapId: '1', row: MINING_WORLD_PLAYABLE_MIN_ROW + 1, col }
  }
  if (id === '3' && gy < MINING_WORLD_PLAYABLE_MIN_ROW + 0.55 && inBand(col, NORTH_SOUTH_COL_BANDS)) {
    return { targetMapId: '1', row: MINING_WORLD_PLAYABLE_MAX_ROW - 1, col }
  }
  if (id === '4' && gx < MINING_WORLD_PLAYABLE_MIN_COL + 0.55 && inBand(row, EAST_WEST_ROW_BANDS)) {
    return { targetMapId: '1', row, col: MINING_WORLD_PLAYABLE_MAX_COL - 1 }
  }
  if (id === '5' && gx > MINING_WORLD_PLAYABLE_MAX_COL + 0.45 && inBand(row, EAST_WEST_ROW_BANDS)) {
    return { targetMapId: '1', row, col: MINING_WORLD_PLAYABLE_MIN_COL + 1 }
  }
...
export function getMiningMapEdgeState(mapId) {
  ...
  return {
    north: adj.north
      ? { open: true, targetMapId: adj.north, bands: colBands }
      : { open: false },
    south: adj.south
      ? { open: true, targetMapId: adj.south, bands: colBands }
      : { open: false },
    east: adj.east
      ? { open: true, targetMapId: adj.east, bands: rowBands }
      : { open: false },
    west: adj.west
      ? { open: true, targetMapId: adj.west, bands: rowBands }
      : { open: false },
  }
}
