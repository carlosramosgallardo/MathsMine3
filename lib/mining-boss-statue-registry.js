 ...
 *  posOverrides: optional { [statueId]: { gx, gy } } for statues currently patrolling. */
export function resolveBossStatueFacing(mapId, gx, gy, angle, posOverrides = null) {
  const aimDx = Math.cos(angle)
  const aimDy = Math.sin(angle)
  let best = null
  for (const statue of getBossStatuesForMap(mapId)) {
    const sgx = posOverrides?.[statue.id]?.gx ?? statue.gx
    const sgy = posOverrides?.[statue.id]?.gy ?? statue.gy
