export const MARKET_COMMANDS = [
  {
    ...
    payment: 'money',
    effect: 'money',
    command: '/ping -c 4 gateway.mainframe',
  ...
  },
  {
    ...
    payment: 'money',
    effect: 'money',
    command: '/kubectl rollout restart deploy/fractal-core',
  ...
  },
  {
    ...
    payment: 'mm3',
    effect: 'mm3',
    command: '/scp file.txt backup:/tmp/',
...
// Extracts just the slash command part, stripping any '=> formula = ?' suffix
export function commandKey(cmd) {
  return normalizeCommandText(String(cmd || '').split('=>')[0]);
}

export function findMarketCommandByText(text) {
  const normalized = normalizeCommandText(text);
  return MARKET_COMMANDS.find((entry) => commandKey(entry.command) === normalized) || null;
}
...
/** Recursive-descent arithmetic (no Function/eval — Sonar javascript:S1523). */
export function evaluateArithmetic(input) {
  ...
  function parseFactor() {
    ...
    if (s.startsWith('log10(', i)) {
      i += 6;
      const value = Math.log10(parseExpression());
    ...
    const match = s.slice(i).match(/^\d+(?:\.\d+)?/);
...
export function evaluateMarketFormula(command, x) {
  ...
  if (!/^[\d\s+\-*/%().log10]+$/.test(formula)) return 0;
  try {
...
export function marketCommandFromBlock(block) {
  ...
  return {
    ...
    effect,
    command,
    solve: fallback?.solve || ((x) => evaluateMarketFormula(command, x)),
  };
}
