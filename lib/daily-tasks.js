...
export async function loadDailyTaskProgress(supabase, walletValue) {
  ...
  const error = miningRes.error || tradingRes.error || marketRes.error || ircRes.error || squeezeRes.error || hiddenRes.error || chainRes.error || claimsRes.error;
  ...
  const counts = {
    ...
    market_chain: toCount(chainRes.count),
  };
