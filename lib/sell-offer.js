import { getWalletTradeMultiplier, WALLET_DECORATIONS, normalizeWalletDecorations } from '@/lib/wallet-decorations';
...
// nftjiLevels: { lucky50: 2, lucky100: 0, ... } — each level adds 5% to that NFTJi's trade effect.
// Life Toll (revive) grants zero commission on all trades.
export function getSellQuote(level = 0, totalMm3 = 0, walletDecorations = [], macroState = {}, diceModifier = 0, nftjiLevels = {}) {
  const safeLevel = clampLevel(level);
  const safeTotal = Math.max(0, Number(totalMm3) || 0);
  const rateCny = getSellRateCny(safeLevel);
  const tradeMultiplier = getWalletTradeMultiplier(walletDecorations, safeLevel, nftjiLevels);
  const owned = new Set(normalizeWalletDecorations(walletDecorations));
  const commissionRate = owned.has(WALLET_DECORATIONS.revive)
    ? 0
    : applyMacroCommissionRate(getCommissionRate(safeTotal), macroState) * (1 + diceModifier);
...
export function getBuyQuote(level = 0, funds = 0, currency = 'EUR', walletDecorations = [], macroState = {}, diceModifier = 0, nftjiLevels = {}) {
  const safeLevel = clampLevel(level);
  ...
  const commissionRate = owned.has(WALLET_DECORATIONS.revive)
    ? 0
    : applyMacroCommissionRate(getBuyCommissionRate(grossMm3), macroState) * (1 + diceModifier);
...
export function formatCompactNum(value, decimals = 2) {
  ...
  if (abs >= 0.01 || abs === 0) return n.toFixed(decimals);
