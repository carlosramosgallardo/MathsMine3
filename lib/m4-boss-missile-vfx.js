...
function symbolId(at) {
  return `${at}-${unitRandom().toString(36).slice(2, 7)}`
}
...
export function spawnBossMissileBurst(existing, { fromGx, fromGy, toGx, toGy, at, mapId = '4', count = 5 }) {
  ...
  for (let i = 0; i < count; i++) {
    const spread = (unitRandom() - 0.5) * 0.45
    const speed = 4.2 + unitRandom() * 2.8
    const loft = 2.8 + unitRandom() * 2.2
    burst.push(createBossMissileSymbol({
      at: at + i * 55 + unitRandom() * 40,
      x: fromGx + dirX * 0.4 + (unitRandom() - 0.5) * 0.2,
      y: 1.1 + unitRandom() * 0.35,
      z: fromGy + dirY * 0.4 + (unitRandom() - 0.5) * 0.2,
      vx: dirX * speed + perpX * spread,
      vy: loft,
      vz: dirY * speed + perpY * spread,
      rot: Math.atan2(dirY, dirX) + Math.PI / 2,
      spin: (unitRandom() - 0.5) * 0.6,
      trail: unitRandom() * Math.PI * 2,
      mapId,
    }))
  }
