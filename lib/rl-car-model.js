...
// One prototype per tier: Home's runtime car and Mining's pixel-art car never
// share geometry, textures or the tint bookkeeping below.
const protoPromises = new Map()

async function loadRlCarGlb(THREE, retro) {
  const { GLTFLoader } = await import('three/addons/loaders/GLTFLoader.js')
  const gltf = await new GLTFLoader().loadAsync((retro ? retroModelUrl : runtimeModelUrl)(RL_CAR_MODEL_URL))
  ...
  root.traverse((obj) => {
    ...
    obj.material.userData.skipDispose = true
    if (obj.material.map) prepareSharedGltfMap(THREE, obj.material.map, { pixelArt: retro })
...
export function loadRlCarPrototype(THREE, { retro = false } = {}) {
  const tier = retro ? 'retro' : 'runtime'
  if (!protoPromises.has(tier)) {
    protoPromises.set(tier, loadRlCarGlb(THREE, retro).catch((err) => {
      protoPromises.delete(tier)
      throw err
    }))
  }
  return protoPromises.get(tier)
}
 ...
 */
export function attachRlCarModel(THREE, parent, { tint = null, lowDetail = false, retro = false, castShadow = false, onReady = null } = {}) {
  // Retro shows the voxel car at once and swaps in the pixel-art tier when it
  // lands; if that never happens the voxel car simply stays.
  const standIn = retro ? addRetroCar(THREE, parent, tint || '#0ea5e9') : null
  loadVisibleModel(parent, () => loadRlCarPrototype(THREE, { retro })).then((result) => {
    if (!result) return
    const { proto, bodyMaterialIndex } = result
    ...
    instance.traverse((obj) => {
      ...
      if (tintColor && obj.userData.rlCarMaterialIndex === bodyMaterialIndex) {
        ...
        mat.userData.skipDispose = false
    ...
    void lowDetail
    if (retro) applyRetroShading(instance)
