...
async function loadRlCarGlb(THREE) {
  ...
  const gltf = await new GLTFLoader().loadAsync(runtimeModelUrl(RL_CAR_MODEL_URL))
...
export function attachRlCarModel(THREE, parent, { tint = null, lowDetail = false, retro = false, castShadow = false, onReady = null } = {}) {
  // Retro shows the voxel car at once and swaps in the model when it lands;
  // if that never happens the voxel car simply stays.
  const standIn = retro ? addRetroCar(THREE, parent, tint || '#0ea5e9') : null
  loadVisibleModel(parent, () => loadRlCarPrototype(THREE)).then((result) => {
    if (!result) return
