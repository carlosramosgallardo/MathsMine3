 ...
 * Fits with a robust AABB so the skull sits in the cabin; eye glows land on
 * the fitted face. Eye sprites are kept across the mesh swap (never `clear()`).
 */
export function attachManHeadInCar(THREE, parent, {
  ...
  const eyeGlows = attachBotEyeGlows(THREE, fit, {
    color: '#67e8f9',
    size: 0.035,
    idleOpacity: 0,
    y: 0.18,
  ...
  loadHumanoidGlbPrototype(THREE, url).then((proto) => {
    ...
    // Drop any previous mesh but keep eye sprites.
    for (const child of [...fit.children]) {
      if (child.userData?.bossEyeGlow) continue
      fit.remove(child)
    }
    ...
    // Feet/chin of the AABB sit on neckY; X/Z centred on the cockpit.
    ...
    // Eyes in fit-local space. After HUMANOID_GLB_YAW (π), face is on −Z.
    const eyeYLocal = box.min.y + sizeY * 0.58
    const eyeZLocal = box.min.z + size.z * 0.18
    const halfLocal = Math.max(0.015, size.x * 0.13)
    const glows = eyeGlows.length
      ? eyeGlows
      : (() => {
          const list = []
          fit.traverse((obj) => { if (obj.userData?.bossEyeGlow) list.push(obj) })
          return list
        })()
    if (glows[0]) glows[0].position.set(center.x - halfLocal, eyeYLocal, eyeZLocal)
    if (glows[1]) glows[1].position.set(center.x + halfLocal, eyeYLocal, eyeZLocal)
