...
/**
 * Hyper-realistic boss/statue head: portrait baked onto a single skull mesh with
 * soft cheek/chin falloff into sampled skin and hair. No floating cutout mask.
 */
export function attachBossMaskHead(THREE, bodyPivot, textureUrl, lowDetail, options = {}) {
  ...
  vault.scale.set(0.80 * w, 0.92 * h, 0.76 * w)
  ...
  if (!lowDetail) {
    const chin = prep(new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 8), fleshMat), renderOrder - 1)
    chin.scale.set(0.18 * w, 0.12 * h, 0.16 * w)
    chin.position.set(0, -0.38 * h, -0.10 * w)
  }
  ...
  hair.scale.set(0.84 * w, 0.38 * h, 0.70 * w)
  hair.position.set(0, 0.38 * h, 0.14 * w)
  for (const side of [-1, 1]) {
    if (!lowDetail) {
      const cheek = prep(new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 8), fleshMat), renderOrder - 1)
      cheek.scale.set(0.16 * w, 0.18 * h, 0.12 * w)
      cheek.position.set(side * 0.28 * w, -0.08 * h, -0.12 * w)
    }
    const ear = prep(new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 8), fleshMat), renderOrder - 1)
    ear.scale.set(0.09 * w, 0.16 * h, 0.11 * w)
    ear.position.set(side * 0.40 * w, -0.04 * h, 0.06 * w)
    const sideHair = prep(new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 8), hairMat), renderOrder + 1)
    sideHair.scale.set(0.14 * w, 0.26 * h, 0.16 * w)
    sideHair.position.set(side * 0.34 * w, 0.16 * h, 0.12 * w)
  }
