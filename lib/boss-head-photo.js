...
export function attachBotEyeGlows(THREE, parent, {
  ...
  renderOrder = 2,
  idleOpacity = 0.85,
  /** Explicit world-local eye centres (overrides y/z/spacing pair). Use for multi-head sculpts. */
  points = null,
} = {}) {
  ...
  const glows = []
  const centres = Array.isArray(points) && points.length
    ? points
    : [{ x: -spacing, y, z }, { x: spacing, y, z }]
  for (const pt of centres) {
    ...
    halo.position.set(Number(pt.x) || 0, Number(pt.y) || 0, Number(pt.z) || 0)
