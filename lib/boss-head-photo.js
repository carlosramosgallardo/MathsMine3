...
export const BOSS_HEAD_MAP_FACE_V0 = 0.16
export const BOSS_HEAD_MAP_FACE_V_SPAN = 0.62
export const BOSS_HEAD_MAP_LON_SPAN = 0.22
...
function makePhotoSkinMaterial(THREE, map, { lowDetail = false, alphaTest = 0 } = {}) {
  const common = {
    roughness: 0.56,
    metalness: 0,
    // Warm emissive keeps the portrait readable in shade without a hologram look.
...
/** Curved portrait shell — wraps the face around the skull without equirectangular stretch. */
function buildBossFaceShellGeometry(THREE, w, h, lowDetail) {
  const maskArc = 2.18
  const maskRadius = w / (2 * Math.sin(maskArc / 2))
  const maskBend = 0.14 * h
  const maskTaper = 0.10
  const maskWarp = (xv, yv) => {
    const s = (yv / (h / 2)) ** 2
    return { x: xv * (1 - maskTaper * s), dz: maskBend * s }
  }
  const maskGeo = new THREE.CylinderGeometry(
    maskRadius, maskRadius, h * 0.94,
    lowDetail ? 14 : 28, lowDetail ? 4 : 8,
  ...
  return { maskGeo, maskArc, maskRadius, maskWarp }
}
...
/**
 * Full human head for bosses/statues: alpha-cutout portrait on a curved shell
 * over a flesh skull (ears, jaw, neck). Same mount contract (origin = skull
 * centre, planeWidth/Height bounds) so hitboxes and nod/yaw anims stay valid.
 */
export function attachBossMaskHead(THREE, bodyPivot, textureUrl, lowDetail, options = {}) {
  ...
  // Cutout masks include hair/jaw silhouette — skip extra hair/ear caps that clash with the photo.
  if (!cutout) {
  ...
  const { maskGeo, maskArc, maskRadius, maskWarp } = buildBossFaceShellGeometry(THREE, w, h, lowDetail)
  const face = prep(new THREE.Mesh(
    maskGeo,
    makePhotoSkinMaterial(THREE, null, { lowDetail, alphaTest: cutout ? 0.45 : 0 }),
  ), renderOrder + 1)
  face.name = `${name}Face`
  face.position.z = maskRadius - 0.035 - zHeadCenter
  ...
  for (const pt of pts) {
    ...
    const ey = (0.5 - pt.v) * h * 0.94 + 0.02 * h
    const warped = maskWarp(-maskRadius * Math.sin(phi), (0.5 - pt.v) * h * 0.94)
    const ex = warped.x
    const ez = (maskRadius - 0.035 - zHeadCenter) - maskRadius * Math.cos(phi) + warped.dz - eyeInset
    if (eyeHaloTex) {
