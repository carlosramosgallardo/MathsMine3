...
export function attachBossMaskHead(THREE, bodyPivot, textureUrl, lowDetail, options = {}) {
  ...
  const mask = prep(new THREE.Mesh(
    new THREE.CylinderGeometry(maskRadius, maskRadius, h, lowDetail ? 10 : 18, 1, true, Math.PI - maskArc / 2, maskArc),
    makeFaceMaterial(THREE, null, { lowDetail, alphaTest: cutout ? 0.5 : 0 }),
  ), renderOrder)
  mask.name = `${name}Shell`
  mask.position.z = maskRadius - 0.03
  ...
  for (const pt of pts) {
    ...
    // local +x; flipY texture puts v=0 at the top of the shell.
    const phi = (pt.u - 0.5) * maskArc
    const ex = -maskRadius * Math.sin(phi)
    const ey = (0.5 - pt.v) * h
    const ez = (maskRadius - 0.03) - maskRadius * Math.cos(phi) - eyeInset
    if (eyeHaloTex) {
  ...
  // behind the mask (positive local z).
  ...
  vault.scale.set(0.815 * w, 0.64 * h, 0.55 * w)
  vault.position.set(0, 0.18 * h, 0.28 * w)
  const occiput = prep(new THREE.Mesh(new THREE.SphereGeometry(0.5, 20, 16), moldMat), renderOrder - 1)
  occiput.scale.set(0.70 * w, 0.64 * h, 0.52 * w)
  occiput.position.set(0, -0.15 * h, 0.28 * w)
  ...
  const strapArc = Math.PI * 1.12
  const strap = prep(new THREE.Mesh(
    new THREE.TorusGeometry(0.463 * w, 0.012, 8, 40, strapArc),
    strapMat,
  ), renderOrder - 1)
  strap.position.set(0, 0.09 * h, 0.17 * w)
  strap.rotation.set(Math.PI / 2 - 0.12, 0, Math.PI / 2 - strapArc / 2)
  strap.scale.set(1.08, 0.88, 1)
