...
import {
  addBossMiningExclusions,
  addM1CipherHouseGhostExclusions,
  isInBossMiningExclusion,
  isInM1CipherHouseGhostZone,
} from './map-boss-mining-exclusion';
import {
  addM2PitchDomeExclusions,
  isInM2PitchDomeExclusion,
} from './m2-pitch-dome';
import { addM1MileiStatueExclusions } from './m1-milei-statue';
import { addM1ZelenskyStatueExclusions } from './m1-zelensky-statue';
import {
  addM2MacronStatueExclusions,
  isInM2MacronStatueExclusion,
} from './m2-macron-statue';
import {
  addNukeCubeReservations,
  isInNukeCubeZone,
} from './nuke-cube';
...
function buildM1OccupiedCells() {
  ...
  addM1MileiStatueExclusions(occupied);
  addM1ZelenskyStatueExclusions(occupied);
  addNukeCubeReservations('1', occupied);
  return occupied;
}
...
function buildPeriphOccupied(mapId) {
  ...
  if (mapId === '2') {
    ...
    addM2PitchDomeExclusions(mapId, occupied);
    addM2MacronStatueExclusions(occupied);
  }
  addBossMiningExclusions(mapId, occupied);
  addM1CipherHouseGhostExclusions(mapId, occupied);
  addNukeCubeReservations(mapId, occupied);
  return occupied;
}
...
function buildPeriphPlacementCandidates(mapId, occupied) {
  ...
  for (let row = MINING_WORLD_PLAYABLE_MIN_ROW; row <= MINING_WORLD_PLAYABLE_MAX_ROW; row += 1) {
    for (let col = MINING_WORLD_PLAYABLE_MIN_COL; col <= MINING_WORLD_PLAYABLE_MAX_COL; col += 1) {
      ...
      if (isInM1CipherHouseGhostZone(mapId, row, col)) continue;
...
function buildAllVisualBlockPositions() {
  ...
  reconcilePeriphBlockConflicts(positions);
  return positions;
}

function reconcilePeriphBlockConflicts(positions) {
  for (const mapId of PERIPHERAL_MAP_IDS) {
    const pending = [];
    for (const [blockHex, pos] of positions) {
      const blockMapId = pos.mapId || getBlockMapId(blockHex);
      if (blockMapId !== mapId) continue;
      if (
        !isInBossMiningExclusion(mapId, pos.row, pos.col) &&
        !isInM1CipherHouseGhostZone(mapId, pos.row, pos.col) &&
        !isInM2PitchDomeExclusion(mapId, pos.row, pos.col) &&
        !isInM2MacronStatueExclusion(mapId, pos.row, pos.col) &&
        !isInNukeCubeZone(mapId, pos.row, pos.col)
      ) continue;
      pending.push(blockHex);
    }
    for (const blockHex of pending) {
      positions.delete(blockHex);
      ...
      const mapLocalIndex = index - MINING_MAP_BLOCK_BANDS[mapId][0];
      const candidates = buildPeriphPlacementCandidates(mapId, occupied);
      const picked = pickPeriphVisualSlot(mapLocalIndex, index, candidates, occupied);
      if (picked) {
        positions.set(blockHex, { row: picked.row, col: picked.col, mapId });
...
export { isInM2PitchDomeExclusion } from './m2-pitch-dome';
export { isInM1MileiStatueExclusion } from './m1-milei-statue';
export { isInM1ZelenskyStatueExclusion } from './m1-zelensky-statue';
export { isInM2MacronStatueExclusion } from './m2-macron-statue';
