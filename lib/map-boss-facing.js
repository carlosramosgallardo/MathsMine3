...
export function advanceShowcaseSpin(state, dt, speed = 0.55) {
  if (!Number.isFinite(state.spinYaw)) {
    state.spinYaw = 0
    state.spinDir = unitRandom() < 0.5 ? -1 : 1
    state.spinFlipIn = 2 + unitRandom() * 5
  }
...
/**
 * Smoothly turn `current` yaw toward `target` along the shortest arc.
 * Exponential-style response: higher `rate` = snappier turn. Keeps target
 * switches (e.g. a different player becoming the nearest) progressive.
 */
export function approachYaw(current, target, dt, rate = 5) {
