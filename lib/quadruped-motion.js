import { applyQuadrupedLimbPose, quadrupedLimbPose } from './quadruped-crawl-limbs.js'
 ...
 * When the sculpt is split into rigid limb shells (lib/quadruped-crawl-limbs.js),
 * those pivots swing on the same gait clock so the crawl reads as articulated.
 *
...
export const QUADRUPED_GAIT = Object.freeze({
  ...
  strideRate: 6.4,
  /** Subtle body rise — limbs carry the gait; avoid hoppy saltitos. */
  bob: 0.022,
 ...
 * has a clock: crawls in place while `moving`, breathes otherwise. Drives
 * rigid limb pivots when `mountTrumpCrawlLimbs` has split the sculpt.
 */
export function animateQuadruped(pivot, { time = 0, moving = 0, attackT = null, jumpT = null } = {}) {
  ...
  applyQuadrupedLimbPose(pivot, quadrupedLimbPose({ time, moving, attackT }))
  return true
}
