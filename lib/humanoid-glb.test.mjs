...
import {
  ...
  HUMANOID_GLB_SRC_CLOTHES,
  HUMANOID_GLB_CAR_NECK_Y,
  humanoidGlbFit,
  ...
  capsulePartsHiddenByGlb,
  enableCharacterCulling,
...
test('car mount neck peek puts shoulders above the Fennec roofline', () => {
  const { scale, offsetY } = humanoidGlbFit()
  const neckParentY = HUMANOID_GLB_SRC_NECK_Y * scale + offsetY
  const shoulderParentY = HUMANOID_GLB_SRC_CLOTHES.armHiY * scale + offsetY
  const seatedShoulderY = shoulderParentY + (HUMANOID_GLB_CAR_NECK_Y - neckParentY)
  assert.ok(seatedShoulderY > 0.85, 'shoulders sit above the Fennec roofline')
  assert.ok(seatedShoulderY < 1.05, 'shoulders stay with the cabin, not a standing pose')
})
...
test('plaza walk strides hips instead of freezing the cheer rest', () => {
  const leftLeg = { rotation: { x: 0.11, y: -0.04, z: 1.22 } }
  const rightLeg = { rotation: { x: 0.11, y: 0.04, z: -1.22 } }
  const host = {
    userData: {
      plazaArmIdle: PLAZA_ARM_IDLE.macronWave,
      strideFloorY: 0,
      humanoidGlbBones: {
        LeftUpperLeg: leftLeg,
      ...
      humanoidGlbBoneRest: {
        LeftUpperLeg: { euler: { x: 0.11, y: -0.04, z: 1.22 } },
        RightUpperLeg: { euler: { x: 0.11, y: 0.04, z: -1.22 } },
      },
      humanArms: [arm(0.4, 0), arm(-0.4, 0)],
      humanLegs: [{ rotation: { x: 0.8, z: 0 }, userData: {} }, { rotation: { x: -0.8, z: 0 }, userData: {} }],
    },
  }
  syncHumanoidGlbPose(host)
  assert.notEqual(leftLeg.rotation.x, 0.11)
  assert.notEqual(rightLeg.rotation.x, 0.11)
  assert.ok(Math.abs(leftLeg.rotation.x - rightLeg.rotation.x) > 0.5, 'hips swing opposite')
  assert.equal(leftLeg.rotation.y, -0.04, 'bind yaw stays')
})
...
// Mining's retro bots mount before their pixel-art tier lands (#282's bots
// never had one). Seating used to hide the voxel torso and head regardless,
// leaving a car with a tool floating in an empty cockpit.
test('applyHumanoidCarMount keeps the stand-in torso and head until the scan exists', () => {
  const torso = { visible: true, userData: {} }
  const voxelHead = { visible: true }
  const arm = { visible: true }
  const leg = { visible: true }
  const parent = {
    userData: {
      tool: { visible: true },
      proceduralHeadMeshes: [voxelHead],
  ...
  applyHumanoidCarMount(parent, { neckY: 0.52 })
  assert.equal(torso.visible, true, 'voxel torso is the rider for now')
  assert.equal(voxelHead.visible, true, 'voxel head is the rider for now')
  assert.equal(arm.visible, false, 'limbs poke through the car')
  assert.equal(leg.visible, false)
  assert.equal(parent.userData.humanoidGlbSeated, true)

  // The tier streams in: the success path re-seats with the split meshes.
  parent.userData.humanoidGlbHeadMesh = { visible: false }
  parent.userData.humanoidGlbBodyMesh = { visible: true }
  parent.userData.humanoidGlbBody = { visible: false }
  applyHumanoidCarMount(parent, { neckY: 0.52 })
  assert.equal(torso.visible, false, 'stand-in retires once the scan is there')
  assert.equal(voxelHead.visible, false)
  assert.equal(parent.userData.humanoidGlbHeadMesh.visible, true)
  assert.equal(parent.userData.humanoidGlbBodyMesh.visible, false, 'below-waist scan stays hidden')
})
...
// The capsule body is hidden from the first frame now, not when the GLB lands,
// so this selection decides both what disappears up front and what comes back
// if the scan never arrives.
test('the GLB hides every capsule body part except its own holder and the connector hands', () => {
  const hand = { userData: { connectorNative: true }, parent: null }
  const fist = { isMesh: true, parent: hand }
  const bodyHold = { isMesh: false, parent: null }
  const torso = { isMesh: true, parent: null }
  const leg = { isMesh: true, parent: null }
  const pivot = { isMesh: false, parent: null }

  const hidden = capsulePartsHiddenByGlb([bodyHold, torso, leg, fist, pivot, null], bodyHold, [hand, null])

  assert.deepEqual(hidden, [torso, leg])
  assert.ok(!hidden.includes(fist), 'the mini-USB hand keeps its connector')
  assert.ok(!hidden.includes(bodyHold), 'the GLB would hide itself')
  assert.ok(!hidden.includes(pivot), 'limb pivots still drive the bones')
})

test('capsule part selection tolerates a rig with no body meshes', () => {
  assert.deepEqual(capsulePartsHiddenByGlb(null, {}, []), [])
  assert.deepEqual(capsulePartsHiddenByGlb([], {}, null), [])
})

// Mining characters were all frustumCulled=false. Rigid meshes can simply be
// culled; skinned ones get a padded bind-pose sphere so a walk or wave never
// drifts a limb past the sphere three would otherwise freeze on first render.
test('enableCharacterCulling culls rigid meshes and pads skinned spheres', () => {
  const sphere = { radius: 1, clone() { return { radius: this.radius } } }
  const rigid = { isMesh: true, frustumCulled: false, geometry: {} }
  const skinned = { isMesh: true, isSkinnedMesh: true, frustumCulled: false, boundingSphere: null, geometry: { boundingSphere: sphere } }
  const lazy = { isMesh: true, isSkinnedMesh: true, frustumCulled: false, boundingSphere: null,
    geometry: { boundingSphere: null, computeBoundingSphere() { this.boundingSphere = sphere } } }
  const root = { traverse(fn) { for (const o of [root, rigid, skinned, lazy]) fn(o) } }
  enableCharacterCulling(root)
  assert.equal(rigid.frustumCulled, true)
  assert.equal(skinned.frustumCulled, true)
  assert.equal(skinned.boundingSphere.radius, 1.5, 'padded, not the shared geometry sphere')
  assert.equal(lazy.boundingSphere.radius, 1.5, 'computes a missing geometry sphere first')
  assert.equal(sphere.radius, 1, 'the geometry sphere itself is untouched')
})
