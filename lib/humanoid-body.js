...
export function humanSkinFromSeed(seed) {
  ...
  return HUMAN_SKIN_TONES[n % HUMAN_SKIN_TONES.length]
}
...
export function buildHumanHead(THREE, parent, { skinMat, hairMat, eyeColor = '#22d3ee', lowDetail = false }) {
  ...
  nose.scale.set(0.04, 0.055, 0.05)
  nose.position.set(0, 0.80, -0.15)
  parent.add(nose)
  const hair = new THREE.Mesh(new THREE.SphereGeometry(0.5, seg, Math.max(8, seg - 4)), hairMat)
  hair.scale.set(0.34, 0.18, 0.32)
  ...
  for (const side of [-1, 1]) {
    ...
    ear.position.set(side * 0.175, 0.81, 0.02)
    parent.add(ear)
  }
  return { head }
}
