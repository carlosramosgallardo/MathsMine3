import { attachHumanoidGlb, humanoidGlbHandSpan, HUMANOID_GLB_URL, HUMANOID_GLB_SRC_NECK_Y } from './humanoid-glb'
 ...
 * options.handStyle — 'sphere' (default; players/bots, hidden after man.glb
 *   loads), 'rj45' (bosses/statue: an RJ45 connector with gold contacts
 *   instead of a fist) or 'miniusb' (legacy metal mini-USB plug).
...
export function buildHumanoidBody(THREE, parent, options) {
  const {
    ...
    preserveMap = false,
    /** Waist split keeps shoulders on the car-rider upper mesh; default is neck. */
    glbBodyCutY = HUMANOID_GLB_SRC_NECK_Y,
    skipGlb = false,
    /** Mining: voxel stand-in kept until the model lands, then frustum-culled. */
    retro = false,
    onGlbReady = null,
  } = options
  const b = bulk
 ...
 * RL-car seated pose stay valid. Sclera + iris painted on the procedural head.
 */
export function buildHumanHead(THREE, parent, { skinMat, hairMat, eyeColor = '#22d3ee', lowDetail = false }) {
  ...
  const addHead = (mesh) => {
    parent.add(mesh)
    heads.push(mesh)
    // Hidden as soon as a scan is *expected*, not once it has arrived: heads are
    // built after attachHumanoidGlb, so waiting for humanoidGlbReady left the
    // procedural face on screen — and, with the body already hidden, showing as
    // a head floating alone. attachHumanoidGlb clears useGlbHead if the scan
    // never loads, which puts this face back.
    if (parent.userData.useGlbHead) mesh.visible = false
    return mesh
  }
...
/**
 * Full human stride: contralateral arm swing + slight torso lean/sway so a
 * walk reads as bipedal weight transfer, not a rigid hop.
 */
export function walkHumanoidStride(host, phase, amp = 0.5, { arms = true, lean = true } = {}) {
  walkHumanoidLegs(host, phase, amp)
  if (!amp) {
    if (lean && host?.rotation) {
      host.rotation.z = 0
      if (Number.isFinite(host.userData?.walkLeanX)) host.rotation.x = host.userData.walkLeanX
    }
    return
  }
  if (arms) {
    const humanArms = host?.userData?.humanArms
    if (humanArms) {
      const [lArm, rArm] = humanArms
      const armAmp = amp * 0.72
      // Opposite to the ipsilateral leg (left arm ↔ right leg).
      if (lArm) {
        const hold = Number(lArm.userData.holdPitch) || 0
        lArm.rotation.x = hold + Math.sin(phase + Math.PI) * armAmp
        lArm.rotation.z = (lArm.userData.baseRotZ || 0) + Math.sin(phase) * 0.04
      }
      if (rArm) {
        const hold = Number(rArm.userData.holdPitch) || 0
        rArm.rotation.x = hold + Math.sin(phase) * armAmp
        rArm.rotation.z = (rArm.userData.baseRotZ || 0) + Math.sin(phase + Math.PI) * 0.04
      }
    }
  }
  if (lean && host?.rotation) {
    const leanAmp = amp / 0.5
    // Contralateral biped rock — same cadence as man.glb capsule→bone walk.
    host.rotation.z = Math.sin(phase) * 0.085 * leanAmp
    host.rotation.x = -0.055 - Math.sin(phase * 2) * 0.045 * leanAmp
    const floorY = Number.isFinite(host.userData?.strideFloorY)
      ? host.userData.strideFloorY
      : (Number.isFinite(host.userData?.baseY) ? host.userData.baseY : 0)
    host.position.y = floorY + Math.abs(Math.sin(phase)) * 0.045 * leanAmp
    host.position.z = Math.sin(phase) * 0.028 * leanAmp
  }
}

/**
 * Phased melee: windup → strike → recovery. Styles:
 * - `thrust` (Putin): both arms forward punch with staggered lead
 * - `overhead` (Kim): right arm cleaves from high to low
 * Coils the host when it is a rigid mesh; skinned scans skip host rock so
 * limb bones alone carry the strike.
 */
export function poseHumanoidMeleeStrike(host, at, {
  style = 'thrust',
  blend = 1,
} = {}) {
  ...
  const t = Math.max(0, Math.min(1, Number(at) || 0))
  const windupP = Math.min(1, t / 0.28)
  const strikeP = t >= 0.28 ? Math.min(1, (t - 0.28) / 0.22) : 0
  const recoverP = t >= 0.50 ? Math.min(1, (t - 0.50) / 0.50) : 0
  const ease = (p) => Math.sin(Math.min(1, p) * Math.PI * 0.5)
  const w = ease(windupP) * (1 - recoverP)
  const s = ease(strikeP) * (1 - recoverP)
  const b = Math.max(0, Math.min(1, blend))
  const deck = host.userData?.baseY || 0
  const skinned = Boolean(host.userData?.humanoidGlbBones)

  if (style === 'overhead') {
    const rX = w * (-1.45) + s * 2.25
    const lX = w * (-0.48) + s * 0.35
    rArm.rotation.x = rArm.rotation.x * (1 - b) + rX * b
    lArm.rotation.x = lArm.rotation.x * (1 - b) + lX * b
    rArm.rotation.z = (rArm.userData.baseRotZ || 0) + (-0.55 * w - 0.22 * s) * b
    lArm.rotation.z = (lArm.userData.baseRotZ || 0) + (0.32 * w + 0.08 * s) * b
    const scissor = Math.sin(Math.min(1, t / 0.6) * Math.PI) * 0.38
    lLeg.rotation.x = scissor * b
    rLeg.rotation.x = -scissor * 0.85 * b
    lLeg.rotation.z = 0
    rLeg.rotation.z = 0
    if (!skinned) {
      host.position.z = (w * 0.12 + s * -0.26) * b
      host.rotation.x = (w * -0.32 + s * 0.22) * b
      host.rotation.z = (w * 0.10 + s * -0.16) * b
      host.position.y = deck + (w * 0.05 + s * 0.015) * b
      host.scale.setScalar(1 + (0.035 * w + 0.02 * s) * b)
    } else {
      host.position.z = (w * 0.04 + s * -0.12) * b
      host.rotation.x = (w * -0.12 + s * 0.08) * b
    }
  } else {
    const lead = w * (-0.78) + s * 1.85
    const guard = w * (-0.48) + s * 1.05
    rArm.rotation.x = rArm.rotation.x * (1 - b) + lead * b
    lArm.rotation.x = lArm.rotation.x * (1 - b) + guard * b
    rArm.rotation.z = (rArm.userData.baseRotZ || 0) + (-0.28 * w - 0.10 * s) * b
    lArm.rotation.z = (lArm.userData.baseRotZ || 0) + (0.30 * w) * b
    const step = Math.sin(Math.min(1, t / 0.55) * Math.PI) * 0.42
    lLeg.rotation.x = -step * 0.6 * b
    rLeg.rotation.x = step * b
    lLeg.rotation.z = 0
    rLeg.rotation.z = 0
    if (!skinned) {
      host.position.z = (w * 0.08 + s * -0.20) * b
      host.rotation.x = (w * -0.14 + s * 0.10) * b
      host.rotation.z = (w * -0.04 + s * 0.06) * b
      host.position.y = deck + (w * 0.02 + s * 0.01) * b
      host.scale.setScalar(1 + (0.03 * w + 0.018 * s) * b)
    } else {
      // Subtle lunge — the skinned arms/legs do the real work.
      host.position.z = (w * 0.05 + s * -0.14) * b
      host.rotation.x = (w * -0.10 + s * 0.07) * b
    }
  }
}
...
export function swayHumanoidArms(host, time, intensity = 1) {
  ...
  for (const arm of arms) {
    ...
    const holdPitch = Number(arm.userData.holdPitch) || 0
    const holdRoll = Number(arm.userData.holdRoll) || 0
    arm.rotation.x = holdPitch + Math.sin(time * 0.9 + phase) * 0.055 * intensity
    arm.rotation.z = (arm.userData.baseRotZ || 0) + holdRoll
      + Math.sin(time * 0.63 + phase * 1.7) * 0.045 * intensity
  }
}
