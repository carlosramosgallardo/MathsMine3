...
export function buildHumanoidBody(THREE, parent, options) {
  ...
  const skinMat = mat(colors.skin, 0.72, 0.02)
  const torsoMat = mat(colors.torso, 0.78, 0.04)
  const armMat = mat(colors.arms ?? colors.torso, 0.78, 0.04)
  const handMat = colors.hands ? mat(colors.hands, 0.7, 0.04) : skinMat
  const legMat = mat(colors.legs, 0.8, 0.04)
  const shoeMat = mat(colors.shoes, 0.7, 0.08)
