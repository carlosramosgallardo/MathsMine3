...
export function walkHumanoidStride(host, phase, amp = 0.5, { arms = true, lean = true } = {}) {
  ...
  if (lean && host?.rotation) {
    ...
    host.rotation.z = Math.sin(phase) * 0.055 * leanAmp
    host.rotation.x = -0.065 - Math.sin(phase * 2) * 0.032 * leanAmp
    // Absolute bounce from the current floor — never += or statues climb into the air.
    ...
    host.position.y = floorY + Math.abs(Math.sin(phase)) * 0.032 * leanAmp
  }
}
