 ...
 * options.handStyle — 'sphere' (default) or 'rj45' (bosses/statue: an RJ45
 *   connector with gold contacts instead of a fist).
 *
 ...
 * old voxel box, keeping the visor band, display pixel and ear pods. Same
 ...
 */
export function buildBotRoundHead(THREE, parent, { headMat, frameMat, visorMat, pixelMat, earMat, lowDetail = false }) {
  ...
  parent.add(frame)
  const visor = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.045, 0.012), visorMat)
  visor.position.set(0, 0.84, -0.157)
  parent.add(visor)
  const pixel = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.025, 0.008), pixelMat)
  pixel.position.set(-0.065, 0.846, -0.163)
  parent.add(pixel)
