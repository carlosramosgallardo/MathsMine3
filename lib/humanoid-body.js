...
export function buildHumanoidBody(THREE, parent, options) {
  ...
  chest.scale.set(0.42 * b, 0.18, 0.24)
  chest.position.set(0, 0.66, 0)
  parts.chest = chest
  if (shirted) {
  ...
  const makeArm = (side) => {
    ...
    deltoid.scale.set(0.112, 0.095, 0.105)
    parts.bodyMeshes.push(deltoid)
    arm.add(deltoid)
    const upper = new THREE.Mesh(new THREE.CapsuleGeometry(0.05, 0.12, capSeg, seg), upperArmMat)
  ...
  const makeLeg = (side) => {
    ...
    const thigh = new THREE.Mesh(new THREE.CapsuleGeometry(0.07, 0.12, capSeg, seg), legMat)
    thigh.position.set(0, -0.08, 0)
    parts.bodyMeshes.push(thigh)
    leg.add(thigh)
    const knee = new THREE.Mesh(new THREE.SphereGeometry(0.048, seg, 8), legMat)
    knee.position.set(0, -0.155, 0)
    parts.bodyMeshes.push(knee)
    leg.add(knee)
    const calf = new THREE.Mesh(new THREE.CapsuleGeometry(0.048, 0.12, capSeg, seg), legMat)
    calf.position.set(0, -0.23, -0.006)
 ...
 * RL-car seated pose stay valid. Eyes keep attachBotEyeGlows for combat red.
 */
