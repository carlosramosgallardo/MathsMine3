...
function addImmediatePedestalShell(THREE, fit, retro = false) {
  const shell = new THREE.Group()
  shell.name = 'statuePlinthImmediateShell'
  const deck = statuePlinthTopY()
  const radius = 200 * STATUE_PLINTH_SCALE
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(radius, radius, deck, retro ? 8 : 32),
  ...
  body.frustumCulled = retro // Mining culls the shell; Home keeps it drawn
  shell.add(body)
  ...
  const logo = new THREE.Mesh(new THREE.CircleGeometry(radius * 0.75, retro ? 8 : 40), logoMaterial)
  ...
  for (const [y, color] of [[0.012, '#00d9ff'], [deck - 0.012, '#ff2bd6']]) {
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(radius * 0.94, 5 * STATUE_PLINTH_SCALE, 4, retro ? 8 : 48),
 ...
 */
export function attachStatuePlinth(THREE, parent, { onReady = null, retro = false } = {}) {
  ...
  const immediateShell = addImmediatePedestalShell(THREE, fit, retro)

  if (retro) { onReady?.(fit); return fit }

  // pedestal.glb is already the 71 KB lightweight asset, which is why it stays
  // out of RUNTIME_MODEL_NAMES (lib/runtime-model-url.js) and has no .runtime
  // derivative. Load it with priority so mining can separate the fixed base
  // before figures start walking.
