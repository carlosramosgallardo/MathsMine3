...
import { findBossAttack, selectBossAttack } from '@/lib/boss-attack-selection'
...
export async function handleBossAttack(req, {
  ...
  // Validate the cosmetic prediction, but never trust a client-selected damage tier.
  if (body.attackId && !findBossAttack(attacks, body.attackId)) {
    return Response.json({ ok: false, error: 'invalid_attack' }, { status: 400 })
  }
  const attack = selectBossAttack(attacks, randomInt(0, 1_000_000) / 1_000_000)
  if (!attack) return Response.json({ ok: false, error: 'invalid_attack' }, { status: 400 })
