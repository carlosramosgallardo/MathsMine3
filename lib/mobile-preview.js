...
export const MM3_QUALITY_QUERY = 'mm3_quality'
export const MM3_QUALITY_SESSION_KEY = 'mm3_quality'
export const MM3_QUALITY_HIGH = 'high'
...
export function isMobilePreviewQualityQuerySearch(search = '') {
  if (!isMobilePreviewDevAllowed()) return false
  const params = new URLSearchParams(search.startsWith('?') ? search.slice(1) : search)
  return params.get(MM3_QUALITY_QUERY) === MM3_QUALITY_HIGH
}

export function readMobilePreviewQualitySession() {
  if (typeof window === 'undefined' || !isMobilePreviewDevAllowed()) return false
  try {
    return window.sessionStorage.getItem(MM3_QUALITY_SESSION_KEY) === MM3_QUALITY_HIGH
  } catch {
    return false
  }
}

export function setMobilePreviewQualitySession(enabled) {
  if (typeof window === 'undefined' || !isMobilePreviewDevAllowed()) return
  try {
    if (enabled) window.sessionStorage.setItem(MM3_QUALITY_SESSION_KEY, MM3_QUALITY_HIGH)
    else window.sessionStorage.removeItem(MM3_QUALITY_SESSION_KEY)
  } catch {
    /* ignore */
  }
}

/** Dev-only: force desktop visual tier + lights (no lite pass) while testing mobile UI. */
export function isMobilePreviewHighQuality() {
  if (!isMobilePreviewDevAllowed()) return false
  if (readMobilePreviewQualitySession()) return true
  if (typeof window !== 'undefined' && isMobilePreviewQualityQuerySearch(window.location.search)) return true
  return false
}
...
/** Native Android /embed/* WebView — force touch UI without 390px preview chrome. */
export function isNativeEmbedActive() {
  if (typeof window === 'undefined') return false
  try {
    if (window.__MM3_NATIVE_EMBED__) return true
    if (document.documentElement.classList.contains('mm3-native-embed')) return true
...
/** Touch/coarse layout — real phone, native embed, or local mobile-preview dev session. */
export function isCoarsePointerLike() {
  if (typeof window === 'undefined') return false
  if (isMobilePreviewHighQuality()) return false
  if (isNativeEmbedActive()) return true
  if (isMobilePreviewActive()) return true
  return Boolean(window.matchMedia?.('(pointer: coarse)')?.matches)
}

/** Touch UI only — does not imply low graphics quality. */
export function isTouchLayout() {
  return isCoarsePointerLike()
}
...
export function withMobilePreviewQuery(path, origin, { highQuality = false } = {}) {
  ...
  if (highQuality) url.searchParams.set(MM3_QUALITY_QUERY, MM3_QUALITY_HIGH)
