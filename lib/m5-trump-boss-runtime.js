...
import { findTrumpBibiEyeCenters, placeEyeGlowsAtLocalPoints } from './prop-eye-glows'
...
export function createM5TrumpBossVisual(THREE, lowDetail = false) {
  ...
  const finishModel = (ok) => {
    ...
    if (ok) {
      // Skin-cluster centres: Bibi (high) + Trump crawler face (forward −Z).
      // AABB eyeLine bands were planting Trump's pair on the rear/hips.
      const centers = findTrumpBibiEyeCenters(THREE, bodyPivot)
      if (centers?.length >= 4) {
        placeEyeGlowsAtLocalPoints(bodyPivot, centers, 0.045)
      }
    }
