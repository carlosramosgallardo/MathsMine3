...
import { attachBossMaskHead, setBossMaskEyesRed } from './boss-head-photo'
import { buildHumanoidBody, swayHumanoidArms } from './humanoid-body'
import { HUMANOID_GLB_CROWN_Y, humanoidGlbHeadBounds, humanoidGlbNeckY } from './humanoid-glb'
...
export function createM5TrumpBossVisual(THREE, lowDetail = false) {
  ...
  group.userData.headY = humanoidGlbHeadBounds(bulk).centerY * M5_TRUMP_BOSS_SCALE
...
export function updateM5TrumpBoss({
  ...
  if (!fighting) {
    ...
    if (distToSpawn > 0.45) {
      ...
      if (nearestFighter) {
        ...
        const dxF = nearestFighter.gx - M5_TRUMP_BOSS_SPAWN.gx
        const dyF = nearestFighter.gy - M5_TRUMP_BOSS_SPAWN.gy
        const dF = Math.hypot(dxF, dyF) || 1
        const lean = Math.min(0.25, dF * 0.5)
        const tx = M5_TRUMP_BOSS_SPAWN.gx + (dxF / dF) * lean
        const ty = M5_TRUMP_BOSS_SPAWN.gy + (dyF / dF) * lean
  ...
  if (!inAttackRange) {
    const step = Math.min(dist - M5_TRUMP_BOSS_ATTACK_RANGE, speed)
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    if (runtime.waiting && !attacking) {
      // Politician wave: right arm raised forward, sweeping side-to-side
      const arms = bodyPivot.userData?.humanArms
      if (arms) {
        const [lArm, rArm] = arms
        const wave = Math.sin(time * 4.5) * 0.6
        rArm.rotation.x = 1.8
