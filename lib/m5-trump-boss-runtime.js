import { addRetroCrawler } from './mining-retro-props.js'
...
const DEFAULT_ATTACK_MS = M5_TRUMP_BOSS_ATTACKS[0].durationMs
...
/**
 * Trump is a crawling sculpt, not a capsule rig. Keep its GLB fused so the
 * trousers, coat and rider stay watertight; the shared quadruped body pose
 * supplies the crawl without cutting clothing into separate limb shells.
 * Until the sculpt streams in, a slab keeps the boss visible and hittable.
 */
export function createM5TrumpBossVisual(THREE, lowDetail = false, { retro = false } = {}) {
  ...
  // Retro: the voxel crawler stands in until the pixel-art sculpt streams;
  // it also tags the pivot as quadruped so motion runs from frame one.
  const standIn = retro ? addRetroCrawler(THREE, bodyPivot) : []
  attachQuadrupedGlb(THREE, bodyPivot, {
    url: M5_TRUMP_BOSS_MODEL_URL,
    retro,
    onReady: (clone) => {
      if (clone) {
        for (const mesh of standIn) mesh.visible = false
        let sourceMesh = null
        clone.traverse((obj) => {
...
export function createBossRuntime(state) {
  return {
    ...
    hitFlashUntil: 0,
    attackUntil: 0,
    currentAttack: M5_TRUMP_BOSS_ATTACKS[0],
...
function stepM5TrumpBoss({
  ...
  runtime.currentAttack = selectBossAttack(M5_TRUMP_BOSS_ATTACKS, unitRandom())
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    const attack = runtime.currentAttack || M5_TRUMP_BOSS_ATTACKS[0]
    const attackT = attacking ? Math.min(1, (now - runtime.lastAttackMs) / attack.durationMs) : null
    if (attackT != null && attack.id === 'side-swipe') {
      const swipe = Math.sin(attackT * Math.PI)
      applyQuadrupedPose(bodyPivot, quadrupedPose({ time, moving: 0.18 }))
      bodyPivot.rotation.y = Math.PI + swipe * 0.82
      bodyPivot.rotation.z += swipe * 0.34
      bodyPivot.position.z -= swipe * 0.10
    } else if (attackT != null && attack.id === 'bull-rush') {
      const charge = Math.sin(attackT * Math.PI)
      const impact = Math.sin(Math.max(0, Math.min(1, (attackT - 0.42) / 0.38)) * Math.PI)
      applyQuadrupedPose(bodyPivot, quadrupedPose({ time: time * 1.8, moving: 1 }))
      bodyPivot.rotation.x += charge * 0.24 - impact * 0.42
      bodyPivot.position.y += Math.sin(attackT * Math.PI * 2) ** 2 * 0.05
      bodyPivot.position.z += charge * 0.22
    } else {
