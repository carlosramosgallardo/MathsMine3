  ...
  const finishModel = (ok) => {
    ...
    if (ok) {
      ...
      placeEyeGlowsFromFit(THREE, bodyPivot, {
        ...
        extraHeads: [{ eyeLine: 0.38, forwardFrac: 0.05 }],
      })
    }
    ...
    onReady: (clone) => {
      if (clone) {
        clone.traverse((obj) => {
          ...
          // Bake already vivifies COLOR_0 in trump.glb; upgrade Uint8 → Float32
          // so later combat tints cannot truncate midtones, then soft-lift once.
