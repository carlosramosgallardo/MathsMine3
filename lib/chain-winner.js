...
export async function activateDemineMode(supabase, solverWallet, allMinedRows, formulaSolved, formulaChainIndexStart = null) {
  ...
  // Record solver (INSERT, not upsert — UNIQUE(wallet) is the source of truth
  // for "already solved"). A 23505 here means a concurrent request already
  // won the race; bail out before awarding anything so two simultaneous
  // correct attempts can't both collect the 1000 MM3 reward.
  ...
  if (solverError) {
    if (solverError.code !== '23505') {
      console.error('[chain-winner] solver insert error:', solverError);
    }
    return { complete: false, alreadySolvedByRace: true };
  }
