...
export async function checkAndAwardChainWinner(supabase) {
  const [{ count: freeMinedCount }, { data: nftjiOwners }, { data: existingWinner }] = await Promise.all([
    ...
    supabase.from('player_progress').select('market_nftji_key').not('market_nftji_key', 'is', null),
  ...
  const { count: totalNftjiCells } = await supabase
    .from('mm3_market_blocks').select('block_key', { count: 'exact', head: true });
  const freeBlocksTarget = TOTAL_BOARD_CELLS - (Number(totalNftjiCells) || 0);
  const ownedNftjiCount = new Set((nftjiOwners || []).map((r) => r.market_nftji_key)).size;
  ...
  await Promise.all([
    supabase.from('mm3_irc_messages').insert({
      wallet: 'system',
