 ...
 * while the textured mesh stays rigid.
 */
export function createM4KimBossVisual(THREE, lowDetail = false) {
  ...
  const visual = createFixedTexturedStatueVisual(THREE, {
    ...
    lowDetail,
    eyeSize: 0.055,
    // Fit yaw is π/2 only in bodyPivot space (parent π does not flip local
    // samples) — this plant’s face reads on +Z, so lock sockets there.
    eyeLine: 0.48,
    eyeFaceSign: 1,
    eyeForwardFrac: 0.06,
    eyeSkullFrac: 0.12,
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  const bob = active
    ? (attacking ? Math.sin(time * 10) * 0.022 : Math.sin(time * 3.8) * 0.014)
    : Math.sin(time * 1.8 + runtime.idlePhase) * 0.028
