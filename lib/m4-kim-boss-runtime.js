...
export function createM4KimBossVisual(THREE, lowDetail = false) {
  ...
  // Mao jacket — torso kept below the portrait cube
  addBox(bodyPivot, 0.54, 0.44, 0.30, suitGrey, 0, 0.50, 0.02)
  if (!lowDetail) {
    addBox(bodyPivot, 0.48, 0.07, 0.026, suitDark, 0, 0.67, -0.145)
    addBox(bodyPivot, 0.30, 0.09, 0.05, suitDark, 0, 0.665, -0.055)
    addBox(bodyPivot, 0.34, 0.05, 0.032, suitEdge, 0, 0.615, -0.166)
    addBox(bodyPivot, 0.04, 0.34, 0.024, suitDark, 0, 0.49, -0.150)
    addBox(bodyPivot, 0.18, 0.05, 0.022, suitEdge, -0.11, 0.61, -0.154)
    addBox(bodyPivot, 0.18, 0.05, 0.022, suitEdge, 0.11, 0.61, -0.154)
    addBox(bodyPivot, 0.036, 0.036, 0.018, starGold, -0.055, 0.51, -0.165)
    addBox(bodyPivot, 0.036, 0.036, 0.018, starGold, 0.055, 0.51, -0.165)
  }
  // Juche star badge
  addBox(bodyPivot, 0.10, 0.10, 0.02, starRed, -0.06, 0.56, 0.06)
  addBox(bodyPivot, 0.05, 0.05, 0.01, starGold, -0.06, 0.56, 0.062)
  ...
  addBox(bodyPivot, 0.06, 0.08, 0.08, skinBright, 0.05, 0.48, -0.14)
  if (!lowDetail) addBox(bodyPivot, 0.18, 0.045, 0.026, suitDark, 0, 0.43, -0.166)

  // Sleeves
  addBox(bodyPivot, 0.13, 0.34, 0.15, suitGrey, -0.36, 0.52, 0)
  addBox(bodyPivot, 0.13, 0.34, 0.15, suitGrey, 0.36, 0.52, 0)
  // North Korea armband — blue/red/blue bands with the white star on the red.
  const flagBlue = basic('#1d4ed8')
  const flagRed = basic('#dc2626')
  const flagStar = basic('#f8fafc')
  addBox(bodyPivot, 0.04, 0.08, 0.02, flagBlue, -0.36, 0.62, -0.16)
  addBox(bodyPivot, 0.04, 0.12, 0.02, flagRed, -0.36, 0.52, -0.16)
  addBox(bodyPivot, 0.04, 0.08, 0.02, flagBlue, -0.36, 0.42, -0.16)
  addBox(bodyPivot, 0.016, 0.016, 0.012, flagStar, -0.365, 0.545, -0.172)
  if (!lowDetail) {
    addBox(bodyPivot, 0.11, 0.045, 0.026, suitEdge, -0.36, 0.40, -0.085)
    addBox(bodyPivot, 0.11, 0.045, 0.026, suitEdge, 0.36, 0.40, -0.085)
  }
