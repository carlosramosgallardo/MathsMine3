...
function installKimLegShader(bodyPivot) {
  const fit = bodyPivot?.userData?.statueGlbFit
  if (!fit || bodyPivot.userData.kimLegShaderUniforms) return
  const swingUniforms = []
  fit.traverse((mesh) => {
    ...
    if (height <= 0.001 || width <= 0.001) return
    const jointY = bounds.min.y + height * 0.285
    ...
    const nextMaterials = materials.map((source) => {
      ...
      const swing = { value: 0 }
      swingUniforms.push(swing)
      material.onBeforeCompile = (shader, renderer) => {
        previousCompile?.(shader, renderer)
        shader.uniforms.kimLegSwing = swing
        shader.uniforms.kimLegJointY = { value: jointY }
        shader.uniforms.kimLegCenterX = { value: centerX }
        shader.uniforms.kimLegCenterZ = { value: centerZ }
        shader.vertexShader = shader.vertexShader
          .replace('#include <common>', `#include <common>
uniform float kimLegSwing;
uniform float kimLegJointY;
uniform float kimLegCenterX;
uniform float kimLegCenterZ;`)
          .replace('#include <begin_vertex>', `#include <begin_vertex>
if (position.y < kimLegJointY) {
  float kimSide = position.x < kimLegCenterX ? 1.0 : -1.0;
  float kimAngle = kimLegSwing * kimSide;
  float kimSin = sin(kimAngle);
  float kimCos = cos(kimAngle);
  float kimY = transformed.y - kimLegJointY;
  float kimZ = transformed.z - kimLegCenterZ;
  transformed.y = kimLegJointY + kimCos * kimY - kimSin * kimZ;
  transformed.z = kimLegCenterZ + kimSin * kimY + kimCos * kimZ;
}`)
      }
      material.customProgramCacheKey = () => 'kim-articulated-lower-legs-v1'
      material.needsUpdate = true
      return material
    })
    mesh.material = Array.isArray(mesh.material) ? nextMaterials : nextMaterials[0]
  })
  bodyPivot.userData.kimLegShaderUniforms = swingUniforms
}

/**
 * Textured Kim Jong-un prop (Sketchfab CC BY). The source is a fused,
 * multi-surface coat mesh, so it must remain whole: carving triangles into
 * artificial limbs creates long texture strips and overlapping coat panels.
 */
export function createM4KimBossVisual(THREE, lowDetail = false) {
  ...
  const visual = createFixedTexturedStatueVisual(THREE, {
    ...
    gx,
    gy,
    // Source faces +X (T-pose depth along Z); π/2 plant turns him face-on
    // like Putin/Macron after their −Z bake.
    glbYaw: Math.PI / 2,
  ...
  visual.group.userData.onModelReady = () => installKimLegShader(visual.bodyPivot)
  return visual
}
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  // Idle/combat bob only — attack coils the torso via poseHumanoidMeleeStrike.
  ...
  if (bodyPivot) {
    if (runtime.waiting && !attacking) {
      ...
      const at = Math.min(1, (performance.now() - runtime.lastAttackMs) / 780)
      poseHumanoidMeleeStrike(bodyPivot, at, { style: 'overhead' })
    } else {
      if (moved <= 0.0008) {
        swayHumanoidArms(bodyPivot, time, active ? 1.45 : 1)
      }
      syncCapsuleLocomotion(bodyPivot, runtime, { moved, attacking: false, walkAmp: 0.70 })
    }
    // Kim's coat is one fused prop. Human locomotion therefore comes from a
    // planted weight transfer and contralateral shoulder turn, never from
    // splitting or stretching the uniform. This remains visible in wide shots.
    const gaitAmount = moved > 0.0008 ? 1 : active ? 0.58 : 0.3
    const gaitPhase = moved > 0.0008 ? runtime.walkPhase : time * 3.4
    const stride = Math.sin(gaitPhase) * gaitAmount
    const shoulders = Math.cos(gaitPhase) * gaitAmount
    bodyPivot.rotation.z = stride * 0.055
    bodyPivot.rotation.y = Math.PI + shoulders * 0.045
    bodyPivot.position.x = stride * 0.018
    installKimLegShader(bodyPivot)
    for (const uniform of bodyPivot.userData.kimLegShaderUniforms || []) {
      uniform.value = stride * 0.42
    }
  }
