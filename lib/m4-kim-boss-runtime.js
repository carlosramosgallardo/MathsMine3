 ...
 * while the textured mesh stays whole.
 */
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    ...
    const rigidAmount = moved > 0.0008 ? 1 : attacking ? 0.95 : active ? 0.72 : 0.4
    const rigidPhase = moved > 0.0008 ? runtime.walkPhase : time * (attacking ? 7.8 : 5.2)
    applyRigidBipedGait(bodyPivot, rigidPhase, rigidAmount, { armScale: attacking ? 1.22 : 1 })
  }
