...
import { bossMoveDelta, syncCapsuleLocomotion, applyCapsuleBodyBob, finishBossVisualMatrices } from './capsule-anim-driver'
...
/**
 * Textured Kim Jong-un prop (Sketchfab CC BY). Capsule limbs drive walk/attack;
 * the suit is split into rigid shells so arms/legs move without cloth stretch.
 */
export function createM4KimBossVisual(THREE, lowDetail = false) {
  ...
  const visual = createFixedTexturedStatueVisual(THREE, {
    ...
    onGlbReady: (_group, bodyPivot) => {
      // Same crown-fitted plant space as Putin; slightly softer arm cut for the
      // bulkier coat silhouette.
      mountBipedLimbsOnCapsules(THREE, bodyPivot, {
        profile: { ...PUTIN_BIPED_PROFILE, armX: 0.14, legX: 0.04 },
      })
    },
  })
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  const bob = active
    ? (attacking ? Math.sin(time * 10) * 0.01 : Math.sin(time * 3.8) * 0.006)
    : Math.sin(time * 1.8 + runtime.idlePhase) * 0.014
