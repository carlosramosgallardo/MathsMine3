...
import { selectBossAttack } from './boss-attack-selection'

const MOVE_SPD = 47
const DEFAULT_ATTACK_MS = M4_KIM_BOSS_ATTACKS[0].durationMs
 ...
 */
export function createM4KimBossVisual(THREE, lowDetail = false, { retro = false } = {}) {
  const bulk = 1.08
...
export function createBossRuntime(state) {
  return {
    ...
    hitFlashUntil: 0,
    attackUntil: 0,
    currentAttack: M4_KIM_BOSS_ATTACKS[0],
...
export function updateM4KimBoss({
  ...
  runtime.currentAttack = selectBossAttack(M4_KIM_BOSS_ATTACKS, unitRandom())
  runtime.attackUntil = now + (runtime.currentAttack?.durationMs || DEFAULT_ATTACK_MS)
  ...
  if (!myDead && fighterMatchesLocal(target, myWallet, myIdentity)) {
    onAttack?.({
      ...
      bossGx: runtime.gx,
      bossGy: runtime.gy,
      attackId: runtime.currentAttack.id,
    })
  }
...
export function syncBossVisual(group, runtime, bossState, time, groundY = 0) {
  ...
  if (bodyPivot) {
    if (runtime.waiting && !attacking) {
      ...
      const attack = runtime.currentAttack || M4_KIM_BOSS_ATTACKS[0]
      const at = Math.min(1, (performance.now() - runtime.lastAttackMs) / attack.durationMs)
      if (attack.id === 'shoulder-barge') {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'thrust' })
        const drive = Math.sin(at * Math.PI)
        bodyPivot.rotation.x = -drive * 0.36
        bodyPivot.rotation.z = drive * 0.18
        bodyPivot.position.z = -drive * 0.20
      } else if (attack.id === 'quake-stomp') {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'overhead' })
        const lift = Math.sin(Math.min(1, at / 0.48) * Math.PI)
        const slam = Math.sin(Math.max(0, Math.min(1, (at - 0.38) / 0.42)) * Math.PI)
        bodyPivot.position.y += lift * 0.13
        bodyPivot.rotation.x = -lift * 0.12 + slam * 0.28
        const legs = bodyPivot.userData?.humanLegs
        if (legs) {
          legs[0].rotation.x = -lift * 0.82
          legs[1].rotation.x = slam * 0.32
        }
      } else {
        poseHumanoidMeleeStrike(bodyPivot, at, { style: 'overhead' })
      }
