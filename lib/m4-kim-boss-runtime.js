...
/**
 * Textured Kim Jong-un chibi (Sketchfab CC BY). The whole body rides the
...
export function createM4KimBossVisual(THREE, lowDetail = false) {
  ...
  const placeholder = new THREE.Mesh(
    ...
    lowDetail
      ? new THREE.MeshLambertMaterial({ color: '#3f3f46' })
      : new THREE.MeshStandardMaterial({ color: '#3f3f46', roughness: 0.8, metalness: 0.06 }),
  )
  ...
  // Chibi face sits near the crown after AABB fit (feet at 0, height ≈ 1.9).
  // Pull the mesh forward on the rail — Kim's deep Z AABB otherwise reads as
  // standing behind the lineup.
  attachTexturedStatueGlb(THREE, bodyPivot, {
    url: M4_KIM_BOSS_MODEL_URL,
    onReady: (clone) => {
      const fit = bodyPivot.userData.statueGlbFit
      if (fit) fit.position.z += 0.22
  ...
  group.userData.headY = humanoidGlbHeadBounds(bulk).centerY * M4_KIM_BOSS_SCALE
