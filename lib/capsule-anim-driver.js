import { addRetroHead } from './mining-retro-props.js'
import { buildHumanoidBody, humanSkinFromSeed, walkHumanoidStride } from './humanoid-body'
...
export function attachCapsuleAnimDriver(THREE, parent, {
  ...
  if (retro) {
    addRetroHead(THREE, parent, { skin, seed: skinSeed })
  ...
  configureCapsuleVisibility(parent, parts, visibleLimbs)
...
export function syncCapsuleLocomotion(bodyPivot, runtime, {
  ...
  // Skinned scans already bend limbs — skip extra whole-mesh rock.
  const lean = !bodyPivot.userData?.humanoidGlbBones
  if (moved > 0.0008) {
    runtime.walkPhase = (runtime.walkPhase || 0) + moved * 16
    walkHumanoidStride(bodyPivot, runtime.walkPhase, walkAmp, { lean })
    applyRigidBipedGait(bodyPivot, runtime.walkPhase, Math.min(1, walkAmp * 1.3))
  } else {
    walkHumanoidStride(bodyPivot, 0, 0, { lean })
    applyRigidBipedGait(bodyPivot, 0, 0)
    if (lean && bodyPivot) {
      bodyPivot.position.z = 0
...
/** Shared combat bob for capsule-driven textured bosses (Kim / Putin). */
export function applyCapsuleBodyBob(bodyPivot, {
  bob,
  bobScale = 1,
  hitFlash = false,
  attacking = false,
  ...
  bodyPivot.scale.setScalar(hitFlash ? 1.04 : attacking ? attackScale : 1)
  bodyPivot.position.z = attacking ? attackZ : 0
  bodyPivot.rotation.x = attacking ? attackPitch : 0
}

/**
 * Tiny torso bob once limbs carry the gait (avoids hoppy saltitos).
 * Flattened (no nested ternaries) for Sonar.
 */
export function capsuleDriverTorsoBob({
  time = 0,
  idlePhase = 0,
  active = false,
  attacking = false,
  ...
  if (active && attacking) return Math.sin(time * attackHz) * 0.01
  if (active) return Math.sin(time * walkHz) * 0.006
  return Math.sin(time * idleHz + idlePhase) * 0.014
}

export function finishBossVisualMatrices(group, bodyPivot) {
  group?.updateMatrix?.()
...
/** Hide the capsule once a GLB owns the body (retro defers this to the swap). */
export function configureCapsuleVisibility(parent, parts, visibleLimbs) {
  ...
  for (const mesh of parts.bodyMeshes || []) {
    if (!mesh) continue
    // A GLB has already claimed this body; don't hand its limbs back.
    if (mesh.userData?.glbSuppressed) { mesh.visible = false; continue }
    if (!underLimb(mesh)) {
      mesh.visible = false
      continue
    }
