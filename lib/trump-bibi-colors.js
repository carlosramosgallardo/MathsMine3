...
export function vivifyTrumpBibiVertexColors(geometry, THREE = null) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    // Dollar hanging off Bibi's rod — pale flat card in front of Trump's face.
    const isDollarBill = y > 0.55 && y < 1.02 && z > 0.28 && z < 0.72
      && Math.abs(x) < 0.22 && mid > 0.5 && sat < 0.38 && !isRed
