...
export function vivifyTrumpBibiVertexColors(geometry, THREE = null) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const isDollarBill = isTrumpDollarBillVertex(x, y, z)
...
export function isTrumpDollarBillVertex(x, y, z) {
  // The baked bill is the small card at the bottom of the vertical fishing
  // line. Its original colours are mixed purple/beige rather than parchment,
  // so the old colour gates selected zero vertices. The card is spatially
  // isolated at the front edge of the normalized sculpt; a tight box paints
  // all of it without reaching Trump, Bibi, the rod, or the line.
  return y > 0.44 && y < 0.89
    && z > 0.76 && z < 0.98
    && x > -0.43 && x < -0.25
}
