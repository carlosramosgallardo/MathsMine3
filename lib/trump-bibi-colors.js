...
export function isTrumpDollarBillVertex(x, y, z, r, g, b, { isHairDark = false } = {}) {
  ...
  const min = Math.min(r, g, b)
  const mid = (r + g + b) / 3
  return y > 0.68 && y < 1.08 && z > 0.60 && z < 0.78
    && Math.abs(x) < 0.12 && mid > 0.45 && !isHairDark && max - min < 0.55
}

/** USD green lifted for ACES / MeshStandard vertex paint. */
export function trumpDollarBillRgb(mid = 0.75) {
  const lift = 0.04 + mid * 0.08
  return [
    Math.min(1, 0.68 + lift),
    Math.min(1, 0.92 + lift * 0.6),
    Math.min(1, 0.72 + lift * 0.35),
  ]
}
