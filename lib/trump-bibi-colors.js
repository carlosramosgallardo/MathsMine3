...
export function isTrumpDollarBillVertex(x, y, z, r, g, b, { isHairDark = false } = {}) {
  ...
  // Pale parchment card on the line in front of Trump (not suit/skin/hat).
  const paleCard = mid > 0.5 && sat < 0.4 && r > 0.45 && g > 0.45
  // Already painted USD green (re-apply so vivify does not wash it out).
  const alreadyGreen = g > r * 1.35 && g > b * 1.2 && g > 0.5 && mid > 0.3 && mid < 0.85
  return y > 0.58 && y < 1.05 && z > 0.38 && z < 0.70
    && Math.abs(x) < 0.12
...
export function trumpDollarBillRgb(mid = 0.75) {
  ...
  // ~#22c55e base with lighter face (#4ade80) on bright verts.
  return [
    0.14 + t * 0.20,
    0.58 + t * 0.32,
    0.22 + t * 0.20,
