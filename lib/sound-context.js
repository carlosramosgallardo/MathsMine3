...
/** Android shell WebViews — ambient is owned by native AmbientMusic. */
function isNativeAppShell() {
  if (typeof window === 'undefined') return false;
  try {
    return !!(
      window.__MM3_NATIVE_APP__ ||
      document.documentElement?.classList?.contains('mm3-native-app') ||
      document.documentElement?.classList?.contains('mm3-native-embed') ||
...
export function SoundProvider({ children }) {
  // SSR always renders "sound on"; the saved preference is applied after
  // mount. Reading localStorage in the initializer made the first client
  // render differ from the server HTML (title/aria/icon of the mute button),
  // which failed hydration and forced React to regenerate the whole tree.
  ...
  const persistSoundEventRef = useRef(false);

  useEffect(() => {
    try {
      setEnabled(localStorage.getItem('mm3-sound-enabled') !== 'false');
      setMusicEnabled(localStorage.getItem('mm3-music-enabled') !== 'false');
    } catch { /* storage unavailable — keep defaults */ }
  }, []);

  // Native header toggles push prefs via CustomEvent (and localStorage).
  useEffect(() => {
    const onPrefs = (e) => {
      const d = e?.detail;
      if (!d) return;
      if (typeof d.sound === 'boolean' && !persistSoundEventRef.current) setEnabled(d.sound);
      if (typeof d.music === 'boolean') setMusicEnabled(d.music);
    };
    window.addEventListener('mm3-sound-prefs', onPrefs);
    return () => window.removeEventListener('mm3-sound-prefs', onPrefs);
  }, []);

  useEffect(() => {
    if (!prefsHydratedRef.current) return;
    localStorage.setItem('mm3-sound-enabled', enabled ? 'true' : 'false');
    persistSoundEventRef.current = true;
    window.dispatchEvent(new CustomEvent('mm3-sound-prefs', { detail: { sound: enabled } }));
    persistSoundEventRef.current = false;
  }, [enabled]);
  ...
  // Declared after the persist effects: on the mount pass they see the flag
  // still false and skip, so the stored preference is never overwritten by
  // the SSR defaults before the hydration read above lands.
  useEffect(() => {
  ...
  // Skip inside the Android app shell: native AmbientMusic is the single source.
  useEffect(() => {
