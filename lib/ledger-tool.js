...
async function loadLedgerPrototype(THREE) {
  ...
  protoPromise = import('three/addons/loaders/GLTFLoader.js')
    .then(({ GLTFLoader }) => new GLTFLoader().loadAsync(runtimeModelUrl(LEDGER_TOOL_URL)))
    .then((gltf) => {
...
export function createLedgerTool(THREE, {
  ...
  // Retro: the box ledger is the placeholder; the model takes over
  ...
  loadVisibleModel(tool, () => loadLedgerPrototype(THREE)).then((proto) => {
