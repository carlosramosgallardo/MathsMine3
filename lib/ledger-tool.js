import { addRetroLedger } from './mining-retro-props.js'
...
async function loadLedgerPrototype(THREE, { retro = false } = {}) {
  if (typeof window === 'undefined') return null
  const tier = retro ? 'retro' : 'runtime'
  if (protoPromises.has(tier)) return protoPromises.get(tier)
  const promise = import('three/addons/loaders/GLTFLoader.js')
    .then(({ GLTFLoader }) => new GLTFLoader().loadAsync((retro ? retroModelUrl : runtimeModelUrl)(LEDGER_TOOL_URL)))
    .then((gltf) => {
      const root = gltf.scene
      root.traverse(obj => {
        if (obj.geometry) obj.geometry.userData.skipDispose = true
  ...
  protoPromises.set(tier, promise)
  return promise
}
...
export function createLedgerTool(THREE, {
  ...
  // Retro: the box ledger is the placeholder; the pixel-art model takes over
  // when it lands, or the box stays if it never does.
  const standIn = retro ? addRetroLedger(THREE, tool, tint, LEDGER_HOLD_LENGTH) : null
  ...
  // The voxel stand-in already covers retro's wait; keep the thin cylinder
  // for the runtime tier only.
  placeholder.visible = !retro
  tool.add(placeholder)

  loadVisibleModel(tool, () => loadLedgerPrototype(THREE, { retro })).then((proto) => {
    ...
    if (retro) applyRetroShading(clone)
