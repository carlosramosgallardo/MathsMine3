...
function box(THREE, parent, material, x, y, z, w, h, d) {
...
export function addRetroHead(THREE, parent, { skin = '#e8c4a8', hair = '#342820', seed = '' } = {}) {
  ...
  box(THREE,parent,hairMat,0,.987,.02,.28,.055,.27)
  if (seed === 'milei') {
    for(const side of [-1,1]) box(THREE,parent,hairMat,side*.14,.91,.025,.07,.17,.25)
  }
  for (const side of [-1,1]) box(THREE,parent,eyes,side*.055,.86,-.13,.026,.024,.012)
  box(THREE,parent,skinMat,0,.82,-.145,.035,.045,.04)
  parent.userData.proceduralHeadMeshes = parent.userData.proceduralHeadMeshes || []
  parent.userData.proceduralHeadMeshes.push(head)
  return { head }
}
...
export function addRetroNukePanels(THREE, group) {
  ...
  for (const x of [-.48,.48]) box(THREE,group,steel,x,.48,0,.025,.88,.9)
  for(const z of [-.5,.5]) {
    box(THREE,group,caution,0,.5,z,.52,.5,.018)
    box(THREE,group,black,0,.5,z*1.02,.12,.36,.018)
    box(THREE,group,black,0,.5,z*1.03,.36,.12,.018)
  }
}
...
export function addRetroCrawler(THREE, parent) {
  ...
  box(THREE,parent,suit,0,.55,0,.58,.3,.84)
  for(const x of [-.23,.23]) for(const z of [-.31,.31]) box(THREE,parent,suit,x,.23,z,.15,.4,.17)
  box(THREE,parent,skin,0,.87,-.36,.30,.3,.28)
  box(THREE,parent,hair,0,1.04,-.33,.32,.065,.32)
  for(const z of [-.2,0,.2]) box(THREE,parent,money,0,.71,z,.3,.016,.13)
