...
<div className="flex items-center gap-0.5 sm:gap-1">{items.map((item)=><div key={item.emoji} className="flex h-7 sm:h-9 items-center gap-0.5 px-0.5 sm:px-1 font-mono text-[0.88rem] sm:text-[0.90rem] font-black" style={{color:item.color}}><span>{item.emoji}</span><span>{item.value.toFixed(0)}%</span></div>)}{dice&&<div className="flex h-7 sm:h-9 items-center gap-0.5 px-0.5 sm:px-1 font-mono text-[0.88rem] sm:text-[0.90rem] font-black" style={{color:dice.active?dice.color:'#334155'}}><span>🎲</span><span>{dice.active?`${diceSign}${diceModPct}%`:'0%'}</span></div>}</div>
<div className="group relative flex h-7 items-center gap-[3px] px-0.5 sm:px-1 font-mono text-[0.82rem] font-black sm:h-9 sm:text-[0.90rem]" title={isSpanish?`logados: ${activeWallets.length} / wallets: ${totalWallets} · IRC: ${ircConnectedCount}`:`online: ${activeWallets.length} / wallets: ${totalWallets} · IRC: ${ircConnectedCount}`}><span className="text-emerald-400 tabular-nums">{activeWallets.length}</span><span className="text-slate-600 text-[0.70rem]">/</span><span className="text-slate-500 tabular-nums">{totalWallets}</span><span className="text-slate-600 text-[0.65rem]">wallets</span><span className="text-slate-700 mx-[1px]">·</span><span className="text-cyan-700 tabular-nums">{ircConnectedCount}</span><span className="text-cyan-900 text-[0.65rem]">irc</span></div>
</div>);
}
