...
<div className="flex items-center gap-0.5 sm:gap-1">{items.map((item)=><div key={item.emoji} className="flex h-7 sm:h-9 items-center gap-0.5 px-0.5 sm:px-1 font-mono text-[0.58rem] sm:text-[0.68rem] font-black" style={{color:item.color}}><span>{item.emoji}</span><span>{item.value.toFixed(0)}%</span></div>)}{dice&&<div className="flex h-7 sm:h-9 items-center gap-0.5 px-0.5 sm:px-1 font-mono text-[0.58rem] sm:text-[0.68rem] font-black" style={{color:dice.active?dice.color:'#334155'}}><span>🎲</span><span>{dice.active?`${diceSign}${diceModPct}%`:'0%'}</span></div>}</div>
