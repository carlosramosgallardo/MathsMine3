  ...
  return (
    <div className="mx-auto max-w-3xl font-mono">
      ...
      <div className="rounded-xl p-1" style={{ background: 'linear-gradient(180deg,#050810 0%, #08110d 100%)' }}>
          {!account ? (
          <div className="rounded-lg border border-cyan-500/25 bg-black/60 p-4 text-center text-cyan-200">
          ...
          <>
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div className="flex flex-wrap items-center gap-3">
                <div
                  className="relative inline-grid grid-cols-2 rounded-full border bg-black/70 p-1"
                ...
                >
                  ...
                  <button
                    ...
                    className="relative z-10 min-w-[92px] rounded-full px-4 py-2 text-xs font-black uppercase tracking-[0.2em] transition"
              ...
              <div className="flex items-center gap-2">
                <div
                  className="rounded-lg border bg-black/70 px-3 py-2 text-[0.58rem] font-black uppercase tracking-[0.18em]"
                ...
                <button
                  ...
                  className="rounded-lg border bg-black/70 px-4 py-2 text-[0.62rem] font-black uppercase tracking-[0.22em] transition"
                  style={{
            ...
            <div className="mb-4 grid gap-3 sm:grid-cols-4">
              ...
              { label: t('tradeBoard.rate'), value: `${formatMoney(activeRate, currency)} / MM3` },
                ...
                <div key={item.label} className="rounded-lg border bg-black/70 p-3" style={{ borderColor: tier.glow }}>
                  <div className="text-[0.58rem] uppercase tracking-[0.24em]" style={{ color: `${tier.color}99` }}>{item.label}</div>
                  <div className="mt-1 text-base font-black" style={{ color: tier.color }}>{item.value}</div>
                </div>
              ))}
            </div>

            <div className="mb-4 rounded-lg border bg-black/65 p-4" style={{ borderColor: tier.glow }}>
              <div className="mb-4 rounded-lg border border-cyan-500/20 bg-black/50 p-3">
                <div className="mb-2 flex items-center justify-between gap-3 text-[0.58rem] uppercase tracking-[0.22em]" style={{ color: `${tier.color}AA` }}>
                ...
                />
                <div className="mt-2 flex items-center justify-between text-[0.62rem] text-slate-500">
                ...
                {mode === 'buy' && !canBuy ? (
                  <div className="mt-2 text-[0.62rem] uppercase tracking-[0.18em] text-amber-300/80">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg border p-3" style={{ borderColor: tier.glow, background: tier.bg }}>
              <div className="text-[0.58rem] uppercase tracking-[0.22em]" style={{ color: `${tier.color}AA` }}>{t('tradeBoard.youReceive')}</div>
                  <div className="mt-1 text-2xl font-black" style={{ color: tier.color }}>
                    {mode === 'buy' ? `${fmtMm3(activeQuote.netMm3)} MM3` : formatMoney(activeQuote[quoteField('net', currency)] || 0, currency)}
                  </div>
                  <div className="mt-2 text-[0.58rem] leading-relaxed text-cyan-200/70">
                ...
                <div className="rounded-lg border border-amber-300/20 bg-amber-500/5 p-3">
                  <div className="flex items-center justify-between">
                    <div className="text-[0.58rem] uppercase tracking-[0.22em] text-amber-200/60">{t('tradeBoard.commission')}</div>
                    {diceState?.active && (
                      <div
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              {t('tradeBoard.fictionalTrade')}
              </div>
              <button
                onClick={handleTrade}
                disabled={loading || processing || !canTradeToday || (mode === 'buy' ? !canBuy : !canSell)}
                className="rounded-lg border-2 px-6 py-3 text-sm font-black uppercase tracking-[0.22em] transition disabled:opacity-50"
            ...
            {showTransactions && (
              <div ref={ledgerRef} className="mt-4 rounded-lg border bg-black/70 p-4" style={{ borderColor: tier.glow }}>
