...
export default function Leaderboard({ itemsPerPage = 50 }) {
  ...
  }, [leaderboard, quoteCurrency, selectedWallet, sortConfig]);
  ...
  const getPlacementDisplay = (position) => {
    if (position === 1) return { label: '☢', title: '1 :: reactor core // chain meltdown king' };
    if (position === 2) return { label: '🛸', title: '2 :: orbital miner // signal still unstable' };
    if (position === 3) return { label: '🧬', title: '3 :: mutant hash // lab-grade streak' };
  ...
  return (
    <div className="w-full">
      ...
      <style>{`
        ...
        .lb-tbl thead th { color:#22d3ee; font-weight:700; padding:.65rem .5rem; text-align:left; border-right:1px solid rgba(34,211,238,.15); font-size:.7rem; letter-spacing:.05em; }
        @media(min-width:640px){ .lb-tbl thead th { padding:.85rem; font-size:.8rem; } }
        .lb-tbl thead th:last-child { border-right:none; }
        .lb-tbl tbody tr:nth-child(odd) { background:rgba(34,211,238,.015); }
        .lb-tbl td { padding:.6rem .5rem; border-bottom:1px solid rgba(34,211,238,.1); font-size:.72rem; }
        @media(min-width:640px){ .lb-tbl td { padding:.85rem; font-size:.8rem; } }
      ...
      <div className="space-y-3 sm:hidden">
        {isLoading ? (
          ...
          return (
            <article key={entry.wallet} className={`lb-card rounded-xl p-3${isActiveWallet ? ' wallet-active' : ''}${isSelectedWallet ? ' wallet-selected' : ''}`}>
              ...
              <div className="mt-2 flex min-h-7 items-center justify-center gap-1 rounded-lg border border-cyan-500/10 bg-black/50 px-2">
                {ownedEmojis.length > 0
                  ? ownedEmojis.map((emoji, idx) => (
                      <span key={idx} title={getEmojiTitle(emoji)} className="cursor-default text-base leading-none">
                        {emoji}
                      </span>
                    ))
                  : <span className="font-mono text-[0.62rem] text-cyan-900/60">NFTmoji —</span>
                }
              </div>
            </article>
          );
        ...
        <table className="lb-tbl w-full">
          <thead>
            <tr>
              ...
              <th style={{ width:'42%' }}><SortButton sortKey="wallet">{t('leaderboard.minerWallet')}</SortButton></th>
              <th style={{ width:'10%', textAlign:'center' }} title="NFTmoji — probability artifacts that influence MM3 global value"><SortButton sortKey="nftmoji" className="justify-center">NFTmoji</SortButton></th>
          ...
          <tbody>
            {isLoading ? (
                  ...
                  <td style={{ textAlign:'center' }}>
                    <div className="flex items-center justify-center gap-0.5 flex-wrap">
                      {ownedEmojis.length > 0
                        ...
                        : <span className="text-cyan-900/50 text-[0.65rem]">—</span>
                      }
                    </div>
                  </td>
                  <td style={{ textAlign:'center' }}>
              ...
              <tr className="lb-row">
                <td colSpan={7} className="text-center py-8 text-gray-500">{t('leaderboard.noMiners')}</td>
              </tr>
            )}
          </tbody>
        </table>
