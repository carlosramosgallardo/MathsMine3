...
function marketKeyFromBlockHex(blockHex) {
  const match = String(blockHex || '').trim().toUpperCase().match(/^#?([0-9A-F]{1,3})$/);
  if (!match) return MARKET_FIRST_MINING_BLOCK_KEY;
  const index = parseInt(match[1], 16);
  if (!Number.isFinite(index)) return MARKET_FIRST_MINING_BLOCK_KEY;
  return `ph-${Math.floor(index / 28)}-${index % 28}`;
}
...
export default function Leaderboard({ itemsPerPage = 10 }) {
  ...
  const labels = language === 'es'
    ? {
        ...
        poolMissing: 'Faltan tablas de wallet pools en Supabase — aplica las migraciones en supabase/migrations/.',
    ...
    : {
        ...
        poolMissing: 'Wallet pool tables missing in Supabase — apply migrations in supabase/migrations/.',
  ...
  const openMarketChain = useCallback((entry) => {
    router.push(`/mining?block=${entry?.first_mined_block_key || MARKET_FIRST_MINING_BLOCK_KEY}`);
  }, [router]);
    ...
    try {
      ...
      const [{ data: leaderboardRows, error }, progressResponse, marketOwnersResponse, blocksResponse, txResponse, penaltiesResponse, poolMembersResponse, squeezeNftjiResponse, squeezeLaunchesResponse, minedBlocksResponse] = await Promise.all([
        ...
        supabase
          ...
          .select('wallet, block_hex, chain_index')
      ...
      for (const entry of marketOwnersResponse?.data || []) {
        ...
        const level = Math.max(0, Number(progress?.miningNftjiLevels?.[key] ?? 0) || 0);
      ...
      const firstMinedBlockByWallet = new Map();
      if (!minedBlocksResponse?.error) {
        for (const entry of minedBlocksResponse?.data || []) {
          ...
          if (!firstMinedBlockByWallet.has(wallet)) {
            firstMinedBlockByWallet.set(wallet, {
              block_hex: String(entry.block_hex || '').toUpperCase(),
              block_key: marketKeyFromBlockHex(entry.block_hex),
              chain_index: Number(entry.chain_index) || 0,
            });
          }
        }
      ...
      for (const entry of penaltiesResponse?.data || []) {
        ...
        const existing = penaltiesByWallet.get(wallet) || { mm3: [], money: [] };
        existing[effect].push({
      ...
      const mergedData = [...allWallets]
        .map((normalizedWallet) => {
          ...
          return {
            ...
            first_mined_block_hex: firstMinedBlock?.block_hex || '',
            first_mined_block_key: firstMinedBlock?.block_key || '',
            first_mined_chain_index: Number(firstMinedBlock?.chain_index) || 0,
  ...
  const filteredLeaderboard = useMemo(() => {
    ...
    const getValue = (entry) => {
      ...
      if (sortConfig.key === 'block') return (Array.isArray(entry.market_blocks) ? entry.market_blocks.length : 0) + (entry.active_penalty?.mm3?.length || 0) + (entry.active_penalty?.money?.length || 0);
  ...
  const poolLeaderboard = useMemo(() => {
    ...
    const rows = [...grouped.entries()].map(([poolCode, members]) => {
      ...
      const firstMinedBlock = members
        .filter((entry) => entry.first_mined_block_key)
        .sort((a, b) => (Number(a.first_mined_chain_index) || Number.POSITIVE_INFINITY) - (Number(b.first_mined_chain_index) || Number.POSITIVE_INFINITY))[0] || null;
      ...
      const totalPenalties = members.reduce((sum, entry) => {
        return sum + (entry.active_penalty?.mm3?.length || 0) + (entry.active_penalty?.money?.length || 0);
      }, 0);
      const totalPenaltiesMm3 = members.reduce((sum, entry) => {
        return sum + (entry.active_penalty?.mm3 || []).reduce((s, p) => s + (Number(p.penalty_value) || 0), 0);
      }, 0);
      const totalPenaltiesMoneyEur = members.reduce((sum, entry) => {
        return sum + (entry.active_penalty?.money || []).reduce((s, p) => s + (Number(p.penalty_eur || p.penalty_value) || 0), 0);
      }, 0);
      ...
      const miningNftjiMemberCount = members.filter(m => Array.isArray(m.market_blocks) && m.market_blocks.length > 0).length;
      ...
      const activePenalty = {
        mm3: penalties.flatMap((p) => p?.mm3 || []),
        money: penalties.flatMap((p) => p?.money || []),
      };
      ...
      return {
        ...
        block_chain_percent: blockChainPercent,
        mined_block_count: minedBlockCount,
        first_mined_block_hex: firstMinedBlock?.first_mined_block_hex || '',
        first_mined_block_key: firstMinedBlock?.first_mined_block_key || '',
        first_mined_chain_index: Number(firstMinedBlock?.first_mined_chain_index) || 0,
        ...
        mining_nftji_member_count: miningNftjiMemberCount,
        ...
        active_penalty: activePenalty.mm3.length || activePenalty.money.length ? activePenalty : null,
      };
    });
  ...
  return (
    <div className="w-full">
      <style>{`
        .lb-row { transition: background-color .15s, border-color .15s; border: 1px solid rgba(34,211,238,.15); }
        .lb-row:hover { background: rgba(34,211,238,.05); border-color: rgba(34,211,238,.4); }
        .lb-row.wallet-active { background: rgba(74,222,128,.08); border-color: rgba(74,222,128,.55); }
        ...
        .lb-card { border:1px solid rgba(34,211,238,.18); background:rgba(2,6,11,.94); }
        .lb-card.wallet-active { border-color:rgba(74,222,128,.55); }
        .lb-card.wallet-selected { border-color:rgba(34,211,238,.75); }
                ...
                <div className="flex items-center gap-0.5 shrink-0">
                  {TRADE_SLOT_ORDER.map((slot) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: showLvl ? '0.72rem' : '0.88rem', lineHeight: 1, ...lifeNftjiEmojiFilterStyle(slot.emoji) }}>{owned ? slot.emoji : ''}</span>
                  ...
                  {SQUEEZE_SLOT_ORDER.map((slot) => {
                    ...
                    return (
                      <div
                        key={slot.key}
                        title={owned ? `${getEmojiTitle(slot.emoji)} Lv.${lvlSum}` : getEmojiTitle(slot.emoji)}
                        className="relative flex flex-col items-center justify-center rounded border"
                        style={{
                          width: '1.5rem', height: '1.5rem',
                          borderColor: owned ? `${color}99` : `${color}33`,
                      ...
                      >
                        <span style={{ fontSize: owned ? '0.72rem' : '0.88rem', lineHeight: 1, ...lifeNftjiEmojiFilterStyle(slot.emoji) }}>{owned ? slot.emoji : ''}</span>
                        {owned && <span style={{ fontSize: '0.48rem', fontFamily: 'monospace', color, fontWeight: 800, lineHeight: 1 }}>Lv.{lvlSum}</span>}
                      </div>
                    );
                  })}
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: relayOwned ? '0.78rem' : '0.88rem', lineHeight: 1, filter: relayOwned ? 'drop-shadow(0 0 3px #6ee7b7)' : 'none' }}>{relayOwned ? WALLET_DECORATIONS.relay : ''}</span>
          ...
          <thead>
            {viewMode === 'pools' ? (
              <tr>
                ...
                <th style={{ width:'8%', textAlign:'center' }}><SortButton sortKey="execs" className="justify-center">{t('ranking.execs')}</SortButton></th>
                <th style={{ width:'10%', textAlign:'center' }}><SortButton sortKey="block" className="justify-center">Pen.</SortButton></th>
                <th style={{ width:'8%', textAlign:'center' }}><SortButton sortKey="level" className="justify-center">{t('ranking.level')}</SortButton></th>
                <th style={{ width:'8%', textAlign:'center' }}><SortButton sortKey="rank" className="justify-center">{t('ranking.rank')}</SortButton></th>
                <th style={{ width:'11%', textAlign:'right', paddingRight:'1rem' }}><SortButton sortKey="available_mm3" className="justify-end">{t('ranking.mm3Earned')}</SortButton></th>
                <th style={{ width:'12%', textAlign:'right', paddingRight:'1rem' }}><SortButton sortKey="money" className="justify-end">{t('ranking.sellValue')}</SortButton></th>
              </tr>
            ) : (
              <tr>
                <th style={{ width:'7%', textAlign:'center' }}><SortButton sortKey="block_chain_percent" className="justify-center">MM3 Chain</SortButton></th>
                ...
                <th style={{ width:'6%', textAlign:'center' }}><SortButton sortKey="level" className="justify-center">{t('ranking.level')}</SortButton></th>
                <th style={{ width:'7%', textAlign:'center' }}><SortButton sortKey="rank" className="justify-center">{t('ranking.rank')}</SortButton></th>
                <th style={{ width:'9%', textAlign:'right', paddingRight:'1rem' }}><SortButton sortKey="available_mm3" className="justify-end">{t('ranking.mm3Earned')}</SortButton></th>
                <th style={{ width:'9%', textAlign:'right', paddingRight:'1rem' }}><SortButton sortKey="money" className="justify-end">{t('ranking.sellValue')}</SortButton></th>
              </tr>
            )}
          </thead>
          <tbody>
                    ...
                    <div className="flex items-center justify-center gap-1">
                      {TRADE_SLOT_ORDER.map((slot) => {
                        ...
                        return (
                          <div
                            key={slot.key}
                            title={owned ? `${getEmojiTitle(slot.emoji)}${!isLife ? ` Lv.${lvlSum}` : ''}` : getEmojiTitle(slot.emoji)}
                      ...
                      {(() => {
                        ...
                        return (
                          ...
                          >
                            <span style={{ fontSize: relayOwned ? '0.84rem' : '0.95rem', lineHeight: 1, filter: relayOwned ? 'drop-shadow(0 0 3px #6ee7b7)' : 'none' }}>{relayOwned ? WALLET_DECORATIONS.relay : ''}</span>
                            {relayOwned && (
                              <span style={{
                ...
                <tr key={entry.wallet} className={`lb-row${isActiveWallet ? ' wallet-active' : ''}${isSelectedWallet ? ' wallet-selected' : ''}${globalRank <= 3 ? ` rank-${globalRank}` : ''}`}>
                  ...
                  <td style={{ textAlign:'center' }}>
                    {entry.pool_code ? (
                      ...
                      <span className="text-[0.7rem] uppercase tracking-[0.12em] text-slate-700">{t('ranking.none')}</span>
                    )}
                  </td>
                  <td style={{ textAlign:'center' }}>
                    <div className="flex items-center justify-center gap-1">
                      {TRADE_SLOT_ORDER.map((slot) => {
                        ...
                        return (
                          ...
                          >
                            <span style={{ fontSize: showLvl ? '0.78rem' : '0.95rem', lineHeight: 1, ...lifeNftjiEmojiFilterStyle(slot.emoji) }}>{owned ? slot.emoji : ''}</span>
                            {showLvl && (
                              <span style={{
                                fontSize: '0.52rem', fontFamily: 'monospace',
