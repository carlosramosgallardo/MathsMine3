...
import { normalizeWalletDecorations, getEmojiTitle } from '@/lib/wallet-decorations'
...
function ConnectedBar({ address, isRealWallet, onDisconnect, mode = 'full' }) {
  ...
  const visibleAddress = useMemo(() => (!address ? '' : `${address.slice(0, 8)}…${address.slice(-6)}`), [address])
  ...
  const copyAddress = async () => {
    ...
    try {
      ...
      notify(t('wallet.copied'), 'success')
    } catch {
      notify(t('wallet.copyFailed'), 'error')
    }
  }
  ...
  const active = `border-emerald-500/50 text-emerald-400 bg-black hover:border-emerald-400 hover:bg-emerald-950/40 hover:shadow-[0_0_8px_rgba(74,222,128,0.3)]`
  const positionGlyph = walletSummary?.position === 1 ? '☢' : walletSummary?.position === 2 ? '🛸' : walletSummary?.position === 3 ? '🧬' : (walletSummary?.position ? String(walletSummary.position) : '·')
    ...
    <div className="flex items-center gap-1.5">
      {mode !== 'controls' && (
        <button
          type="button"
          onClick={copyAddress}
          className={`flex items-center rounded-md border border-transparent font-mono tracking-wide transition hover:bg-cyan-950/20 focus:border-transparent focus:outline-none focus:ring-0 active:border-transparent ${mode === 'wallet' ? 'min-h-5 px-1.5 py-0.5 text-[0.54rem] sm:text-[0.58rem]' : 'h-9 px-2 text-[0.56rem] sm:text-[0.6rem]'}`}
          style={{ color: walletColor, textShadow: `0 0 10px ${walletColor}33` }}
          title={`${pathname === '/ranking' ? t('leaderboard.toggleMyWallet') : t('wallet.copyAddress')}: ${address}`}
        >
          {mode === 'wallet' ? (
            <div className="flex items-center gap-1.5">
              <span className="text-[0.76rem] leading-none" title={walletSummary?.position ? `#${walletSummary.position}` : 'shell'}>
                {positionGlyph}
              </span>
              <span className="max-w-[24ch] truncate">{visibleAddress}</span>
              {walletSummary?.emojis?.length ? (
                <span className="flex items-center gap-0.5">
                  {walletSummary.emojis.map((emoji, idx) => (
                    <span key={`${emoji}-${idx}`} className="text-[0.72rem] leading-none" title={getEmojiTitle(emoji)}>
                      {emoji}
                    </span>
                  ))}
                </span>
              ...
              {walletSummary ? (
                <>
                  <span title={`${t('leaderboard.level')}: ${walletSummary.level}`} className="text-cyan-300/90">{walletSummary.level}</span>
                  <span title={`${t('leaderboard.rank')}: ${walletSummary.tier.label}`} style={{ color: walletSummary.tier.color }}>{walletSummary.tier.emoji}</span>
                  <span title={`${t('leaderboard.mm3Earned')}: ${walletSummary.availableMm3.toFixed(8)}`} className="text-cyan-300/90">{mm3Compact}</span>
...
function SplitConnectButton({ onGoogleClick, onWalletClick, googleBusy, walletBusy, err, noGoogle }) {
  ...
  const borderColor = 'rgba(74,222,128,0.35)'
  const dividerColor = 'rgba(74,222,128,0.18)'
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title={googleTitle}
        className="flex h-full items-center justify-center px-2.5 bg-black transition-colors duration-150 disabled:cursor-not-allowed focus:outline-none"
        style={{
          borderRight: `1px solid ${dividerColor}`,
          background: hoverG && !noGoogle && !googleBusy ? 'rgba(74,222,128,0.08)' : 'black',
        }}
      ...
      <button
        ...
        disabled={walletBusy}
        className="relative z-10 flex h-full min-w-10 items-center justify-center px-2.5 bg-black transition-colors duration-150 focus:outline-none disabled:cursor-not-allowed"
        style={{ background: hoverW && !walletBusy ? 'rgba(34,211,238,0.08)' : 'black' }}
