...
BEGIN
  ...
  END IF;
  -- Anon can only attack other anon players, never logged-in wallets
  IF p_attacker LIKE 'anon-%' AND NOT p_victim_is_anon THEN
  ...
  IF NOT p_victim_is_anon THEN
    ...
    IF v_stolen_eur > 0 OR v_stolen_usd > 0 OR v_stolen_cny > 0 THEN
      UPDATE player_progress SET
        ...
        updated_at = NOW()
      WHERE wallet = p_victim;
      -- Only credit attacker if they are logged in (anon has no balance)
      IF p_attacker NOT LIKE 'anon-%' THEN
        UPDATE player_progress SET
  ...
  RETURN jsonb_build_object(
    ...
    'attacker_balances', CASE WHEN p_attacker LIKE 'anon-%' THEN NULL ELSE jsonb_build_object('EUR',v_attacker_eur,'USD',v_attacker_usd,'CNY',v_attacker_cny) END,
